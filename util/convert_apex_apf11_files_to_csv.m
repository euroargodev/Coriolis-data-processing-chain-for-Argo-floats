...
% output directory
DIR_OUTPUT_CSV_FILES = 'C:\Users\jprannou\_RNU\DecApx_info\APEX_APF11\IRIDIUM_RUDICS\20181220\CSV\';
...
for idFloat = 1:nbFloats
   ...
   for iFile = 1:length(binSciFiles)
      [error, ~] = read_apx_apf11_ir_binary_log_file([floatFileDir binSciFiles(iFile).name], 'science', 1);
      if (error)
   ...
   for iFile = 1:length(binVitFiles)
      [error, ~] = read_apx_apf11_ir_binary_log_file([floatFileDir binVitFiles(iFile).name], 'vitals', 1);
      if (error)
