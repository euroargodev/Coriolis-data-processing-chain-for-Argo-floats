...
%
% SYNTAX :
%   nc_update_argo_format_set_DEPLOYMENT_CRUISE_ID_08aa or
%   nc_update_argo_format_set_DEPLOYMENT_CRUISE_ID_08aa(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   12/20/2021 - RNU - V 08aa: update meta.nc files :
%                              - set DEPLOYMENT_CRUISE_ID provided by an
%                              external file (export of CRUISE_NAME stored in DB)
% ------------------------------------------------------------------------------
function nc_update_argo_format_set_DEPLOYMENT_CRUISE_ID_08aa(varargin)
...
g_couf_ncUpdateArgoFormatVersion = '08aa';
...
try
   ...
   if (rem(size(fileContents, 1), 4) ~= 0)
      fprintf('ERROR: Unable to parse file: %s\n', DIR_INPUT_CSV_FILE);
      return
   end

   inputData = reshape(fileContents, 4, size(fileContents, 1)/4)';
   clear fileContents
   ...
   tmpDir = [DIR_TMP '/' 'nc_update_argo_format_set_DEPLOYMENT_CRUISE_ID_08aa_' currentTime];
   status = mkdir(tmpDir);
   if (status ~= 1)
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_update_argo_format_set_DEPLOYMENT_CRUISE_ID_08aa_' currentTime '.log'];
   diary(logFile);
                     ...
                     if (length(idF) == 1)
                        ...
                        for idFile = 1:length(floatFiles)
                           ...
                           if (exist(floatFilePathName, 'file') == 2)
                              process_nc_file(floatFilePathName, tmpDir, dataFloat);
                           end
                        end
                     end
...
%
% SYNTAX :
%  process_nc_file(a_ncPathFileName, a_tmpDir, a_inputData)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   12/20/2021 - RNU - creation
% ------------------------------------------------------------------------------
function process_nc_file(a_ncPathFileName, a_tmpDir, a_inputData)
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   if (~isempty(ncData))
      ...
      if (strcmp(formatVersion, '3.1'))
         ...
         idVal = find(strcmp('DEPLOYMENT_CRUISE_ID', ncData(1:2:end)) == 1, 1);
         deploymentCruiseId = strtrim(ncData{2*idVal}');

         if (~strcmp(deploymentCruiseId, a_inputData))
            updateNeeded = 1;
         end
      end
   end
...
%
% SYNTAX :
%  [o_ok] = update_file(a_ncPathFileName, a_inputData)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_ok] = update_file(a_ncPathFileName, a_inputData)
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   valueStr = repmat(' ', 1, 32);
   valueStr(1:length(a_inputData)) = a_inputData;
   netcdf.putVar(fCdf, netcdf.inqVarID(fCdf, 'DEPLOYMENT_CRUISE_ID'), ...
      0, length(valueStr), valueStr);
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis update format tool (nc_update_argo_format_set_DEPLOYMENT_CRUISE_ID_08aa)'));
docRootNode.appendChild(newChild);
