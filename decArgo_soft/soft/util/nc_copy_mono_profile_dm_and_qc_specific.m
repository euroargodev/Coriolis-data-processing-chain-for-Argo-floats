...
%                             has been modified
%   09/06/2022 - RNU - V 1.9: version of nc_copy_mono_profile_dm_and_qc only
%   09/15/2022 - RNU - V 1.10: version of nc_copy_mono_profile_dm_and_qc only
%   09/15/2022 - RNU - V 1.11: finalize output data set (when a BD file
%                              disapeared)
%   09/11/2023 - RNU - V 1.12: - all parameter data of DATA_MODE(N_PROF) = 'D'
...
%                              - if present in DM profile SCIENTIFIC_CALIB_* of
%                              MTIME should also be fill with 'not applicable'
%   09/13/2023 - RNU - V 1.13: added KEEP_PROFILE_LOCATION_FOR_ALL_FLAG to
%                              report the location information of the first DM
%                              profile on all the profiles of the file
%   09/19/2023 - RNU - V 1.14: error while accessing to PARAMETER(N_PROF,
%                              N_CALIB, N_PARAM, STRING16)
% ------------------------------------------------------------------------------
function nc_copy_mono_profile_dm_and_qc_specific(varargin)
...
% top directory of OLD input NetCDF files containing the Qc values and DM data
...
g_cocd_ncCopyMonoProfileDmAndQcVersion = '1.14';
...
logFile = [DIR_LOG_FILE '/' 'nc_copy_mono_profile_dm_and_qc_specific_' datestr(now, 'yyyymmddTHHMMSS') '.log'];

% process the files according to input and configuration parameters
nc_copy_mono_profile_dm_and_qc_specific_(floatList, logFile, ...
