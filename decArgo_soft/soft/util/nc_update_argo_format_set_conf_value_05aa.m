...
%
% SYNTAX :
%   nc_update_argo_format_set_conf_value_05aa or nc_update_argo_format_set_conf_value_05aa(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   09/14/2016 - RNU - V 05aa: replace NaN configuration values by associated
%                              _FillValue values
% ------------------------------------------------------------------------------
function nc_update_argo_format_set_conf_value_05aa(varargin)
...
g_couf_ncUpdateArgoFormatVersion = '05aa';
...
try
   ...
   tmpDir = [DIR_TMP '/' 'nc_update_argo_format_set_conf_value_05aa_' currentTime];
   status = mkdir(tmpDir);
   if (status ~= 1)
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_update_argo_format_set_conf_value_05aa_' currentTime '.log'];
   diary(logFile);
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   if (~isempty(ncData))
      ...
      if (strcmp(formatVersion, '3.1'))
         ...
         idVal = find(strcmp('LAUNCH_CONFIG_PARAMETER_VALUE', ncData(1:2:end)) == 1, 1);
         launchConfigParamValue = ncData{2*idVal};
         idVal = find(strcmp('CONFIG_PARAMETER_VALUE', ncData(1:2:end)) == 1, 1);
         configParamValue = ncData{2*idVal};
         
         if (~isempty(find(isnan(launchConfigParamValue) == 1, 1)) || ...
               ~isempty(find(isnan(configParamValue) == 1, 1)))
            updateNeeded = 1;
         end
      end
   end
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   % update LAUNCH_CONFIG_PARAMETER_VALUE and CONFIG_PARAMETER_VALUE
   varList = [{'LAUNCH_CONFIG_PARAMETER_VALUE'} {'CONFIG_PARAMETER_VALUE'}];
   for idV = 1:length(varList)
      varName = varList{idV};
      if (var_is_present_dec_argo(fCdf, varName))
         varValue = netcdf.getVar(fCdf, netcdf.inqVarID(fCdf, varName));
         varValue(isnan(varValue)) = netcdf.getAtt(fCdf, netcdf.inqVarID(fCdf, varName), '_FillValue');
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis update format tool (nc_update_argo_format_set_conf_value_05aa)'));
docRootNode.appendChild(newChild);
