...
fprintf(fidOut, 'DAC;WMO;FILE_TYPE;FILE_NAME;COMMENT\n');
      ...
      if (~isempty(idF))
         ...
         if (exist(ncInputFileDir, 'dir') == 7)
            ...
            if (~any(strcmp(paramNameList, 'MTIME')))
               ...
               fprintf(fidOut, '%s;%d;%s;%s;%s\n', ...
                  dirName1, floatNum, 'META', sprintf('%d_meta.nc', floatNum), '''MTIME'' missing');
            end
               ...
               for idFile = 1:length(ncInputFiles)
                  ...
                  if (monoProfInputFileName(1) == 'B')
                     if (any(strcmp(paramNameList, 'MTIME')))
                        ...
                        fprintf(fidOut, '%s;%d;%s;%s;%s\n', ...
                           dirName1, floatNum, 'B-PROF', monoProfInputFileName, '''MTIME'' present');
                     end
                  else
                     if (~any(strcmp(paramNameList, 'MTIME')))
                        % fprintf('ERROR: Float %d: ''MTIME'' missing in file : %s\n', floatNum, monoProfInputFileName);
                        fprintf(fidOut, '%s;%d;%s;%s;%s\n', ...
                           dirName1, floatNum, 'C-PROF', monoProfInputFileName, '''MTIME'' missing');
                     end
                  end
               end
            else
