...
%
% SYNTAX :
%   nc_update_argo_format_set_SPECIAL_FEATURES_09aa or
%   nc_update_argo_format_set_SPECIAL_FEATURES_09aa(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   01/10/2022 - RNU - V 09aa: update meta.nc files :
%                              - set SPECIAL_FEATURES provided by an external
%                              file 
% ------------------------------------------------------------------------------
function nc_update_argo_format_set_SPECIAL_FEATURES_09aa(varargin)
...
g_couf_ncUpdateArgoFormatVersion = '09aa';
...
try
   ...
   tmpDir = [DIR_TMP '/' 'nc_update_argo_format_set_SPECIAL_FEATURES_09aa_' currentTime];
   status = mkdir(tmpDir);
   if (status ~= 1)
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_update_argo_format_set_SPECIAL_FEATURES_09aa_' currentTime '.log'];
   diary(logFile);
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   if (~isempty(ncData))
      ...
      if (strcmp(formatVersion, '3.1'))
         ...
         idVal = find(strcmp('SPECIAL_FEATURES', ncData(1:2:end)) == 1, 1);
         specialFeatures = strtrim(ncData{2*idVal}');

         if (~strcmp(specialFeatures, a_inputData))
            updateNeeded = 1;
         end
      end
   end
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   valueStr = repmat(' ', 1, 1024);
   valueStr(1:length(a_inputData)) = a_inputData;
   netcdf.putVar(fCdf, netcdf.inqVarID(fCdf, 'SPECIAL_FEATURES'), ...
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis update format tool (nc_update_argo_format_set_SPECIAL_FEATURES_09aa)'));
docRootNode.appendChild(newChild);
