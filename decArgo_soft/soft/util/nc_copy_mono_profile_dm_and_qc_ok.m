...
% retrieve data from 4 files
[profCDataOld, profCDataNew, profBDataOld, profBDataNew] = ...
   get_data_from_file(a_profFileNameOld, a_profFileNameNew, ...
   a_bProfFileNameOld, a_bProfFileNameNew);
...
function process_cycle_file( ...
   ...
   a_ignoredParameterList, a_ignoredParameterListAll, ...
   a_dirOutputNcFiles, a_bFileFlag, ...
   a_profCDataOld, a_profCDataNew, a_profBDataOld, a_profBDataNew)
...
if (a_bFileFlag)
   fileType = 'B';
   profDataOld = a_profBDataOld;
   profDataNew = a_profBDataNew;
   profBisDataOld = a_profCDataOld;
   profBisDataNew = a_profCDataNew;
else
   fileType = 'C';
   profDataOld = a_profCDataOld;
   profDataNew = a_profCDataNew;
   profBisDataOld = a_profBDataOld;
   profBisDataNew = a_profBDataNew;
end
...
for idLoop = 1:2
   ...
   wantedVarsBis = [];
   % process new files first because new files may have paameters stored in
   % B-files that have been moved to C-files
   if (idLoop == 2)
...
% manage statistical parameters
if (any(dataModeOld == 'D'))
   if (a_bFileFlag)
      % icicic
      a=1
   else
      % if a cycle is in DM, check that SCIENTIFIC_CALIB_* of statistical
      % parameters are set to 'not applicable'
      ...
      if (needUpdate == 1)
         ...
         % update 'info' variables with the N_PARAM dimension
...
% %   08/10/2018 - RNU - creation
% % ------------------------------------------------------------------------------
function [o_profCDataOld, o_profCDataNew, o_profBDataOld, o_profBDataNew] = ...
   get_data_from_file( ...
   a_profCFileNameOld, a_profCFileNameNew, ...
   a_profBFileNameOld, a_profBFileNameNew)
...
o_profCDataOld = get_data_from_nc_file(a_profCFileNameOld, wantedVars);
o_profCDataNew = get_data_from_nc_file(a_profCFileNameNew, wantedVars);
o_profBDataOld = get_data_from_nc_file(a_profBFileNameOld, wantedVars);
o_profBDataNew = get_data_from_nc_file(a_profBFileNameNew, wantedVars);
...
if (~isempty(a_dmProfIdToUpdate))
   ...
   for idProf = 1:length(a_dmProfIdToUpdate)
      ...
      %       stationParametersOld = a_profStructOld.info.STATION_PARAMETERS;
      %       if (ndims(stationParametersOld) == 3)
      %          nProf = size(stationParametersOld, 1);
      %          nParam = size(stationParametersOld, 2);
      %       elseif (ndims(stationParametersOld) == 2)
      %          nProf = 1;
      %          nParam = size(stationParametersOld, 1);
      %       end
      %       stationParametersVarId = netcdf.inqVarID(fCdf, 'STATION_PARAMETERS');
      %       for idParam = 1:nParam
      %          if (nProf == 1)
      %             valueStr = stationParametersOld(idParam, :);
      %          else
      %             valueStr = squeeze(stationParametersOld(profId, idParam, :))';
      %          end
