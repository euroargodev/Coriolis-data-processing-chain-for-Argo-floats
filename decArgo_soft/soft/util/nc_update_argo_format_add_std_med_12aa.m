...
%
% SYNTAX :
%   nc_update_argo_format_add_std_med_12aa or 
%   nc_update_argo_format_add_std_med_12aa(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   03/21/2022 - RNU - V 12aa: move <PARAM>_STD and <PARAM>_MED parameters as
%                              intermediate parameters
% ------------------------------------------------------------------------------
function nc_update_argo_format_add_std_med_12aa(varargin)
...
DIR_INPUT_OUTPUT_NC_FILES = 'C:\Users\jprannou\_DATA\OUT\test_add_std_med\';
...
g_couf_ncUpdateArgoFormatVersion = '12aa';
...
try
   ...
   tmpDir = [DIR_TMP '/' 'nc_update_argo_format_add_std_med_12aa_' currentTime];
   status = mkdir(tmpDir);
   if (status ~= 1)
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_update_argo_format_add_std_med_12aa_' currentTime '.log'];
   diary(logFile);
   ...
   if (strcmp(formatVersion, '3.1'))
      ...
      for idProf = 1:nProf
         for idParam = 1:nParam
            ...
            if (strcmp(paramName(end-3:end), '_MED'))
               updateNeeded = 1;
               break
            end
         end
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis update format tool (nc_update_argo_format_add_std_med_12aa)'));
docRootNode.appendChild(newChild);
