...
%    a_floatWmo, a_floatLoginName, a_rsyncDir1, a_rsyncDir2, a_spoolDir, a_outputDirName, a_maxFileAge)
%
% INPUT PARAMETERS :
%   a_floatWmo       : float WMO number
%   a_floatLoginName : float login name
%   a_rsyncDir1      : RSYNC #1 directory
%   a_rsyncDir2      : RSYNC #2 directory
...
% copy files from DIR_INPUT_RSYNC_DATA_1
fprintf('DIR_INPUT_RSYNC_DATA_1 files (%s):\n', a_rsyncDir1);

fileNameList = [];
for idType = 1:length(fileTypeList)
   if (ismember(fileTypeList{idType, 1}, [3 4 5]))
      files = dir([a_rsyncDir1 '/' a_floatLoginName '/' fileTypeList{idType, 2}]);
...
for idFile = 1:length(fileNameList)
   ...
   filePathName = [a_rsyncDir1 '/' a_floatLoginName '/' fileName];
...
% copy files from DIR_INPUT_RSYNC_DATA_2
fprintf('DIR_INPUT_RSYNC_DATA_2 files (%s):\n', a_rsyncDir2);

fileNameList = [];
for idType = 1:length(fileTypeList)
   if (ismember(fileTypeList{idType, 1}, [3 4 5]))
      files = dir([a_rsyncDir2 '/' a_floatLoginName '/' fileTypeList{idType, 2}]);
      for idFile = 1:length(files)
         if ((curUtcDate - files(idFile).datenum) <= a_maxFileAge/24)
            fileNameList{end+1} = files(idFile).name;
         end
...
for idFile = 1:length(fileNameList)
   fileName = fileNameList{idFile};
   filePathName = [a_rsyncDir2 '/' a_floatLoginName '/' fileName];
   fileInfo = dir(filePathName);
   fileNameOut = [ ...
      fileName(1:end-4) '_' ...
      datestr(datenum(fileInfo.date, 'dd-mmmm-yyyy HH:MM:SS'), 'yyyymmddHHMMSS') ...
