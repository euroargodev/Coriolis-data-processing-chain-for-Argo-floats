...
% in a C file, if statistical parameters (MTIME, <PARAM>_MED or <PARAM>_STD) are
% present in a profile in DM, we should fill SCIENTIFIC_CALIB_* with 'not applicable'
if (any(dataModeOld == 'D'))
   if (~a_bFileFlag)
      idCyNumList = find(dataModeOld == 'D');
      needUpdate = 0;
      for idCy = idCyNumList
         paramNewIdList = find(strcmp(profStructNew.parameterList{idCy}, 'MTIME'));
         paramOldIdList = find(strcmp(profStructOld.parameterList{idCy}, 'MTIME'));
