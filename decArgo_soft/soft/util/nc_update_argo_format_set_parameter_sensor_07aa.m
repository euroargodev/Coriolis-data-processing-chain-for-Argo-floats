...
% 
% SYNTAX :
%   nc_update_argo_format_set_parameter_sensor_07aa or nc_update_argo_format_set_parameter_sensor_07aa(6900189, 7900118)
% 
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   04/18/2021 - RNU - V 07aa: update 'PARAMETER_SENSOR' value:
%                              - replace 'CTD_PSAL' by 'CTD_CNDC'
% ------------------------------------------------------------------------------
function nc_update_argo_format_set_parameter_sensor_07aa(varargin)
...
g_couf_ncUpdateArgoFormatVersion = '07aa';
...
try
   ...
   tmpDir = [DIR_TMP '/' 'nc_update_argo_format_set_parameter_sensor_07aa_' currentTime];
   status = mkdir(tmpDir);
   if (status ~= 1)
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_update_argo_format_set_parameter_sensor_07aa_' currentTime '.log'];
   diary(logFile);
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   if (~isempty(ncData))
      ...
      if (strcmp(formatVersion, '3.1'))
         ...
         idVal = find(strcmp('PARAMETER_SENSOR', ncData(1:2:end)) == 1, 1);
         parameterSensorList = unique(cellstr(ncData{2*idVal}'));
         for idL = 1:length(parameterSensorList)
            if (any(strfind(parameterSensorList{idL}, 'CTD_PSAL')))
               updateNeeded = 1;
               break
            end
         end
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   varName = 'PARAMETER_SENSOR';
   if (var_is_present_dec_argo(fCdf, varName))
      varValue = netcdf.getVar(fCdf, netcdf.inqVarID(fCdf, varName));
      for id = 1:size(varValue, 2)
         if (strcmp(strtrim(varValue(:, id)'), 'CTD_PSAL'))
            valueStr = repmat(' ', 1, size(varValue, 1));
            valueStr(1:length('CTD_CNDC')) = 'CTD_CNDC';
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis update format tool (nc_update_argo_format_set_parameter_sensor_07aa)'));
docRootNode.appendChild(newChild);
