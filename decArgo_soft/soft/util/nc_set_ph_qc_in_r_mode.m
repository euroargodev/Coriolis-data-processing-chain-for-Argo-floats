% ------------------------------------------------------------------------------
% Set PH_IN_SITU_TOTAL_QC = '3' in profile and trajectory files.
%
% SYNTAX :
%   nc_set_ph_qc_in_r_mode or nc_set_ph_qc_in_r_mode(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function nc_set_ph_qc_in_r_mode(varargin)
...
% FLOAT_LIST_FILE_NAME = 'C:\Users\jprannou\_RNU\DecArgo_soft\lists\floats_with_ph_in_snapshot_202309.txt';
...
% temporary directory
DIR_TMP_FILES = 'C:\Users\jprannou\_RNU\DecArgo_soft\work\tempo\';
...
% program version
global g_cosq_setPhQcInRModeVersion;
g_cosq_setPhQcInRModeVersion = '2.0';
...
try
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_set_ph_qc_in_r_mode' name '_' currentTime '.log'];
   diary(logFile);
   tic;
   ...
   for idFloat = 1:nbFloats
      ...
      if (exist(ncInputFileDir, 'dir') == 7)
         ...
         if (exist(ncInputFileDir, 'dir') == 7)
            ...
            ncInputFiles = dir([ncInputFileDir 'BR*.nc']);
            for idFile = 1:length(ncInputFiles)
               ...
               [modifiedFileFlag, tmpOutputFileName] = set_ph_qc_to_profile_file(monoProfInputFilePathName, DIR_TMP_FILES);
         ...
         ncInputFiles = dir([ncInputFileDir '/' sprintf('%d_*traj.nc', floatNum)]);
         for idFile = 1:length(ncInputFiles)

            trajInputFileName = ncInputFiles(idFile).name;
            trajInputFilePathName = [ncInputFileDir '/' trajInputFileName];
            trajOutputFilePathName = '';
            if (updateFiles == 0)
               trajOutputFilePathName = [ncOutputFileDir '/' trajInputFileName];
            end

            fprintf('%s\n', trajInputFileName);

            % process current file
            [modifiedFileFlag, tmpOutputFileName] = set_ph_qc_to_traj_file(trajInputFilePathName, DIR_TMP_FILES);

            if (modifiedFileFlag == 1)

               [trajOutputPath, ~, ~] = fileparts(trajOutputFilePathName);
               [~, fileName, fileExtension] = fileparts(tmpOutputFileName);
               if (g_cosq_doItFlag == 1)
                  move_file(tmpOutputFileName, [trajOutputPath '/' fileName fileExtension]);
               end

               % store the information for the XML report
               g_cosq_reportData.float = [g_cosq_reportData.float g_cosq_floatNum];
               g_cosq_reportData.trajFile = [g_cosq_reportData.trajFile {[trajOutputPath '/' fileName fileExtension]}];
            end
         end
      else
         fprintf('WARNING: Directory not found: %s\n', ncInputFileDir);
...
xmlFileName = [DIR_XML_FILE '/' 'nc_set_ph_qc_in_r_mode' name '_' currentTime '.xml'];
...
% ------------------------------------------------------------------------------
% Set PH_IN_SITU_TOTAL_QC = '3' in trajectory file.
%
% SYNTAX :
%  [o_modifiedFileFlag, o_trajOutputFileName] = ...
%    set_ph_qc_to_traj_file(a_trajInputFileName, a_tmpDirName)
%
% INPUT PARAMETERS :
%   a_trajInputFileName : input NetCDF file name
%   a_tmpDirName        : name of the temporary directory
%
% OUTPUT PARAMETERS :
%   o_modifiedFileFlag   : 1 if the file has been modified, 0 otherwise
%   o_trajOutputFileName : output NetCDF file name
%
% EXAMPLES :
%
% SEE ALSO :
...
%   11/15/2023 - RNU - creation
% ------------------------------------------------------------------------------
function [o_modifiedFileFlag, o_trajOutputFileName] = ...
   set_ph_qc_to_traj_file(a_trajInputFileName, a_tmpDirName)
   
% output parameters initialization
o_modifiedFileFlag = 0;
o_trajOutputFileName = '';
...
[ncTrajData] = get_data_from_nc_file(a_trajInputFileName, wantedVars);
...
for idParam = 1:nParam
   ...
   if (~isempty(paramName))
      if (ismember(paramName, phParamList))
         toBeCheckFlag = 1;
         break
      end
   end
...
if (toBeCheckFlag)
   ...
   if (g_cosq_doItFlag == 1)
      ...
      [~, fileName, ~] = fileparts(a_trajInputFileName);
      ...
      [~, fileName, fileExtension] = fileparts(a_trajInputFileName);
      tmpProfOutputFileName = [tmpDirName '/' fileName fileExtension];
      copy_file(a_trajInputFileName, tmpProfOutputFileName);
      ...
      for idParam = 1:nParam
         ...
         if (~isempty(paramName))
            if (ismember(paramName, phParamList))
               ...
               idToSet = find((dataQc == g_decArgo_qcStrGood) | (dataQc == g_decArgo_qcStrProbablyGood));
               if (~isempty(idToSet))
                  % updated <PARAM>_QC
                  dataQc(idToSet) = g_decArgo_qcStrCorrectable;
      ...
      if (o_modifiedFileFlag == 1)
         ...
         % N_HISTORY dimension is not UNLIMITED, we should update this dimension
         ok = update_n_history_dim_in_traj_file(tmpProfOutputFileName, 1);

         if (ok == 0)
            fprintf('ERROR: Unable to update the N_HISTORY dimension of the NetCDF file: %s\n', tmpProfOutputFileName);
            return
         end
         ...
         histoInstitution = 'IF';
         histoSoftware = 'COSQ';
         histoSoftwareRelease = g_cosq_setPhQcInRModeVersion;
         histoParameter = 'PH_IN_SITU_TOTAL';
         ...
         netcdf.putVar(fCdf, netcdf.inqVarID(fCdf, 'HISTORY_PARAMETER'), ...
            fliplr([nHistory-1 0]), ...
            fliplr([1 length(histoParameter)]), histoParameter');

         netcdf.close(fCdf);

         o_trajOutputFileName = tmpProfOutputFileName;
      end
      ...
      [~, fileName, fileExtension] = fileparts(a_trajInputFileName);
      o_trajOutputFileName = [a_tmpDirName '/' fileName '/' fileName fileExtension];
   end
end
...
% ------------------------------------------------------------------------------
% Set PH_IN_SITU_TOTAL_QC = '3' in profile file.
%
% SYNTAX :
%  [o_modifiedFileFlag, o_profOutputFileName] = ...
%    set_ph_qc_to_profile_file(a_profInputFileName, a_tmpDirName)
%
% INPUT PARAMETERS :
...
if (~isempty(profIdList))
   ...
   if (g_cosq_doItFlag == 1)
            ...
            if (~isempty(paramName))
               if (ismember(paramName, phParamList))
                  ...
                  if (~isempty(idToSet))
                     ...
                     % if PARAMETER_DATA_MODE = 'R', updated PROFILE_<PARAM>_QC
                     if (parameterDataMode(idProf, idParam) == 'R')
                        newProfParamQc = compute_profile_quality_flag(dataQc(:, idProf));
                        netcdf.putVar(fCdf, netcdf.inqVarID(fCdf, ['PROFILE_' paramName '_QC']), idProf-1, newProfParamQc);
                     end
                  end
               end
            end
      ...
      [~, fileName, fileExtension] = fileparts(a_profInputFileName);
      o_profOutputFileName = [a_tmpDirName '/' fileName '/' fileName fileExtension];
   end
end
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis set PH_IN_SITU_TOTAL_QC in R mode file tool (nc_set_ph_qc_in_r_mode)'));
docRootNode.appendChild(newChild);

newChild = docNode.createElement('tool_version');
newChild.appendChild(docNode.createTextNode(g_cosq_setPhQcInRModeVersion));
docRootNode.appendChild(newChild);
...
if (isfield(g_cosq_reportData, 'trajFile'))
   for idFile = 1:length(g_cosq_reportData.trajFile)
      if (g_cosq_doItFlag == 1)
         newChildBis = docNode.createElement('updated_traj_file');
      else
         newChildBis = docNode.createElement('to_be_updated_traj_file');
      end
      textNode = g_cosq_reportData.trajFile{idFile};
      newChildBis.appendChild(docNode.createTextNode(textNode));
      newChild.appendChild(newChildBis);
   end
end
