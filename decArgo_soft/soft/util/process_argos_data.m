...
% - STEP4 (FINAL): move_and_rename_argos_files
%
% SYNTAX :
%   process_argos_data
%
...
% ------------------------------------------------------------------------------
function process_argos_data(varargin)

% directory of input Argos files (all the files in only one directory)
DIR_INPUT_ARGOS_FILES = 'C:\Users\jprannou\_DATA\OUT\tmp\';
...
DIR_OUTPUT = 'C:\Users\jprannou\_DATA\OUT\tmp_out\';
...
logFile = [DIR_LOG_FILE '/' 'process_argos_data' name '_' datestr(now, 'yyyymmddTHHMMSS') '.log'];
diary(logFile);
tic;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% STEP 4: identify cycle files
fprintf('\nSTEP 4: identify cycle files\n');
dirOutStep4 = [DIR_OUTPUT '/FINAL/'];
mkdir(dirOutStep4);
% create temporary directory
mkdir([dirOutStep4 '/IN/']);
% duplicate the input directory in the output one
copyfile(dirOutStep3, [dirOutStep4 '/IN/']);
[ok] = move_and_rename_argos_files_bis(floatList, [dirOutStep4 '/IN/'], dirOutStep4);
if (ok == 0)
   fprintf('ERROR: In step4 => exit\n');
   return;
end
rmdir([dirOutStep4 '/IN/'], 's');
