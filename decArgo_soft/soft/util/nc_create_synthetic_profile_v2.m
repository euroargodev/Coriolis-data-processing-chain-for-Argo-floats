...
REF_PROFILE_FILE = 'C:\Users\jprannou\_RNU\DecArgo_soft\soft\util\misc/ArgoSProf_V1.00.nc';
...
g_cocs_ncCreateSyntheticProfileVersion = '1.0';
...
g_cocs_reportData = [];
g_cocs_reportData.trajFile = [];
g_cocs_reportData.mProfFil = [];
g_cocs_reportData.profFile = [];
g_cocs_reportData.float = [];
...
% if ~(exist(DIR_XML_FILE, 'dir') == 7)
%    fprintf('ERROR: Xml directory not found: %s\n', DIR_XML_FILE);
%    errorFlag = 1;
% end
...
% xmlFileName = [DIR_XML_FILE '/nc_create_synthetic_profile_' currentTime '.xml'];
...
for idType= 1:2
   if (idType == 1)
      ...
      if ~(exist(trajFilePathName, 'file') == 2)
         fprintf('ERROR: Float #%d: File not found: %s\n', ...
            g_cocs_floatNum, trajFilePathName);
         return;
      end
   else
   ...
   if (~strcmp(formatVersion, '3.1'))
      fprintf('WARNING: Float #%d: Input TRAJ file (%s) format version is %s => not used\n', ...
         g_cocs_floatNum, trajFilePathName, formatVersion);
      return;
   end
...
% OLD CODE START
% % concatenate pumped and unpumped data
...
% idUnpumped = find([idUnpumped{:}] == 1);
% nbToFind = length(idUnpumped);
% for idProfUnpumped = idUnpumped
%    paramList = profDataTab(idProfUnpumped).paramList;
%    for idProf = 1:length(profDataTab)
%       if (~ismember(idProf, idUnpumped))
%          if (isempty(setdiff(paramList, profDataTab(idProf).paramList)) && ...
%                isempty(setdiff(profDataTab(idProf).paramList, paramList)))
%             profDataTab(idProf).paramData = [profDataTab(idProfUnpumped).paramData; profDataTab(idProf).paramData];
%             profDataTab(idProf).paramDataQc = [profDataTab(idProfUnpumped).paramDataQc; profDataTab(idProf).paramDataQc];
%             profDataTab(idProf).paramDataAdjusted = [profDataTab(idProfUnpumped).paramDataAdjusted; profDataTab(idProf).paramDataAdjusted];
%             profDataTab(idProf).paramDataAdjustedQc = [profDataTab(idProfUnpumped).paramDataAdjustedQc; profDataTab(idProf).paramDataAdjustedQc];
%             profDataTab(idProf).paramDataAdjustedError = [profDataTab(idProfUnpumped).paramDataAdjustedError; profDataTab(idProf).paramDataAdjustedError];
%             nbToFind = nbToFind - 1;
%             break;
%          end
%       end
%    end
% end
% profDataTab(idUnpumped) = [];
% if (nbToFind ~= 0)
%    fprintf('WARNING: Float #%d Cycle #%d%c: %d unpumped profiles are not used\n', ...
%       g_cocs_floatNum, g_cocs_cycleNum, g_cocs_cycleDir, nbToFind);
% end

% % concatenate PTS profiles together
% ptsOnlyId = [];
% ptsAndOtherId = [];
% for idProf = 1:length(profDataTab)
%    if (isempty(strfind(profDataTab(idProf).verticalSamplingScheme, 'Near-surface sampling')))
%       profParamList = profDataTab(idProf).paramList;
%       if (any(strcmp(profParamList, 'PRES')) && ...
%             any(strcmp(profParamList, 'TEMP')) && ...
%             any(strcmp(profParamList, 'PSAL')) && ...
%             (length(profParamList) == 3))
%          ptsOnlyId = [ptsOnlyId idProf];
%       elseif (any(strcmp(profParamList, 'PRES')) && ...
%             any(strcmp(profParamList, 'TEMP')) && ...
%             any(strcmp(profParamList, 'PSAL')) && ...
%             (length(profParamList) > 3))
%          ptsAndOtherId = [ptsAndOtherId idProf];
%       end
%    end
% end
% if (~isempty(ptsOnlyId) && ~isempty(ptsAndOtherId))
%
%    for idProf = 1:length(ptsAndOtherId)
%       profDataTab(ptsOnlyId(1)).paramData = [profDataTab(ptsOnlyId(1)).paramData; profDataTab(ptsAndOtherId(idProf)).paramData(:, 1:3)];
%       profDataTab(ptsOnlyId(1)).paramDataQc = [profDataTab(ptsOnlyId(1)).paramDataQc; profDataTab(ptsAndOtherId(idProf)).paramDataQc(:, 1:3)];
%       profDataTab(ptsOnlyId(1)).paramDataAdjusted = [profDataTab(ptsOnlyId(1)).paramDataAdjusted; profDataTab(ptsOnlyId(idProf)).paramDataAdjusted(:, 1:3)];
%       profDataTab(ptsOnlyId(1)).paramDataAdjustedQc = [profDataTab(ptsOnlyId(1)).paramDataAdjustedQc; profDataTab(ptsOnlyId(idProf)).paramDataAdjustedQc(:, 1:3)];
%       profDataTab(ptsOnlyId(1)).paramDataAdjustedError = [profDataTab(ptsOnlyId(1)).paramDataAdjustedError; profDataTab(ptsOnlyId(idProf)).paramDataAdjustedError(:, 1:3)];
%
%       idDel = 2:3;
%       profDataTab(ptsAndOtherId(idProf)).paramList(idDel) = [];
%       profDataTab(ptsAndOtherId(idProf)).paramDataMode(idDel) = [];
%       profDataTab(ptsAndOtherId(idProf)).paramData(:, idDel) = [];
%       profDataTab(ptsAndOtherId(idProf)).paramDataQc(:, idDel) = [];
%       profDataTab(ptsAndOtherId(idProf)).paramDataAdjusted(:, idDel) = [];
%       profDataTab(ptsAndOtherId(idProf)).paramDataAdjustedQc(:, idDel) = [];
%       profDataTab(ptsAndOtherId(idProf)).paramDataAdjustedError(:, idDel) = [];
%       profDataTab(ptsAndOtherId(idProf)).scientificCalibEquation(idDel) = [];
%       profDataTab(ptsAndOtherId(idProf)).scientificCalibCoefficient(idDel) = [];
%       profDataTab(ptsAndOtherId(idProf)).scientificCalibComment(idDel) = [];
%       profDataTab(ptsAndOtherId(idProf)).scientificCalibDate(idDel) = [];
%
%       fprintf('INFO: Float #%d Cycle #%d%c: PTS data of profile #%d concatenated to profile #%d\n', ...
%          g_cocs_floatNum, g_cocs_cycleNum, g_cocs_cycleDir, ...
%          profDataTab(ptsAndOtherId(idProf)).nProfId, profDataTab(ptsOnlyId(1)).nProfId);
%    end
% end
% if (length(ptsOnlyId) > 1)
%
%    for idProf = 2:length(ptsOnlyId)
%       profDataTab(ptsOnlyId(1)).paramData = [profDataTab(ptsOnlyId(1)).paramData; profDataTab(ptsOnlyId(idProf)).paramData];
%       profDataTab(ptsOnlyId(1)).paramDataQc = [profDataTab(ptsOnlyId(1)).paramDataQc; profDataTab(ptsOnlyId(idProf)).paramDataQc];
%       profDataTab(ptsOnlyId(1)).paramDataAdjusted = [profDataTab(ptsOnlyId(1)).paramDataAdjusted; profDataTab(ptsOnlyId(idProf)).paramDataAdjusted];
%       profDataTab(ptsOnlyId(1)).paramDataAdjustedQc = [profDataTab(ptsOnlyId(1)).paramDataAdjustedQc; profDataTab(ptsOnlyId(idProf)).paramDataAdjustedQc];
%       profDataTab(ptsOnlyId(1)).paramDataAdjustedError = [profDataTab(ptsOnlyId(1)).paramDataAdjustedError; profDataTab(ptsOnlyId(idProf)).paramDataAdjustedError];
%    end
%
%    profDataTab(ptsOnlyId(2:end)) = [];
%
%    fprintf('INFO: Float #%d Cycle #%d%c: %d PTS profiles concatenated\n', ...
%       g_cocs_floatNum, g_cocs_cycleNum, g_cocs_cycleDir, length(ptsOnlyId));
% end

% % remove CTD profile created from SUNA data for a PROVOR CTS5 float
% vssList = {profDataTab.verticalSamplingScheme};
% idDel = find(strcmp(vssList, 'Secondary sampling: discrete [CTD measurements concurrent with SUNA measurements, just slightly offset in time]'));
% profDataTab(idDel) = [];
%
% % remove TEMP and PSAL data from SUNA profile for a PROVOR CTS4 float
% idCor = [];
% for idProf = 1:length(profDataTab)
%    profParamList = profDataTab(idProf).paramList;
%    if (any(strcmp(profParamList, 'PRES')) && ...
%          any(strcmp(profParamList, 'TEMP')) && ...
%          any(strcmp(profParamList, 'PSAL')) && ...
%          any(strcmp(profParamList, 'NITRATE')))
%       idCor = [idCor idProf];
%    end
% end
% for idProf = 1:length(idCor)
%    profData = profDataTab(idCor(idProf));
%    idDel = [find(strcmp(profData.paramList, 'TEMP')) find(strcmp(profData.paramList, 'PSAL'))];
%
%    profData.paramList(idDel) = [];
%    profData.paramDataMode(idDel) = [];
%    profData.paramData(:, idDel) = [];
%    profData.paramDataQc(:, idDel) = [];
%    profData.paramDataAdjusted(:, idDel) = [];
%    profData.paramDataAdjustedQc(:, idDel) = [];
%    profData.paramDataAdjustedError(:, idDel) = [];
%    profData.scientificCalibEquation(idDel) = [];
%    profData.scientificCalibCoefficient(idDel) = [];
%    profData.scientificCalibComment(idDel) = [];
%    profData.scientificCalibDate(idDel) = [];
%
%    profDataTab(idCor(idProf)) = profData;
% end
% OLD CODE END
...
syntProfPres = get_synthetic_profile_pres_axis(a_profData, a_metaData);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% step #2: keep only levels with PRES_QC/PRES_ADJUSTED_QC ~= '4' or '9'
% then sort PRES levels in ascending order
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% keep only levels with PRES_QC/PRES_ADJUSTED_QC = '1'
...
if (paramDataMode(1) == 'R')
   ...
   idDel = find((paramDataAdjustedQc(:, 1) == g_decArgo_qcStrBad) | (paramDataAdjustedQc(:, 1) == g_decArgo_qcStrMissing));
end
samplingCode(idDel) = [];
paramData(idDel, :) = [];
paramDataQc(idDel, :) = [];
         ...
         if (~isempty(trajParamId))
            ...
            for idM = 1:length(idMeas)
               ...
               if (~isempty(idLev))
                  ...
                  %                      g_cocs_floatNum, g_cocs_cycleNum, g_cocs_cycleDir, idMeas(idM));
               end
            end
         end
      else
...
% round PRES data to the nearest cbar
paramPres = int64(paramPres*10);
paramPresDiff = int64(paramPresDiff(:, 4:end)*10);
paramPresDiffFillValue = intmax('int64');
...
presAxisFlagConfig = ones(size(paramData, 1), 1);
[~, idNoLev] = setdiff(paramData(:, 1), syntProfPres);
presAxisFlagConfig(idNoLev) = 0;

presAxisFlagAlgo = zeros(size(paramData, 1), 1);
presAxisFlagAlgo(idSPLev) = 1;

if (g_cocs_printCsv)
   print_profile_in_csv(paramList, paramDataMode, juldDataMode, ...
      samplingCode, presAxisFlagConfig, presAxisFlagAlgo, juld, juldQc, juldAdjusted, juldAdjustedQc, ...
      [paramData(:, 1:3) single(paramPresDiff)/10], paramDataQc, [], paramDataAdjusted, paramDataAdjustedQc, paramDataAdjustedError, ...
      'step8');
end
...
for idParam = 2:size(paramData, 2)
   ...
   % interpolate non adjusted data
   idToInterp = find(paramData(idSPLev, idParam) == paramFillValue(idParam));
   idToInterp = idSPLev(idToInterp);
   idBase = find(paramData(:, idParam) ~= paramFillValue(idParam));
   if (length(idBase) > 1)
      interpData = interp1(paramData(idBase, 1), paramData(idBase, idParam), paramData(idToInterp, 1), 'linear');
      idSet = find(~isnan(interpData));
      paramData(idToInterp(idSet), idParam) = interpData(idSet);
      paramDataQc(idToInterp(idSet), idParam) = g_decArgo_qcStrInterpolated;
   end
   
   % duplicate non adjusted data
   idToDuplicate = find(paramData(idSPLev, idParam) == paramFillValue(idParam));
   idToDuplicate = idSPLev(idToDuplicate);
   idBaseFirst = find(paramData(:, idParam) ~= paramFillValue(idParam), 1, 'first');
   idBaseLast = find(paramData(:, idParam) ~= paramFillValue(idParam), 1, 'last');
   if (~isempty(idToDuplicate) && ~isempty(idBaseFirst) && ~isempty(idBaseLast))
      idFirst = find(idToDuplicate < idBaseFirst);
      paramData(idToDuplicate(idFirst), idParam) = paramData(idBaseFirst, idParam);
      paramDataQc(idToDuplicate(idFirst), idParam) = g_decArgo_qcStrInterpolated;
      
      idLast = find(idToDuplicate > idBaseLast);
      paramData(idToDuplicate(idLast), idParam) = paramData(idBaseLast, idParam);
      paramDataQc(idToDuplicate(idLast), idParam) = g_decArgo_qcStrInterpolated;
   end

   % interpolate adjusted data
   idToInterp = find(paramDataAdjusted(idSPLev, idParam) == paramFillValue(idParam));
   idToInterp = idSPLev(idToInterp);
   idBase = find(paramDataAdjusted(:, idParam) ~= paramFillValue(idParam));
   if (length(idBase) > 1)
      interpData = interp1(refPresParamData(idBase, 1), paramDataAdjusted(idBase, idParam), refPresParamData(idToInterp, 1), 'linear');
      idSet = find(~isnan(interpData));
      paramDataAdjusted(idToInterp(idSet), idParam) = interpData(idSet);
      paramDataAdjustedQc(idToInterp(idSet), idParam) = g_decArgo_qcStrInterpolated;
      
      interpData = interp1(refPresParamData(idBase, 1), paramDataAdjustedError(idBase, idParam), refPresParamData(idToInterp, 1), 'linear');
      idSet = find(~isnan(interpData));
      paramDataAdjustedError(idToInterp(idSet), idParam) = interpData(idSet);
   end
   
   % duplicate adjusted data
   idToDuplicate = find(paramDataAdjusted(idSPLev, idParam) == paramFillValue(idParam));
   idToDuplicate = idSPLev(idToDuplicate);
   idBaseFirst = find(paramDataAdjusted(:, idParam) ~= paramFillValue(idParam), 1, 'first');
   idBaseLast = find(paramDataAdjusted(:, idParam) ~= paramFillValue(idParam), 1, 'last');
   if (~isempty(idToDuplicate) && ~isempty(idBaseFirst) && ~isempty(idBaseLast))
      idFirst = find(idToDuplicate < idBaseFirst);
      paramDataAdjusted(idToDuplicate(idFirst), idParam) = paramDataAdjusted(idBaseFirst, idParam);
      paramDataAdjustedQc(idToDuplicate(idFirst), idParam) = g_decArgo_qcStrInterpolated;
      paramDataAdjustedError(idToDuplicate(idFirst), idParam) = paramDataAdjustedError(idBaseFirst, idParam);
      
      idLast = find(idToDuplicate > idBaseLast);
      paramDataAdjusted(idToDuplicate(idLast), idParam) = paramDataAdjusted(idBaseLast, idParam);
      paramDataAdjustedQc(idToDuplicate(idLast), idParam) = g_decArgo_qcStrInterpolated;
      paramDataAdjustedError(idToDuplicate(idLast), idParam) = paramDataAdjustedError(idBaseLast, idParam);
   end
end
...
if (~isempty(juldDataMode))
   ...
   idToInterp = find(juld(idSPLev) == paramJuldFillValue);
   idToInterp = idSPLev(idToInterp);
   idBase = find(juld ~= paramJuldFillValue);
   if (length(idBase) > 1)
      interpData = interp1(paramData(idBase, 1), juld(idBase), paramData(idToInterp, 1), 'linear');
      idSet = find(~isnan(interpData));
      juld(idToInterp(idSet)) = interpData(idSet);
      juldQc(idToInterp(idSet)) = g_decArgo_qcStrInterpolated;
   end
   
   idToInterp = find(juldAdjusted(idSPLev) == paramJuldFillValue);
   idToInterp = idSPLev(idToInterp);
   idBase = find(juldAdjusted ~= paramJuldFillValue);
   if (length(idBase) > 1)
      interpData = interp1(refPresParamData(idBase, 1), juldAdjusted(idBase), refPresParamData(idToInterp, 1), 'linear');
      idSet = find(~isnan(interpData));
      juldAdjusted(idToInterp(idSet)) = interpData(idSet);
      juldAdjustedQc(idToInterp(idSet)) = g_decArgo_qcStrInterpolated;
   end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% step #5: for each measurements, keep only nearest meeasurement on the
%          synthetic profile PRES axis 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

paramDataDPres = ones(size(paramData))*paramFillValue(1);
for idParam = 2:size(paramData, 2)
   
   paramDataNew = ones(size(paramData(:, idParam)))*paramFillValue(idParam);
   paramDataNewQc = repmat(g_decArgo_qcStrMissing, size(paramDataQc(:, idParam)));

   % duplicate kept measurements and compte associated <PARAM>_dPRES
   idVal = find((paramData(:, idParam) ~= paramFillValue(idParam)) & ...
      (paramDataQc(:, idParam) ~= g_decArgo_qcStrInterpolated));
   for idLev = idVal'
      ...
      if (~isempty(idUp) && ~isempty(idDown))
         if (idUp == idLev)
            ...
            paramDataNew(idLev) = paramData(idLev, idParam);
            paramDataNewQc(idLev) = paramDataQc(idLev, idParam);
            paramDataDPres(idLev, idParam) = 0;
         else
            % measurement not on the synthetic profile PRES axis
            if ((paramData(idDown, 1)-paramData(idLev, 1)) <= (paramData(idLev, 1)-paramData(idUp, 1)))
               % the 'down' value should be kept
               paramDataNew(idDown) = paramData(idDown, idParam);
               paramDataNewQc(idDown) = paramDataQc(idDown, idParam);
               paramDataDPres(idDown, idParam) = paramData(idLev, 1) - paramData(idDown, 1);
            else
               % the 'up' value should be kept
               paramDataNew(idUp) = paramData(idUp, idParam);
               paramDataNewQc(idUp) = paramDataQc(idUp, idParam);
               paramDataDPres(idUp, idParam) = paramData(idLev, 1) - paramData(idUp, 1);
            end
         end
      elseif (~isempty(idDown))
         if (idDown == idLev)
            ...
            paramDataNew(idDown) = paramData(idDown, idParam);
            paramDataNewQc(idDown) = paramDataQc(idDown, idParam);
            paramDataDPres(idUp, idParam) = paramData(idLev, 1) - paramData(idDown, 1);
         end
      elseif (~isempty(idUp))
         if (idUp == idLev)
            % measurement on the synthetic profile PRES axis
   ...
   idVal = find((paramData(:, idParam) ~= paramFillValue(idParam)) & ...
      (paramDataQc(:, idParam) ~= g_decArgo_qcStrInterpolated));
   idInterpol = find(paramDataNewQc == g_decArgo_qcStrInterpolated);
   for idLev = idInterpol'
      ...
      idDown = idVal(idDown);
      if (~isempty(idUp) && ~isempty(idDown))
         paramDataNewQc(idLev) = max(paramDataQc(idUp, idParam), paramDataQc(idDown, idParam));
      elseif (~isempty(idDown))
         paramDataNewQc(idLev) = paramDataQc(idDown, idParam);
      elseif (~isempty(idUp))
         paramDataNewQc(idLev) = paramDataQc(idUp, idParam);
      end
   end
   
   paramData(:, idParam) = paramDataNew;
   paramDataQc(:, idParam) = paramDataNewQc;
   
   clear paramDataNew;
   clear paramDataNewQc;
end
...
%                g_cocs_floatNum, g_cocs_cycleNum, g_cocs_cycleDir, length(presCutOffProf));
...
%   01/11/2018 - RNU - creation
% ------------------------------------------------------------------------------
function [o_syntProfPres] = get_synthetic_profile_pres_axis_bis(a_profData, a_metaData)
...
if (isempty(a_metaData))
   fprintf('ERROR: Float #%d Cycle #%d%c: no meta-data cannot retrieve sensor measurement zones\n', ...
      g_cocs_floatNum, g_cocs_cycleNum, g_cocs_cycleDir);
   return;
end

% retrieve information on each sensor measurement zones
sensorProf = [];
for idProf = 1:length(a_profData)
   profParamList = a_profData(idProf).paramList;
   if (ismember('PRES', profParamList) && ismember('TEMP', profParamList) && ismember('PSAL', profParamList))
      sensorProf = [sensorProf; idProf {'Ctd'}];
   elseif (ismember('DOXY', profParamList))
      sensorProf = [sensorProf; idProf {'Optode'}];
   elseif (any(cellfun(@(x) strncmp(x, 'DOWN_IRRADIANCE', length('DOWN_IRRADIANCE')), profParamList)))
      sensorProf = [sensorProf; idProf {'Ocr'}];
   elseif (ismember('CHLA', profParamList) && ismember('CDOM', profParamList))
      sensorProf = [sensorProf; idProf {'Eco'}];
   elseif (ismember('CHLA', profParamList) && ...
         (any(cellfun(@(x) strncmp(x, 'BBP', length('BBP')), profParamList))))
      sensorProf = [sensorProf; idProf {'Flbb'}];
   elseif (ismember('CHLA', profParamList) && ismember('TURBIDITY', profParamList))
      sensorProf = [sensorProf; idProf {'Flntu'}];
   elseif (ismember('CP660', profParamList))
      sensorProf = [sensorProf; idProf {'Crover'}];
   elseif (ismember('NITRATE', profParamList))
      sensorProf = [sensorProf; idProf {'Suna'}];
   else
      paramListStr = sprintf('''%s'' ', profParamList{:});
      fprintf('ERROR: Float #%d Cycle #%d%c: cannot determine <short_sensor_name> from param list (%s)\n', ...
         g_cocs_floatNum, g_cocs_cycleNum, g_cocs_cycleDir,paramListStr(1:end-1));
   end
end

launchConfigParameterNames = cellstr(a_metaData.launchConfigParameterName);
configParameterNames = cellstr(a_metaData.configParameterName);
shortSensorNameList = unique(sensorProf(:, 2));
sensorZones = [];
for idSSN = 1:length(shortSensorNameList)
   zones = [];
   shortSensorName = shortSensorNameList{idSSN};
   
   % from launch configuration
   idZones1 = cellfun(@(x) strfind(x, ['CONFIG_' shortSensorName 'DepthZone']) & strfind(x, 'PressureThreshold_dbar'), launchConfigParameterNames, 'UniformOutput', 0);
   idZones2 = find(cellfun(@(x) ~isempty(x), idZones1));
   if (isempty(idZones2))
      idZones1 = cellfun(@(x) strfind(x, ['CONFIG_' upper(shortSensorName) 'DepthZone']) & strfind(x, 'PressureThreshold_dbar'), launchConfigParameterNames, 'UniformOutput', 0);
      idZones2 = find(cellfun(@(x) ~isempty(x), idZones1));
   end
   for idZ = 1:length(idZones2)
      configParamName = launchConfigParameterNames{idZones2(idZ)};
      idF1 = strfind(configParamName, 'DepthZone');
      idF2 = strfind(configParamName, 'PressureThreshold_dbar');
      z1 = str2num(configParamName(idF1(1)+length('DepthZone'):idF1(2)-1));
      z2 = str2num(configParamName(idF1(2)+length('DepthZone'):idF2(1)-1));
      zones = [zones; z1 z2 a_metaData.launchConfigParameterValue(idZones2(idZ))];
   end
   
   % from cycle configuration
   idZones1 = cellfun(@(x) strfind(x, ['CONFIG_' shortSensorName 'DepthZone']) & strfind(x, 'PressureThreshold_dbar'), configParameterNames, 'UniformOutput', 0);
   idZones2 = find(cellfun(@(x) ~isempty(x), idZones1));
   if (isempty(idZones2))
      idZones1 = cellfun(@(x) strfind(x, ['CONFIG_' upper(shortSensorName) 'DepthZone']) & strfind(x, 'PressureThreshold_dbar'), configParameterNames, 'UniformOutput', 0);
      idZones2 = find(cellfun(@(x) ~isempty(x), idZones1));
   end
   if (~isempty(idZones2))
      idMission = find(a_metaData.configMissionNumber == unique([a_profData.configMissionNumber]));
      for idZ = 1:length(idZones2)
         configParamName = configParameterNames{idZones2(idZ)};
         idF1 = strfind(configParamName, 'DepthZone');
         idF2 = strfind(configParamName, 'PressureThreshold_dbar');
         z1 = str2num(configParamName(idF1(1)+length('DepthZone'):idF1(2)-1));
         z2 = str2num(configParamName(idF1(2)+length('DepthZone'):idF2(1)-1));
         idF = find((zones(:, 1) == z1) & (zones(:, 2) == z2));
         zones(idF, 3) = a_metaData.configParameterValue(idMission, idZones2(idZ));
      end
   end
   
   % store information
   [~, idSort] = sort(zones(:, 1));
   sensorZones = [sensorZones; {shortSensorName} {zones(idSort, :)}];
end

% retrieve PRES measurements from each sensor
sensorZonePres = [sensorZones cell(size(sensorZones, 1), 2)];
for idProf = 1:length(a_profData)
   profData = a_profData(idProf);
   profDataPres = profData.paramData(find(profData.paramDataQc(:, 1) ~= g_decArgo_qcStrBad), 1);
   idFSensor = find(strcmp(sensorProf{idProf, 2}, sensorZonePres(:, 1)));
   if (isempty(sensorZonePres{idFSensor, 3}))
      sensorZonePres(idFSensor, 3) = {profDataPres};
      sensorZonePres(idFSensor, 4) = {idProf};
   else
      sensorZonePres(idFSensor, 3) = {[profDataPres; sensorZonePres{idFSensor, 3}]};
   end
end

% ignore CTD sensor data
idBgc = find(~strcmp('Ctd', sensorZonePres(:, 1)));
sensorZonePres = sensorZonePres(idBgc, :);

% collect synthetic profile PRES measurements
...
% Zone 4 : Z3 < P <= Z4
% Zone 5 : Z4 < P
zones = cat(1, sensorZonePres{:, 2});
zones = unique(zones(:, 3));
syntProfPres = [];
for idZ = 1:length(zones)
   presRef = [];
   presMaxRef = [];
   if (idZ == 1)
      for idS = 1:size(sensorZonePres, 1)
         pres = sensorZonePres{idS, 3};
         pres = pres(find(pres <= zones(idZ)));
         if (~isempty(pres))
            if (length(pres) > length(presRef))
               presRef = pres;
               presMaxRef = max(pres);
            elseif (length(pres) == length(presRef))
               if (presMaxRef < max(pres))
                  presRef = pres;
                  presMaxRef = max(pres);
               end
            end
      ...
      syntProfPres = [syntProfPres; presRef];
   else
      for idS = 1:size(sensorZonePres, 1)
         pres = sensorZonePres{idS, 3};
         pres = pres(find((zones(idZ-1) < pres) & (pres <= zones(idZ))));
         if (~isempty(pres))
            if (length(pres) > length(presRef))
               presRef = pres;
               presMaxRef = max(pres);
...
%   01/11/2018 - RNU - creation
% ------------------------------------------------------------------------------
function [o_syntProfPres] = get_synthetic_profile_pres_axis(a_profData, a_metaData)
...
for idZ = 1:length(zones)
   ...
   if (idZ == 1)
      for idS = 1:size(sensorZonePres, 1)
         ...
         if (length(pres) > length(presRef))
            presRef = pres;
            presNProfRef = sensorZonePres{idS, 4};
         elseif (length(pres) == length(presRef))
            if (sensorZonePres{idS, 4} < presNProfRef)
               presRef = pres;
               presNProfRef = sensorZonePres{idS, 4};
            end
         end
...
% decoder version
global g_cocs_ncDuplicatePresAxisVersion;
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis duplicate PRES axis tool (nc_create_synthetic_profile)'));
docRootNode.appendChild(newChild);

newChild = docNode.createElement('tool_version');
newChild.appendChild(docNode.createTextNode(g_cocs_ncDuplicatePresAxisVersion));
docRootNode.appendChild(newChild);
...
if (isfield(g_cocs_reportData, 'float'))
   wmoList = sort(unique(g_cocs_reportData.float));
...
if (isfield(g_cocs_reportData, 'profFile'))
   for idFile = 1:length(g_cocs_reportData.profFile)
      newChildBis = docNode.createElement('updated_mono_profile_file');
      textNode = g_cocs_reportData.profFile{idFile};
      newChildBis.appendChild(docNode.createTextNode(textNode));
      newChild.appendChild(newChildBis);
   end
end
if (isfield(g_cocs_reportData, 'mProfFil'))
   for idFile = 1:length(g_cocs_reportData.mProfFil)
      newChildBis = docNode.createElement('updated_multi_profile_file');
      textNode = g_cocs_reportData.mProfFil{idFile};
      newChildBis.appendChild(docNode.createTextNode(textNode));
      newChild.appendChild(newChildBis);
   end
end
