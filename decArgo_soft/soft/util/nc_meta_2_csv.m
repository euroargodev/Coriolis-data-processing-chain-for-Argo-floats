...
FLOAT_LIST_FILE_NAME = 'C:\Users\jprannou\_RNU\Argo\ActionsCoriolis\GenerateMeta3.1WhenNoData_20210920\list\float list.txt';
...
idVal = find(strcmp('FORMAT_VERSION', metaData));
formatVersion = strtrim(metaData{idVal+1}');

if (strcmp(formatVersion, '1.0'))

   nc_meta_aux_1_0_to_csv_file(a_inputPathFileName, a_outputPathFileName, ...
      a_floatNum, a_comparisonFlag)

elseif (strcmp(formatVersion, '2.0'))

   nc_meta_aux_2_0_to_csv_file(a_inputPathFileName, a_outputPathFileName, ...
      a_floatNum, a_comparisonFlag)

else
   fprintf('File format version is %g (1.0 or 2.0 expected) => exit\n', ...
      formatVersion);
   return
end
...
%   02/21/2017 - RNU - creation
% ------------------------------------------------------------------------------
function nc_meta_aux_1_0_to_csv_file(a_inputPathFileName, a_outputPathFileName, ...
   a_floatNum, a_comparisonFlag)
...
% ------------------------------------------------------------------------------
function nc_meta_aux_2_0_to_csv_file(a_inputPathFileName, a_outputPathFileName, ...
   a_floatNum, a_comparisonFlag)
...
for idConfParam = 1:length(launchConfigParamValue)
   fprintf(fidOut, ' %d; %s; %s; %s\n', a_floatNum, ...
      strtrim(launchConfigParamName(idConfParam, :)), ...
      strtrim(launchConfigParamValue(idConfParam, :)), ...
      strtrim(launchConfigParamDescription(idConfParam, :)));
end
...
[~, nConf, nMis] = size(configParamValue);
for idConfParam = 1:nConf
   fprintf(fidOut, ' %d; %s', a_floatNum, strtrim(configParamName(idConfParam, :)));
   for idMis = 1:nMis
      fprintf(fidOut, '; %s', strtrim(configParamValue(:, idConfParam, idMis)'));
   end
   fprintf(fidOut, '\n');
end
