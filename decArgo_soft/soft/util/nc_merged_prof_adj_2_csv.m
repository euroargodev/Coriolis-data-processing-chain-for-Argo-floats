...
logFile = [DIR_LOG_FILE '/' 'nc_merged_prof_adj_2_csv' name '_' datestr(now, 'yyyymmddTHHMMSS') '.log'];
diary(logFile);
tic;
...
for idFloat = 1:nbFloats
   ...
   if (exist(ncFileDirRef, 'dir') == 7)
      ...
      if (exist(profFilePathName, 'file') == 2)
         ...
         nc_merged_prof_adj_2_csv_file(profFilePathName, outputFilePathName, floatNum, COMPARISON_MODE);
      end
      ...
      if (exist(ncFileDir, 'dir') == 7)
         ...
         for idFile = 1:length(ncFiles)
            ...
            nc_merged_prof_adj_2_csv_file(ncFilePathName, outputFilePathName, floatNum, COMPARISON_MODE);
         end
      else
      ...
      if (exist(ncAuxFileDir, 'dir') == 7)
         ...
         ncFiles = dir([ncAuxFileDir 'M*_aux.nc']);
         for idFile = 1:length(ncFiles)
            ...
            cFileFlag = -1;
            nc_merged_prof_adj_2_csv_file(ncFilePathName, outputFilePathName, floatNum, COMPARISON_MODE, WRITE_QC_FLAG);
         end
      end
   else
      fprintf('WARNING: Directory not found: %s\n', ncFileDirRef);
