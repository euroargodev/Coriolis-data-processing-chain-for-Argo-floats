...
%
% SYNTAX :
%   nc_create_dm_mono_profile_apx_in_V3_1 or nc_create_dm_mono_profile_apx_in_V3_13(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function nc_create_dm_mono_profile_apx_in_V3_1(varargin)
...
DIR_OUTPUT_NC_FILES = 'C:\Users\jprannou\_DATA\convert_DM_apex_in_3.1\DM_profile_file_updated\';
DIR_OUTPUT_NC_FILES = 'C:\Users\jprannou\_RNU\DecArgo_soft\work\tmp\';
...
% FLOAT_LIST_FILE_NAME = 'C:\Users\jprannou\_RNU\DecArgo_soft\lists\_apex_with_DM_profile_021009.txt';
% FLOAT_LIST_FILE_NAME = 'C:\Users\jprannou\_RNU\DecArgo_soft\lists\_apex_with_DM_profile_061810.txt';
...
% FLOAT_LIST_FILE_NAME = 'C:\Users\jprannou\_RNU\DecArgo_soft\lists\_apex_with_DM_profile_tmp.txt';
...
logFile = [DIR_LOG_FILE '/' 'nc_create_dm_mono_profile_apx_in_V3_1' name '_' datestr(now, 'yyyymmddTHHMMSS') '.log'];
diary(logFile);
tic;
...
%    fprintf('%03d/%03d %d\n', idFloat, nbFloats, floatNum);
   ...
   for idFile = 1:length(idForWmo)
      ...
      dmInFilePathName = [dmInFloatPath sprintf('D%d_%03d.nc', floatNum, dmCyNumList(idForWmo(idFile)))];
      rtInCFilePathName = [rtInFloatPath sprintf('R%d_%03d%s.nc', floatNum, rtCyNumList(idForWmo(idFile)))];
      rtInBFilePathName = [rtInFloatPath sprintf('BR%d_%03d%s.nc', floatNum, rtCyNumList(idForWmo(idFile)))];
      dmOutCFilePathName = [dmOutFloatPath sprintf('D%d_%03d.nc', floatNum, rtCyNumList(idForWmo(idFile)))];
      dmOutBFilePathName = [dmOutFloatPath sprintf('BD%d_%03d.nc', floatNum, rtCyNumList(idForWmo(idFile)))];
      
      ok = 0;
      if (exist(dmInFilePathName, 'file') == 2)
         
         % update the file
         [ok] = create_dm_file(dmInFilePathName, ...
            rtInCFilePathName, rtInBFilePathName, ...
            dmOutCFilePathName, dmOutBFilePathName, ...
            profDirList(idForWmo(idFile)), DIR_JSON_FLOAT_INFO, DIR_JSON_FLOAT_META);
      end
         
      if (ok == 0)
         % copy the RT V3.1 file to the output directory
      end
...
%
% SYNTAX :
%  [o_ok] = create_dm_file(a_inputDmFileName, ...
%    a_inputRtCFileName, a_inputRtBFileName, ...
%    a_outputCFileName, a_outputBFileName, ...
%    a_direction, a_jsonFloatInfoDirName, a_jsonFloatMetaDirName)
%
% INPUT PARAMETERS :
...
%   07/29/2014 - RNU - creation
% ------------------------------------------------------------------------------
function [o_ok] = create_dm_file(a_inputDmFileName, ...
   a_inputRtCFileName, a_inputRtBFileName, ...
   a_outputCFileName, a_outputBFileName, ...
   a_direction, a_jsonFloatInfoDirName, a_jsonFloatMetaDirName)
...
[inputDmInfo] = get_data_from_nc_file(a_inputDmFileName, wantedInputVars);

idVal = find(strcmp('FORMAT_VERSION', inputDmInfo(1:2:end)) == 1, 1);
inputDmFileFormatVersionStr = strtrim(inputDmInfo{2*idVal}');
idVal = find(strcmp('PI_NAME', inputDmInfo(1:2:end)) == 1, 1);
piName = strtrim(inputDmInfo{2*idVal}');

% fprintf(' (format version %s)\n', inputFileFormatVersionStr);

% check the format version of the input file
if ((strcmp(inputDmFileFormatVersionStr, '3.0') == 0) && ...
      (strcmp(inputDmFileFormatVersionStr, '2.3') == 0) && ...
      (strcmp(inputDmFileFormatVersionStr, '2.2') == 0))
   ...
   fprintf('ERROR: Input DM file (%s) is expected to be of 2.2 or 2.3 or 3.0 format version (but FORMAT_VERSION = %s)', ...
      [fileName fileExt], inputDmFileFormatVersionStr);
   return;
end
inputDmFileFormatVersion = str2num(inputDmFileFormatVersionStr);

% create the list of parameters
idVal = find(strcmp('STATION_PARAMETERS', inputDmInfo(1:2:end)) == 1, 1);
stationParameters = inputDmInfo{2*idVal};
...
if (inputDmFileFormatVersion == 2.2)
   wantedInputVars = [ ...
      wantedInputVars ...
      {'CALIBRATION_DATE'} ...
      ];
elseif (inputDmFileFormatVersion == 2.3)
   wantedInputVars = [ ...
      wantedInputVars ...
      {'FIRMWARE_VERSION'} ... % missing in 2.2 for Coriolis floats
      {'SCIENTIFIC_CALIB_DATE'} ... % for Coriolis floats
      ];
elseif (inputDmFileFormatVersion == 3.0)
   wantedInputVars = [ ...
      wantedInputVars ...
      {'FIRMWARE_VERSION'} ... % missing in 2.2 for Coriolis floats
...
idVal = find(strcmp('PARAMETER', inputDmInfo(1:2:end)) == 1, 1);
parameter = inputDmInfo{2*idVal};
idVal = find(strcmp('SCIENTIFIC_CALIB_EQUATION', inputDmInfo(1:2:end)) == 1, 1);
scientificCalibEquation = inputDmInfo{2*idVal};
...
[inputDmData] = get_var_from_nc_file_bis(a_inputDmFileName, wantedInputVars);
...
for idParam = 1:length(inputDmParamList)
   ...
   idVal = find(strcmp(paramName, inputDmData) == 1, 1);
   param = inputDmData{idVal+1};
   paramFillValue = inputDmData{idVal+2};
   
   idVal = find(strcmp(paramNameAdj, inputDmData) == 1, 1);
   paramAdj = inputDmData{idVal+1};
   paramAdjFillValue = inputDmData{idVal+2};
   
   idVal = find(strcmp(paramNameAdjErr, inputDmData) == 1, 1);
   paramAdjErr = inputDmData{idVal+1};
   paramAdjErrFillValue = inputDmData{idVal+2};
   
   idVal = find(strcmp('DATA_MODE', inputDmInfo(1:2:end)) == 1, 1);
   dataMode = inputDmInfo{2*idVal}';
   dmProfNumList = find(dataMode == 'D');
   if (length(dmProfNumList) > 1)
      fprintf('WARNING: %d profiles in DM in file %s\n', ...
         length(dmProfNumList), a_inputDmFileName);
   end
      
   for idProf = 1:length(dmProfNumList)
      profId = dmProfNumList(idProf);
      profParam = param(:, profId);
      profParamAdj = paramAdj(:, profId);
      profParamAdjErr = paramAdjErr(:, profId);
      
      idOk = find((profParam ~= paramFillValue) & ...
         (profParamAdj ~= paramAdjFillValue));
      ...
      if (~isempty(idOk))
         ...
         for idParam = 1:size(profParameter, 2)
            if (strcmp(strtrim(profParameter(:, idParam)'), paramName))
               paramId = idParam;
               break;
            end
         end
         if (strcmp(paramName, 'PSAL'))
            ...
            fprintf('%s;%s;%s\n', piName, fileName, strtrim(scientificCalibEquation(:, paramId, 1, profId)'));
         end
         
      end
   end
      
   %    if (isempty(paramAdjInfo))
   %       fprintf('WARNING: no consistent adjustment information available in file %s\n', ...
   %          a_inputDmFileName);
   %    end
end
...
inputDmParamList = sort(unique(inputDmParamList));
nParamDimInputDm = length(inputDmParamList);
...
nProfDimOutput = 1;
nLevDimOutput = size(inputFilePres, 1);
nLevelsPrimary = nLevDimOutput;
...
[~, ~, nHistoryDimInputDm] = size(inputHistoInstitution);
...
[~, ~, nCalibDimInputDm, ~] = size(inputParameter);

% for NST floats, the NST profile is copied from the RT file to the DM one
...
% retrieve N_HISTORY dimension from input RT file
idVal = find(strcmp('HISTORY_INSTITUTION', inputRtCData(1:2:end)) == 1, 1);
inputHistoInstitution = inputRtCData{2*idVal};
[~, ~, nHistoryDimInputRt] = size(inputHistoInstitution);

% retrieve N_CALIB dimension from input RT file
idVal = find(strcmp('PARAMETER', inputRtCData(1:2:end)) == 1, 1);
inputParameter = inputRtCData{2*idVal};
[~, ~, nCalibDimInputRt, ~] = size(inputParameter);

idVal = find(strcmp('VERTICAL_SAMPLING_SCHEME', inputRtCData(1:2:end)) == 1, 1);
vssRt = cellstr(strtrim(inputRtCData{2*idVal}'));
idF = find(strncmp(vssRt, 'Near-surface sampling:', length('Near-surface sampling:')) == 1, 1);
if (~isempty(idF))
   idVal = find(strcmp('FIRMWARE_VERSION', inputRtCData(1:2:end)) == 1, 1);
   firmwareVersionRt = strtrim(inputRtCData{2*idVal}');
   firmwareVersionRt = firmwareVersionRt(idF, :);
...
nHistoryDimOutput = max(nHistoryDimInputDm, nHistoryDimInputRt);
nCalibDimOutput = max(nCalibDimInputDm, nCalibDimInputRt);
...
[outputCFileSchema] = update_dim_in_nc_schema(outputCFileSchema, ...
   'N_PROF', nProfDimOutput+nstProfRt);
[outputCFileSchema] = update_dim_in_nc_schema(outputCFileSchema, ...
   'N_LEVELS', max(nLevDimOutput, nstLevRt));
[outputCFileSchema] = update_dim_in_nc_schema(outputCFileSchema, ...
...
% the RT NST profile will be duplicated from DM output file
if (nstProfRt == 1)
   wantedInputVars = [wantedInputVars ...
      {'DATA_CENTRE'} ...
      {'DATA_STATE_INDICATOR'} ...
      {'DATA_MODE'} ...
      {'SCIENTIFIC_CALIB_EQUATION'} ...
   ...
   for idParam = 1:length(inputRtParamList)
      paramName = inputRtParamList{idParam};
      profParamQcName = ['PROFILE_' paramName '_QC'];
      paramNameQc = [paramName '_QC'];
      paramNameAdj = [paramName '_ADJUSTED'];
      paramNameAdjQc = [paramName '_ADJUSTED_QC'];
