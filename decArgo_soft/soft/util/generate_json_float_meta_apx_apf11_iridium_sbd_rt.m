...
%
% SYNTAX :
%  generate_json_float_meta_apx_apf11_iridium_sbd_rt()
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function generate_json_float_meta_apx_apf11_iridium_sbd_rt(varargin)
...
try
   ...
   if (~isempty(g_cogj_outputLogDirName))
      logFileName = [g_cogj_outputLogDirName '/generate_json_float_meta_apx_apf11_iridium_sbd_rt_' currentTime '.log'];
   else
      logFileName = [tempdir '/generate_json_float_meta_apx_apf11_iridium_sbd_rt_' currentTime '.log'];
   end
...
if (~isempty(g_cogj_xmlReportDirName))
   xmlFileName = [g_cogj_xmlReportDirName '/generate_json_float_meta_apx_apf11_iridium_sbd_rt_' currentTime '.xml'];
else
   xmlFileName = [tempdir '/generate_json_float_meta_apx_apf11_iridium_sbd_rt_' currentTime '.xml'];
end
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis float meta-data json file generating tool (generate_json_float_meta_apx_apf11_iridium_sbd_rt)'));
docRootNode.appendChild(newChild);
   ...
   if (rem(length(a_varargin), 2) ~= 0)
      ...
      for id = 1:2:length(a_varargin)
         ...
         elseif (strcmpi(a_varargin{id}, 'outputCsvDirName'))
            g_cogj_outputCsvDirName = a_varargin{id+1};
...
if (isempty(g_cogj_outputCsvDirName))
   o_logLines{end+1} = sprintf('ERROR: ''outputCsvDirName'' input parameter is mandatory\n');
   o_inputError = 1;
   return
end
if (isempty(g_cogj_outputLogDirName))
