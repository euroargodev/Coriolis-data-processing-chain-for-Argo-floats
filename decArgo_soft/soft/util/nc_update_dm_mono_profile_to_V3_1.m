...
%
% SYNTAX :
%   nc_update_dm_mono_profile_to_V3_1 or nc_update_dm_mono_profile_to_V3_13(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function nc_update_dm_mono_profile_to_V3_1(varargin)
...
logFile = [DIR_LOG_FILE '/' 'nc_update_dm_mono_profile_to_V3_1' name '_' datestr(now, 'yyyymmddTHHMMSS') '.log'];
diary(logFile);
tic;
         ...
         if (var_is_present(fCdf, varNameOut))
            ...
            if (strcmp(paramNamePrefix, 'TEMP') && (idS == 2))
               ...
               if (~isempty(idFQc0))
                  ...
                  if (~isempty(varAdjQcValue))
                     if (any((varAdjQcValue ~= ' ') & (varAdjQcValue ~= '0')))
            ...
            if (idS == 2)
               ...
               if (~isempty(varParamValue))
                  ...
                  if (any(~((varValue(idFParamFillval) == ' ') | ...
                        (varValue(idFParamFillval) == '9'))))
                     idFQcToSet = find(~((varValue(idFParamFillval) == ' ') | ...
                        (varValue(idFParamFillval) == '9')));
                     fprintf('INFO: %d %s values set to ''9'' (because %s = FillValue) (file %s)\n', ...
                        length(idFQcToSet), varNameIn, varParamName, outputFileName);
                     varValue(idFParamFillval(idFQcToSet)) = '9';
                     inputDmData{2*idVal} = varValue;
                  end
               end
            end
            ...
            if (idS == 4)
               ...
               if (~isempty(varAdjValue))
                  ...
                  if (any(~((varValue(idFAdjFillval) == ' ') | ...
                        (varValue(idFAdjFillval) == '9') | ...
                        (varValue(idFAdjFillval) == '4'))))
                     idFQcToSet = find(~((varValue(idFParamFillval) == ' ') | ...
                        (varValue(idFParamFillval) == '9') | ...
                        (varValue(idFParamFillval) == '4')));
                     fprintf('INFO: %d %s values set to ''9'' (because %s = FillValue) (file %s)\n', ...
                        length(idFQcToSet), varNameIn, varParamName, outputFileName);
                     varValue(idFAdjFillval(idFQcToSet)) = '9';
                     inputDmData{2*idVal} = varValue;
                  end
               end
            end
            ...
            if (nLevelsPrimary > 0)
               ...
               if (idS == 4)
                  ...
                  if (idFile == 2)
                     ...
                     if (length(find((qcData == ' ') | (qcData == '0') | (qcData == '9'))) ~= length(qcData))
                        % QC performed
                        dataMode = 'D';
                     else
                        % no QC performed
      ...
      if (~isempty(find(strcmp('MOLAR_DOXY', inputDmMissingParamList) ==1, 1)))
         ...
         for idProf = 1:size(presOutputCMeas, 2)
            ...
            if (ok == 1)
               ...
               molarDoxyQcStr = repmat(' ', size(molarDoxy, 1), 1);
               molarDoxyQcStr(find(molarDoxy ~= molarDoxyParam.fillValue)) = '0';
      ...
      if (~isempty(find(strcmp('DOXY', inputDmMissingParamList) ==1, 1)))
         ...
         for idProf = 1:size(presOutputCMeas, 2)
            ...
            if (~isempty(idDef))
               ...
               doxyQcStr = repmat(' ', size(doxy, 1), 1);
               doxyQcStr(find(doxy ~= doxyParam.fillValue)) = '0';
