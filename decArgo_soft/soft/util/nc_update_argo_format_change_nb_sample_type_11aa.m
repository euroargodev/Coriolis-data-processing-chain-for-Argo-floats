...
%
% SYNTAX :
%   nc_update_argo_format_change_nb_sample_type_11aa or 
%   nc_update_argo_format_change_nb_sample_type_11aa(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   03/17/2022 - RNU - V 11aa: change NB_SAMPLE_CTD and NB_SAMPLE_SFET variables
%                              type from NC_FLOAT (single) to NC_SHORT (int16)
% ------------------------------------------------------------------------------
function nc_update_argo_format_change_nb_sample_type_11aa(varargin)
...
DIR_INPUT_OUTPUT_NC_FILES = 'C:\Users\jprannou\_DATA\OUT\test_change_nb_sample_type\';
...
g_couf_ncUpdateArgoFormatVersion = '11aa';
...
try
   ...
   tmpDir = [DIR_TMP '/' 'nc_update_argo_format_change_nb_sample_type_11aa_' currentTime];
   status = mkdir(tmpDir);
   if (status ~= 1)
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_update_argo_format_change_nb_sample_type_11aa_' currentTime '.log'];
   diary(logFile);
                  ...
                  if ((isempty(floatList)) || (~isempty(floatList) && ismember(floatWmo, floatList)))
                     ...
                     for idFile = 1:length(floatFiles)
                        ...
                        if (exist(floatFilePathName, 'file') == 2)
                           process_nc_file(floatFilePathName, tmpDir, DO_IT, 2);
                        end
                     end
                     ...
                     if (exist(floatDirPathName, 'dir') == 7)
                        ...
                        for idFile = 1:length(floatFiles)
                           ...
                           floatFilePathName = [floatDirPathName '/' floatFileName];
                           if (exist(floatFilePathName, 'file') == 2)
                              process_nc_file(floatFilePathName, tmpDir, DO_IT, 1);
                           end
                        end
                     end
...
% ------------------------------------------------------------------------------
% RELEASES :
%   03/17/2022 - RNU - creation
% ------------------------------------------------------------------------------
function process_nc_file(a_ncPathFileName, a_tmpDir, a_doIt, a_fileType)
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   ncData = get_var_from_nc_file(a_ncPathFileName, wantedVars);
      
   formatVersion = strtrim(get_data_from_name('FORMAT_VERSION', ncData)');
   if (strcmp(formatVersion, '3.1'))

      % retrieve the file schema
      inputFileSchema = ncinfo(a_ncPathFileName);

      % retrieve the type of the NB_SAMPLE_* variables
      idF = find(strcmp('NB_SAMPLE_CTD', {inputFileSchema.Variables.Name}), 1);
      if (~isempty(idF))
         if (strcmp(inputFileSchema.Variables(idF).Datatype, 'single'))
            updateNeeded = 1;
         end
      end
      idF = find(strcmp('NB_SAMPLE_SFET', {inputFileSchema.Variables.Name}), 1);
      if (~isempty(idF))
         if (strcmp(inputFileSchema.Variables(idF).Datatype, 'single'))
            updateNeeded = 1;
         end
   ...
   if (updateNeeded == 1)
      ...
      if (status == 1)
         ...
         % update the file
         ok = update_file(fileToUpdate, a_fileType);
         
         if (ok == 1)
            
            % move the updated file
            status = movefile(fileToUpdate, a_ncPathFileName);
            if (status ~= 1)
...
%
% SYNTAX :
%  [o_ok] = update_file(a_ncPathFileName, a_fileType)
%
% INPUT PARAMETERS :
...
%   a_fileType : 1: mono-prof, 2: multi-prof, 3: traj
%
% EXAMPLES :
%
% SEE ALSO :
...
% ------------------------------------------------------------------------------
function [o_ok] = update_file(a_ncPathFileName, a_fileType)
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   outputFileSchema = update_file_schema(inputFileSchema);
   ...
   for idVar = 1:length(inputFileSchema.Variables)
      ...
      varData = ncread(a_ncPathFileName, varName);
      if (~isempty(varData))
         if (a_fileType == 1)
            if (ismember(varName, {'NB_SAMPLE_CTD', 'NB_SAMPLE_SFET'}))
               [~, nProf] = size(varData);
               for idProf = 1:nProf
                  if (any(~isnan(varData(:, idProf))))
                     profList = [profList idProf];
                  end
               end
            end
         end
         ncwrite(tmpFileName, varName, varData);
      end
   end

   % we only update HISTORY_* variables for mono-profile
   if (a_fileType == 1)
...
%
% SYNTAX :
%  [o_outputSchema] = update_file_schema(a_inputSchema)
%
% INPUT PARAMETERS :
...
%   03/17/2022 - RNU - creation
% ------------------------------------------------------------------------------
function [o_outputSchema] = update_file_schema(a_inputSchema)

% output parameters initialization
o_outputSchema = [];


% change NB_SAMPLE_CTD and NB_SAMPLE_SFET variables type from 'single' to 'int16'
idF1 = find(strcmp('NB_SAMPLE_CTD', {a_inputSchema.Variables.Name}), 1);
if (~isempty(idF1))
   if (strcmp(a_inputSchema.Variables(idF1).Datatype, 'single'))
      a_inputSchema.Variables(idF1).Datatype = 'int16';
      idF2 = find(strcmp('_FillValue', {a_inputSchema.Variables(idF1).Attributes.Name}), 1);
      if (~isempty(idF2))
         a_inputSchema.Variables(idF1).Attributes(idF2).Value = netcdf.getConstant('NC_FILL_SHORT');
      end
      idF2 = find(strcmp('resolution', {a_inputSchema.Variables(idF1).Attributes.Name}), 1);
      if (~isempty(idF2))
         a_inputSchema.Variables(idF1).Attributes(idF2).Value = int16(1);
      end
   end
end
idF1 = find(strcmp('NB_SAMPLE_SFET', {a_inputSchema.Variables.Name}), 1);
if (~isempty(idF1))
   if (strcmp(a_inputSchema.Variables(idF1).Datatype, 'single'))
      a_inputSchema.Variables(idF1).Datatype = 'int16';
      idF2 = find(strcmp('_FillValue', {a_inputSchema.Variables(idF1).Attributes.Name}), 1);
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis update format tool (nc_update_argo_format_change_nb_sample_type_11aa)'));
docRootNode.appendChild(newChild);
