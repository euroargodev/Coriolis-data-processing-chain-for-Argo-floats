...
%
% SYNTAX :
%  generate_json_float_meta_prv_cts4_ir_sbd_rt(varargin)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function generate_json_float_meta_prv_cts4_ir_sbd_rt(varargin)
...
try
   ...
   if (~isempty(g_cogj_outputLogDirName))
      logFileName = [g_cogj_outputLogDirName '/generate_json_float_meta_prv_cts4_ir_sbd_rt_' currentTime '.log'];
   else
      logFileName = [tempdir '/generate_json_float_meta_prv_cts4_ir_sbd_rt_' currentTime '.log'];
   end
...
if (~isempty(g_cogj_xmlReportDirName))
   xmlFileName = [g_cogj_xmlReportDirName '/generate_json_float_meta_prv_cts4_ir_sbd_rt_' currentTime '.xml'];
else
   xmlFileName = [tempdir '/generate_json_float_meta_prv_cts4_ir_sbd_rt_' currentTime '.xml'];
end
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis float meta-data json file generating tool (generate_json_float_meta_prv_cts4_ir_sbd_rt)'));
docRootNode.appendChild(newChild);
