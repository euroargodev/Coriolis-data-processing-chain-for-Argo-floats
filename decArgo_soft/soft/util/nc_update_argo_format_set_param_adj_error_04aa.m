...
%
% SYNTAX :
%   nc_update_argo_format_set_param_adj_error_04aa or 
%   nc_update_argo_format_set_param_adj_error_04aa(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   05/11/2016 - RNU - V 04aa: update DM data along the following rules:
%                              - if <PARAM>_ADJUSTED is set then
%                              <PARAM>_ADJUSTED_ERROR should be set
%                              - if <PARAM>_ADJUSTED is not set then
%                              <PARAM>_ADJUSTED_ERROR should be set to FillValue
% ------------------------------------------------------------------------------
function nc_update_argo_format_set_param_adj_error_04aa(varargin)
...
DIR_INPUT_OUTPUT_NC_FILES = 'C:\Users\jprannou\_DATA\OUT\test_update_param_adj_error\';
...
g_couf_ncUpdateArgoFormatVersion = '04aa';
...
try
   ...
   tmpDir = [DIR_TMP '/' 'nc_update_argo_format_set_param_adj_error_04aa_' currentTime];
   status = mkdir(tmpDir);
   if (status ~= 1)
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_update_argo_format_set_param_adj_error_04aa_' currentTime '.log'];
   diary(logFile);
            ...
            if ((exist(floatDirPathName, 'dir') == 7) && ~strcmp(floatDirName, '.') && ~strcmp(floatDirName, '..'))
               ...
               if (status == 1)
                  ...
                  if ((isempty(floatList)) || (~isempty(floatList) && ismember(floatWmo, floatList)))
                     ...
                     % we only consider mono-profile files (excluding merged
                     % mono-profile files) - checked with Laure on 05/11/2016
                     
                     % mono-profile file names are: <M><B><R/D>WMO_CyNum<D>.nc
                     % only DWMO_CyNum<D>.nc and BDWMO_CyNum<D>.nc are
                     % considered here
                     floatDirPathName = [floatDirPathName '/profiles'];
                     if (exist(floatDirPathName, 'dir') == 7)
                        floatFiles = dir([floatDirPathName '/' sprintf('*D%d_*.nc', floatWmo)]);
                        for idFile = 1:length(floatFiles)
                           
                           floatFileName = floatFiles(idFile).name;
                           if (floatFileName(1) ~= 'M')
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   if (~isempty(varList))
      ...
      idVal = find(strcmp('FORMAT_VERSION', varList(1:2:end)) == 1, 1);
      formatVersion = strtrim(varList{2*idVal}');
      if (strcmp(formatVersion, '3.1'))
         
         idVal = find(strcmp('DATA_MODE', varList(1:2:end)) == 1, 1);
         dataMode = varList{2*idVal}';
         if (~isempty(dataMode))
            if (any(dataMode == 'D'))
               idVal = find(strcmp('PARAMETER_DATA_MODE', varList(1:2:end)) == 1, 1);
               parameterDataMode = varList{2*idVal}';
               idVal = find(strcmp('STATION_PARAMETERS', varList(1:2:end)) == 1, 1);
               stationParameters = varList{2*idVal};
               dmProfNumList = find(dataMode == 'D');
               paramNameList = [];
               for idProf = 1:length(dmProfNumList)
                  profId = dmProfNumList(idProf);
                  ...
                  if (isempty(parameterDataMode))
                     for idParam = 1:size(stationParameters, 2)
                        ...
                        if (~isempty(paramName))
                           paramNameList = [paramNameList {[paramName '_ADJUSTED']} {[paramName '_ADJUSTED_ERROR']}];
                        end
                     end
                  else
                     profParamDataMode = parameterDataMode(profId, :);
                     dmParamNumList = find(profParamDataMode == 'D');
                     for idParam = 1:length(dmParamNumList)
                        paramId = dmParamNumList(idParam);
                        paramName = strtrim(stationParameters(:, paramId, profId)');
                        if (~isempty(paramName))
                           paramNameList = [paramNameList {[paramName '_ADJUSTED']} {[paramName '_ADJUSTED_ERROR']}];
                        end
                     end
                  ...
                  [ncData] = get_var_from_nc_file_bis(a_ncPathFileName, paramNameList);
                  if (~isempty(ncData))

                     for idParam = 1:6:length(ncData)
                        paramAdj = ncData{idParam+1};
                        profParamAdj = paramAdj(:, profId);
                        paramAdjFillValue = ncData{idParam+2};
                  
                        paramAdjErr = ncData{idParam+4};
                        profParamAdjErr = paramAdjErr(:, profId);
                        paramAdjErrFillValue = ncData{idParam+5};
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   for idProf = 1:length(dmProfNumList)
      ...
      for idParam = 1:length(paramNameList)
         paramName = paramNameList{idParam};
         
         paramAdj = netcdf.getVar(fCdf, netcdf.inqVarID(fCdf, [paramName '_ADJUSTED']));
         profParamAdj = paramAdj(:, profId);
         paramAdjFillValue = netcdf.getAtt(fCdf, netcdf.inqVarID(fCdf, [paramName '_ADJUSTED']), '_FillValue');

         paramAdjErr = netcdf.getVar(fCdf, netcdf.inqVarID(fCdf, [paramName '_ADJUSTED_ERROR']));
         profParamAdjErr = paramAdjErr(:, profId);
         paramAdjErrFillValue = netcdf.getAtt(fCdf, netcdf.inqVarID(fCdf, [paramName '_ADJUSTED_ERROR']), '_FillValue');
         
         idKo = find((profParamAdj ~= paramAdjFillValue) & (profParamAdjErr == paramAdjErrFillValue));
         if (~isempty(idKo))
            
            % retrieve <PARAM>_ADJUSTED_ERROR value
            paramAdjErrValue = unique(paramAdjErr(find(paramAdjErr ~= paramAdjErrFillValue)));
            
            % update <PARAM>_ADJUSTED_ERROR
            if (length(paramAdjErrValue) == 1)
               paramAdjErr(idKo, profId) = paramAdjErrValue;
               netcdf.putVar(fCdf, netcdf.inqVarID(fCdf, [paramName '_ADJUSTED_ERROR']), paramAdjErr);
               profList = [profList profId];

               fprintf('INFO: ''%s'' set to %g at %d levels of profile #%d (file: %s)\n', ...
                  [paramName '_ADJUSTED_ERROR'], paramAdjErrValue, length(idKo), profId, fileName);
            else
               if (isempty(paramAdjErrValue))
                  fprintf('WARNING: ''%s'' should be set at %d levels of profile #%d but cannot be determined (no value) (file: %s)\n', ...
                     [paramName '_ADJUSTED_ERROR'], length(idKo), profId, fileName);
               else
                  fprintf('WARNING: ''%s'' should be set at %d levels of profile #%d but cannot be determined (multiple valuse) (file: %s)\n', ...
                     [paramName '_ADJUSTED_ERROR'], length(idKo), profId, fileName);
               end
            end
         end
         ...
         if (~isempty(idKo))
            ...
            % update <PARAM>_ADJUSTED_ERROR
            paramAdjErr(idKo, profId) = paramAdjErrFillValue;
            netcdf.putVar(fCdf, netcdf.inqVarID(fCdf, [paramName '_ADJUSTED_ERROR']), paramAdjErr);
            profList = [profList profId];
            
            fprintf('INFO: ''%s'' set to fillValue (%g) at %d levels of profile #%d (file: %s)\n', ...
               [paramName '_ADJUSTED_ERROR'], paramAdjErrFillValue, length(idKo), profId, fileName);
         end
      end
   end
   ...
   % mono-profile file names are: <M><B><R/D>WMO_CyNum<D>.nc
   % only DWMO_CyNum<D>.nc and BDWMO_CyNum<D>.nc are considered here
...
%
% SYNTAX :
%  [o_ncVarList] = get_var_from_nc_file_bis(a_ncPathFileName, a_wantedVars)
%
% INPUT PARAMETERS :
%   a_ncPathFileName : NetCDF file name
%   a_wantedVars     : name of the variables to retrieve
...
%   05/11/2016 - RNU - creation
% ------------------------------------------------------------------------------
function [o_ncVarList] = get_var_from_nc_file_bis(a_ncPathFileName, a_wantedVars)
...
if (exist(a_ncPathFileName, 'file') == 2)
   ...
   for idVar = 1:length(a_wantedVars)
      ...
      if (var_is_present_dec_argo(fCdf, varName))
         varValue = netcdf.getVar(fCdf, netcdf.inqVarID(fCdf, varName));
         varFillValue = netcdf.getAtt(fCdf, netcdf.inqVarID(fCdf, varName), '_FillValue');
         o_ncVarList = [o_ncVarList {varName} {varValue} {varFillValue}];
      else
         o_ncVarList = [o_ncVarList {varName} {[]} {[]}];
      end
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis update format tool (nc_update_argo_format_set_param_adj_error_04aa)'));
docRootNode.appendChild(newChild);
