...
%
% SYNTAX :
%  generate_json_float_meta_prv_cts4_ir_rudics_rt(varargin)
%
% INPUT PARAMETERS :
...
%   'calibFileName'      : list of calibration coefficient (retrieved from
%                          decoded data)
%   'configDirName'      : directory of float configuration at launch files
%   'sunaConfigDirName'  : directory of SUNA configuration files
...
% ------------------------------------------------------------------------------
function generate_json_float_meta_prv_cts4_ir_rudics_rt(varargin)
...
global g_cogj_sunaConfigDirName;
...
try
   ...
   if (~isempty(g_cogj_outputLogDirName))
      logFileName = [g_cogj_outputLogDirName '/generate_json_float_meta_prv_cts4_ir_rudics_rt_' currentTime '.log'];
   else
      logFileName = [tempdir '/generate_json_float_meta_prv_cts4_ir_rudics_rt_' currentTime '.log'];
   end
...
if (~isempty(g_cogj_xmlReportDirName))
   xmlFileName = [g_cogj_xmlReportDirName '/generate_json_float_meta_prv_cts4_ir_rudics_rt_' currentTime '.xml'];
else
   xmlFileName = [tempdir '/generate_json_float_meta_prv_cts4_ir_rudics_rt_' currentTime '.xml'];
end
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis float meta-data json file generating tool (generate_json_float_meta_prv_cts4_ir_rudics_rt)'));
docRootNode.appendChild(newChild);
...
if (~isempty(a_varargin))
   if (rem(length(a_varargin), 2) ~= 0)
      ...
      for id = 1:2:length(a_varargin)
         ...
         elseif (strcmpi(a_varargin{id}, 'sunaConfigDirName'))
            g_cogj_sunaConfigDirName = a_varargin{id+1};
...
o_logLines{end+1} = sprintf('sunaConfigDirName: %s\n', g_cogj_sunaConfigDirName);
