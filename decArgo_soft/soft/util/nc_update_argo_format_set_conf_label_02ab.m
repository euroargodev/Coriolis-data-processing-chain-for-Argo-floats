...
% 
% SYNTAX :
%   nc_update_argo_format_set_conf_label_02ab or nc_update_argo_format_set_conf_label_02ab(6900189, 7900118)
% 
% INPUT PARAMETERS :
...
%                              lists below
%   10/05/2016 - RNU - V 02ab: update A SUB STRING of configuration labels:
%                              - see g_couf_oldLabelList and g_couf_newLabelList
%                              lists below
% ------------------------------------------------------------------------------
function nc_update_argo_format_set_conf_label_02ab(varargin)
...
g_couf_ncUpdateArgoFormatVersion = '02ab';
...
try
   ...
   tmpDir = [DIR_TMP '/' 'nc_update_argo_format_set_conf_label_02ab_' currentTime];
   status = mkdir(tmpDir);
   if (status ~= 1)
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_update_argo_format_set_conf_label_02ab_' currentTime '.log'];
   diary(logFile);
      ...
      if (var_is_present_dec_argo(fCdf, varName))
         ...
         for idL = 1:length(g_couf_oldLabelList)
            for id = 1:size(varValue, 2)
               if (strfind(strtrim(varValue(:, id)'), g_couf_oldLabelList{idL}))
                  oldVarValue = strtrim(varValue(:, id)');
                  newVarValue = regexprep(oldVarValue, g_couf_oldLabelList{idL}, g_couf_newLabelList{idL});                  
                  valueStr = repmat(' ', 1, size(varValue, 1));
                  valueStr(1:length(newVarValue)) = newVarValue;
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis update format tool (nc_update_argo_format_set_conf_label_02ab)'));
docRootNode.appendChild(newChild);
...
g_couf_oldLabelList = [ ...
   {'DriftAtParkPresSamplingPeriod_seconds'} ...
   ];
