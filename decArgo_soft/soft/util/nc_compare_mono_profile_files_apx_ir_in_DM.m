...
% Similar to nc_compare_mono_profile_files_apx_ir.m but the files of the BASE set are
% restricted to the cycle numbers less or equal to the maximum cycle number of
% the existing DM files.
%
% SYNTAX :
%   nc_compare_mono_profile_files_apx_ir_in_DM or nc_compare_mono_profile_files_apx_ir_in_DM(6900189, 7900118)
%
% INPUT PARAMETERS :
...
%                      ones)
% ------------------------------------------------------------------------------
function nc_compare_mono_profile_files_apx_ir_in_DM(varargin)

% top directory of base NetCDF mono-profile files
% DIR_INPUT_BASE_NC_FILES = 'C:\Users\jprannou\_DATA\nc_file_apex_co_in_archive_201602\';
% DIR_INPUT_BASE_NC_FILES = 'H:\archive_201608\coriolis\\';
...
% DIR_INPUT_NEW_NC_FILES = 'C:\Users\jprannou\_DATA\OUT\nc_output_decArgo\';
DIR_INPUT_NEW_NC_FILES = 'C:\Users\jprannou\_DATA\OUT\Apx_Ir_rudics_&_Navis_20170817\';
...
logFile = [DIR_LOG_CSV_FILE '/' 'nc_compare_mono_profile_files_apx_ir_in_DM' name '_' datestr(now, 'yyyymmddTHHMMSS') '.log'];
diary(logFile);
tic;
...
outputFileName = [DIR_LOG_CSV_FILE '/' 'nc_compare_mono_profile_files_apx_ir_in_DM' name '_' datestr(now, 'yyyymmddTHHMMSS') '.csv'];
...
for idFloat = 1:nbFloats
   ...
   [descProfNumBase, descProfDateBase, descProfLocDateBase, ...
      ascProfNumBase, ascProfDateBase, ascProfLocDateBase, lastCycle] = ...
      get_nc_profile_dates(DIR_INPUT_BASE_NC_FILES, floatNum, 'Base', -1);
   
   [descProfNumNew, descProfDateNew, descProfLocDateNew, ...
      ascProfNumNew, ascProfDateNew, ascProfLocDateNew, lastCycle] = ...
      get_nc_profile_dates(DIR_INPUT_NEW_NC_FILES, floatNum, 'New', lastCycle);
   ...
   for idProf = 1:length(descProfNumBase)
      ...
      if (descProfNumBase(idProf) == -1)
         ...
         [profDate, profLocDate, profLon, profLat, ...
            profMode, profCut, profNbLevels, fileVersion, dataStr, paramStr, piName] = ...
      ...
      elseif (descProfNumBase2New(idProf) == -1)
         ...
         [profDateBase, profLocDateBase, profLonBase, profLatBase, ...
            profModeBase, profCutBase, profNbLevelsBase, fileVersionBase, dataStrBase, paramStrBase, piName] = ...
         ...
         [profDateNew, profLocDateNew, profLonNew, profLatNew, ...
            profModeNew, profCutNew, profNbLevelsNew, fileVersionNew, dataStrNew, paramStrNew, piName] = ...
   ...
   for idProf = 1:length(ascProfNumBase)
      ...
      elseif (ascProfNumBase2New(idProf) == -1)
         ...
         [profDateBase, profLocDateBase, profLonBase, profLatBase, ...
            profModeBase, profCutBase, profNbLevelsBase, fileVersionBase, ...
            dataStrBase, paramStrBase, piName] = ...
            get_nc_profile_info(profFileNameBase, 1);
...
function [o_descProfNum, o_descProfDate, o_descProfLocDate, ...
   o_ascProfNum, o_ascProfDate, o_ascProfLocDate, o_LastCycle] = ...
   get_nc_profile_dates(a_ncDirName, a_floatNum, a_commentStr, a_lastCycle)
...
if (strcmp(a_commentStr, 'Base'))
   ...
   o_LastCycle = lastCycle;
else
   lastCycle = a_lastCycle;
   %    monoProfFileName = [monoProfDirName sprintf('*%d_*.nc', a_floatNum)];
   %    monoProfFiles = dir(monoProfFileName);
   %    for idFile = 1:length(monoProfFiles)
   %       fileName = monoProfFiles(idFile).name;
...
%    o_profMode, o_profCut, o_profNbLevels, o_fileVersion, o_dataStr, o_paramStr, o_piName] = ...
