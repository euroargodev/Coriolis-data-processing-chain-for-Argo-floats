...
%
% SYNTAX :
%   nc_traj_2_csv or nc_traj_2_csv(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   06/11/2014 - RNU - creation
% ------------------------------------------------------------------------------
function nc_traj_2_csv(varargin)
...
logFile = [DIR_LOG_FILE '/' 'nc_traj_2_csv' name '_' datestr(now, 'yyyymmddTHHMMSS') '.log'];
diary(logFile);
tic;
...
for idFloat = 1:nbFloats
   ...
   if (exist(ncFileDir, 'dir') == 7)
      ...
      for idFile = 1:length(ncFiles)
         ...
         nc_traj_2_csv_file(ncFilePathName, outputFilePathName);
      end
...
%
% SYNTAX :
%  nc_traj_2_csv_file(a_inputPathFileName, a_outputPathFileName)
%
% INPUT PARAMETERS :
