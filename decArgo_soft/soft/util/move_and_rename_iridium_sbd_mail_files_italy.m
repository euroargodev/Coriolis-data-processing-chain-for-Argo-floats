...
%
% SYNTAX :
%   move_and_rename_iridium_sbd_mail_files_italy
%
% INPUT PARAMETERS :
%
% OUTPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   04/04/2017 - RNU - creation
% ------------------------------------------------------------------------------
function move_and_rename_iridium_sbd_mail_files_italy()
...
logFile = [DIR_LOG_FILE '/' 'move_and_rename_iridium_sbd_mail_files_italy_' datestr(now, 'yyyymmddTHHMMSS') '.log'];
diary(logFile);
tic;
...
%          pidNum = fileName(idFUs(end)+1:idFExt-1);
         ...
         for idFile = 1:length(imei)
            ...
            if (length(imei) == 1)
               copy_file_bis(filePathName, [outputDirName newfilename]);
            else
