...
%
% SYNTAX :
%  create_nc_meta_file_3_1_from_json_float_meta_with_config()
%
% INPUT PARAMETERS :
%
% OUTPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function create_nc_meta_file_3_1_from_json_float_meta_with_config(varargin)
...
dirInputJsonConfLabel = configVal{3};
dirInputJsonFloatMetaDataFile = configVal{4};
dirOutputNetcdfFile = configVal{5};
...
for idFloat = 1:length(floatList)
   ...
   % retrieve information about configuration labels
   [ncParamIds, ncParamNames, ncParamDescriptions] = get_nc_config_parameters_json( ...
      dirInputJsonConfLabel, floatDecId);
   ...
   % remove unset config values
   idDel = [];
   for idL = 1:size(configValues, 1)
      if (any(isnan(configValues(idL, :))))
         if (all(isnan(configValues(idL, :))))
            idDel = [idDel idL];
         else
            fprintf('ERROR: Float #%d: NOT IMPLEMENTED YET - ASK FOR AN UPDATE OF THE TOOL\n', floatNum);
         end
      end
   end
   configNames(idDel) = [];
   ...
   for idL = 1:length(configNames)
      ...
      confId = configName(idFUs(1)+1:idFUs(2)-1);
      idF = find(strcmp(confId, ncParamIds));
      if (~isempty(idF))
         ncConfigName = ncParamNames{idF};
         if (strncmp(ncConfigName, 'CONFIG_AUX_', length('CONFIG_AUX_')))
            idDel = [idDel idL];
         else
            configNames{idL} = ncConfigName;
         end
      else
         idDel = [idDel idL];
      end
   ...
   % select mandatory configuration parameters for mission configuration
