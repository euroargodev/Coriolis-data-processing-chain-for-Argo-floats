...
%
% SYNTAX :
%   nc_convert_mono_profile_iridium_to_V3_1 or nc_convert_mono_profile_iridium_to_V3_1(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function nc_convert_mono_profile_iridium_to_V3_1(varargin)
...
% reference files
refNcCFileName1 = 'C:\Users\jprannou\_RNU\DecArgo_soft\work\ref/ArgoProf_V3.1_cfile_part1.nc';
refNcCFileName2 = 'C:\Users\jprannou\_RNU\DecArgo_soft\work\ref/ArgoProf_V3.1_cfile_part2.nc';
   ...
   for idFile = 1:length(monoProfFiles)
      ...
      if (~isempty(strfind(fileName, 'D.nc')))
         ...
         [ok, comment] = convert_mono_profile_to_V3_1(profFileName, ...
            profFilePathNameOutput, refCFileSchema, refBFileSchema, -1, vssInfoStruct, metaData, corCyNumData);
      else
         % ascending profile
         vssInfoStruct.direction = 'A';
         [ok, comment] = convert_mono_profile_to_V3_1(profFileName, ...
            profFilePathNameOutput, refCFileSchema, refBFileSchema, cutOffPres, vssInfoStruct, metaData, corCyNumData);
      end
   end
...
%    a_inputFileName, a_outputFileName, a_refCFileSchema, a_refBFileSchema, a_cutOffPres, ...
...
if (a_cutOffPres == -1)
   [o_ok, o_comment] = convert_file(a_inputFileName, ...
      a_outputFileName, str2num(inputFileFormatVersionStr), a_refCFileSchema, a_refBFileSchema, a_vssInfoStruct, a_metaData, a_corCyNumData);
   ...
   [o_ok, o_comment] = convert_and_update_file(a_inputFileName, ...
      a_outputFileName, str2num(inputFileFormatVersionStr), a_refCFileSchema, a_refBFileSchema, a_cutOffPres, a_vssInfoStruct, a_metaData, a_corCyNumData);
...
%    a_refCFileSchema, a_refBFileSchema, a_vssInfoStruct, a_metaData, a_corCyNumData)
%
% INPUT PARAMETERS :
%   a_inputFileName           : mono-profile NetCDF input file name
%   a_outputFileName          : mono-profile NetCDF output file name
...
if (doxyAdded == 1)
   ...
   presValue = inputMeasData{2*idVal};
   idVal = find(strcmp('TEMP', inputMeasData(1:2:end)) == 1, 1);
   tempValue = inputMeasData{2*idVal};
   idVal = find(strcmp('PSAL', inputMeasData(1:2:end)) == 1, 1);
   ...
   idValDoxyAdj = find(strcmp('DOXY_ADJUSTED', inputMeasData(1:2:end)) == 1, 1);
...
%    a_inputFileName, a_outputFileName, a_inputFileFormatVersion, ...
%    a_refCFileSchema, a_refBFileSchema, a_cutOffPres, a_vssInfoStruct, a_metaData, a_corCyNumData)
%
% INPUT PARAMETERS :
%   a_inputFileName           : mono-profile NetCDF input file name
%   a_outputFileName          : mono-profile NetCDF output file name
...
if (~isempty(floatWmo) && ~isempty(dacFormatId) && ~isempty(metaConfMisNum))
   ...
   if (argosFloat == 1)
      ...
      if (length(repRateMetaData) == 1)
         ...
         configMisNum = [];
         for idRep = 1:length(repRateMetaData)
            repRate = str2num(repRateMetaData{idRep}.(char(fieldnames(repRateMetaData{idRep}))));
            configMisNum = [configMisNum repmat(idRep, 1, repRate)];
         end
         o_confMissionNumber = metaConfMisNum(configMisNum(corCycleNumber));
      end
   end
end
