   ...
   if (~isempty(configNames))
      ...
      switch (a_decoderId)
         ...
         case {201, 202, 203, 205, 206, 207, 208, 209, 215, 216, 218, 221}
            ...
            if (~isempty(prof.additionnalBottomThreshold))
               nbThreshold = 3;
               threshold3 = prof.additionnalBottomThreshold;
               thickBottom2 = 1;
            end
         case {210, 211, 212, 222, 223, 224, 213, 214, 217}
            nbThreshold = 2;
      ...
      if (prof.direction == 'A')
            ...
            if (~ismember(a_decoderId, [224]))
               ...
               if (~isempty(ascSamplingPeriod) && ~isempty(profilePres) && ...
                  ...
                  description = sprintf( ...
                     ...
                     ascSamplingPeriod, thickBottom, profilePres, threshold2, ...
                     ascSamplingPeriod, thickMiddle, threshold2, threshold1, ...
                     ascSamplingPeriod, thickSurf, threshold1);
               else
                  description = '';
                  fprintf('WARNING: Float #%d Cycle #%d: Missing information to create the description of the vertical sampling scheme\n', ...
                     g_decArgo_floatNum, ...
         ...
         elseif (nbThreshold == 3)
            if (~isempty(ascSamplingPeriod) && ~isempty(profilePres) && ...
               ...
               if (prof.primarySamplingProfileFlag == 1)
                  description = sprintf( ...
                     ['%d s sampling, %d dbar avg from %d dbar to %.1f dbar;' ...
                     '%d s sampling, %d dbar avg from %.1f dbar to %d dbar;' ...
                     '%d s sampling, %d dbar avg from %d dbar to %d dbar;', ...
                     '%d s sampling, %d dbar avg from %d dbar to %.1f dbar'], ...
                     ascSamplingPeriod, thickBottom2, profilePres, threshold3, ...
                     ascSamplingPeriod, thickBottom, threshold3, threshold2, ...
                     ascSamplingPeriod, thickMiddle, threshold2, threshold1, ...
                     ascSamplingPeriod, thickSurf, threshold1, prof.presCutOffProf);
               else
                  description = sprintf( ...
