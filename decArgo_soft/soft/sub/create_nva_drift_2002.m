...
%    o_parkPres, o_parkTemp, o_parkSal, o_parkTempDoxy, o_parkPhaseDelayDoxy] = ...
%    create_nva_drift_2002(a_dataCTDO, ...
...
packType = [50 53];
idForType = find((a_dataCTDO(:, 1) >= packType(1)) & (a_dataCTDO(:, 1) <= packType(2)));
for idP = 1:length(idForType)
   ...
   for idMeas = 1:nbMeas
      ...
      o_parkPres = [o_parkPres; data(idMeas+3+g_decArgo_maxCTDOSampleInDovaDataPacket)];
      o_parkTemp = [o_parkTemp; data(idMeas+3+g_decArgo_maxCTDOSampleInDovaDataPacket*2)];
      o_parkSal = [o_parkSal; data(idMeas+3+g_decArgo_maxCTDOSampleInDovaDataPacket*3)];
      o_parkTempDoxy = [o_parkTempDoxy; data(idMeas+3+g_decArgo_maxCTDOSampleInDovaDataPacket*4)];
      o_parkPhaseDelayDoxy = [o_parkPhaseDelayDoxy; data(idMeas+3+g_decArgo_maxCTDOSampleInDovaDataPacket*5)];
   end
end
