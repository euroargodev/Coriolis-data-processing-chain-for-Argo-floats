...
%
% SYNTAX :
%  [o_tabProfiles] = merge_profile_meas_ir_rudics_cts5_usea_uvp_lpm(a_tabProfiles)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_tabProfiles] = merge_profile_meas_ir_rudics_cts5_usea_uvp_lpm(a_tabProfiles)
   ...
   for idP = 1:length(idProfForDir)
      ...
      switch (a_tabProfiles(idProf).treatType)
         case {g_decArgo_treatRaw, g_decArgo_treatAverage, g_decArgo_treatDecimatedRaw}
            ...
            if (a_tabProfiles(idProf).treatType == g_decArgo_treatAverage)
               finalData = [finalData; ...
                  data];
            else
               finalData = [finalData; ...
