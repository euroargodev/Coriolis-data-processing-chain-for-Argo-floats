...
%
% SYNTAX :
%  [o_tabTrajNMeas, o_tabTrajNCycle, o_tabTechNMeas] = process_trajectory_data_218( ...
...
% ------------------------------------------------------------------------------
function [o_tabTrajNMeas, o_tabTrajNCycle, o_tabTechNMeas] = process_trajectory_data_218( ...
   a_cycleNum, a_deepCycle, ...
   a_gpsData, a_iridiumMailData, ...
   a_cycleStartDate, ...
   a_descentToParkStartDate, a_firstStabDate, a_firstStabPres, a_descentToParkEndDate, ...
...
if (~isempty(a_inAirDate))
   ...
   for idMeas = 1:length(a_inAirPres)
      ...
      if (a_inAirDate(idMeas) ~= g_decArgo_dateDef)
         ...
         measStruct = create_one_meas_float_time(g_MC_InAirSeriesOfMeasPartOfSurfaceSequenceRelativeToTST, a_inAirDate(idMeas), measTimeStatus, floatClockDrift);
      else
      ...
      if (~isempty(a_inAirC1PhaseDoxy))
         measStruct.paramList = [paramPres paramTemp paramSal paramC1PhaseDoxy paramC2PhaseDoxy paramTempDoxy paramPpoxDoxy];
      else
...
if (a_deepCycle == 1)
   ...
   if (~isempty(a_nearSurfDate))
      ...
      for idMeas = 1:length(a_nearSurfPres)
         ...
         if (a_nearSurfDate(idMeas) ~= g_decArgo_dateDef)
            ...
            measStruct = create_one_meas_float_time(g_MC_InWaterSeriesOfMeasPartOfSurfaceSequenceRelativeToTST, a_nearSurfDate(idMeas), measTimeStatus, floatClockDrift);
         else
            measStruct = get_traj_one_meas_init_struct();
   ...
   if (~isempty(tabTech2) && (a_iceDetected == 0))
      ...
      pres = sensor_2_value_for_pressure_201_203_215_216_218(tabTech2(11));
      temp = sensor_2_value_for_temperature_201_to_203_215_216_218(tabTech2(12));
      psal = tabTech2(13)/1000;
      if (any([pres temp psal] ~= 0))
         measStruct = get_traj_one_meas_init_struct();
         measStruct.measCode = g_MC_LastAscPumpedCtd;
         if (any(tabTech2(14:16) ~= 0) && any(tabTech2(14:16) ~= 65535))
            paramPres = get_netcdf_param_attributes('PRES');
            paramTemp = get_netcdf_param_attributes('TEMP');
            paramSal = get_netcdf_param_attributes('PSAL');
            paramC1PhaseDoxy = get_netcdf_param_attributes('C1PHASE_DOXY');
            ...
            c1PhaseDoxy = sensor_2_value_C1C2Phase_doxy_201_to_203_206_to_209_213_to_218(14);
            c2PhaseDoxy = sensor_2_value_C1C2Phase_doxy_201_to_203_206_to_209_213_to_218(15);
            tempDoxy = sensor_2_value_for_temp_doxy_201_to_203_206_to_209_213_to_218(16);
            doxy = compute_DOXY_201_203_206_209_213_to_218(c1PhaseDoxy, c2PhaseDoxy, tempDoxy, pres, temp, psal);
