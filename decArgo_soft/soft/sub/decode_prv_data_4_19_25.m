...
%    decode_prv_data_4_19_25(a_tabSensors, a_tabDates, a_decoderId)
%
% INPUT PARAMETERS :
...
for idMes = 1:size(a_tabSensors, 1)
   ...
   switch (msgType)
      ...
      case 0
         ...
         % CTS3-DO V4.41 & V4.43
         if ((a_decoderId == 19) || (a_decoderId == 25))
      ...
      case {4, 6}
         ...
         % there are between 4 and 6 PTSO measurements per Argos message

         % get the first PTSO date and data
         firstBit = 21;
         tabNbBits = [9 11 15 15 9];
         ...
         while (1)
            ...
            if (formatBit == 0)
               ...
               if (presCountsOk(curCtdoMes-1) == 0)
            ...
            curBit = curBit + 1;
            if (formatBit == 0)
               value = get_bits(curBit, 9, msgData);
               if (isempty(value))
                  break
               end
               ...
               relOxyCounts = get_bits(curBit, 5, msgData);
               if (isempty(relOxyCounts))
                  break
               end
               oxyCounts(curCtdoMes) = oxyCounts(curCtdoMes-1) + relOxyCounts - 16;
               curBit = curBit + 5;
            end
   ...
   if (~isempty(presCounts))
      ...
      % store the data
      presCounts(nbMes+1:6) = ones(1, 6-(nbMes+1)+1)*g_decArgo_presCountsDef;
      presCountsOk(nbMes+1:6) = ones(1, 6-(nbMes+1)+1)*g_decArgo_presCountsOkDef;
      tempCounts(nbMes+1:6) = ones(1, 6-(nbMes+1)+1)*g_decArgo_tempCountsDef;
      salCounts(nbMes+1:6) = ones(1, 6-(nbMes+1)+1)*g_decArgo_salCountsDef;
      oxyCounts(nbMes+1:6) = ones(1, 6-(nbMes+1)+1)*g_decArgo_molarDoxyCountsDef;
      if (msgType == 5)
...
if (~isempty(tabDrifCTDO))
   parkPres = reshape(tabDrifCTDO(:, 5:10), size(tabDrifCTDO, 1)*6, 1);
   parkPresOk = reshape(tabDrifCTDO(:, 11:16), size(tabDrifCTDO, 1)*6, 1);
...
if (~isempty(tabProfCTDO))
   ...
   if (~isempty(idAscProf))
      profPres = reshape(tabProfCTDO(idAscProf, 5:10), size(idAscProf, 1)*6, 1);
      profPresOk = reshape(tabProfCTDO(idAscProf, 11:16), size(idAscProf, 1)*6, 1);
