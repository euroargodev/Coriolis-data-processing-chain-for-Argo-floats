...
% ------------------------------------------------------------------------------
function update_float_config_ir_sbd_204(a_floatParam)
...
for id = [0:15 18 20:23]
   ...
   idPos = find(strcmp(name, configNames) == 1, 1);
   if (~isempty(idPos))
      newConfig(idPos) = floatParam(id+24);
   end
end
name = 'CONFIG_PX00';
idPos = find(strcmp(name, configNames) == 1, 1);
if (~isempty(idPos))
   if (floatParam(9) > g_decArgo_firstDeepCycleNumber)
...
if (floatParam(40) ~= 1)
   ...
   if (mod(floatParam(9)-1, floatParam(40)) == 0)
      % profile pressure is PT17
      idPos = find(strcmp(configNames, 'CONFIG_PM09') == 1, 1);
      if (~isempty(idPos))
         newConfig(idPos) = floatParam(41)*10;
      end
   end
end
...
if (floatParam(43) ~= 0)
   ...
   if (~isempty(configNames))
      ...
      if (~isempty(parkPresPrevCycle))
         ...
         if (~isempty(idPos))
            newConfig(idPos) = parkPresPrevCycle + floatParam(43);
         end
      end
   end
end
