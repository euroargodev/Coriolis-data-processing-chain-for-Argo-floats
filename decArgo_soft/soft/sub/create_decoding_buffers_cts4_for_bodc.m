% ------------------------------------------------------------------------------
% Create decoding buffers.create_decoding_buffers_cts4_for_bodc
%
% SYNTAX :
...
%   12/15/2022 - RNU - specific version for 6904182 BODC float - not reliable in
%                      case of delayed transmission
% ------------------------------------------------------------------------------
function [o_decodedData] = create_decoding_buffers_cts4_for_bodc(a_decodedData, a_decoderId)
   ...
   switch g_decArgo_floatNum
      case 6903247
         ...
         if (any((tabDate > afterResetDate1) & ((tabCyNumFile >= 294) | (tabCyNumRaw >= 294))))
            fprintf('ERROR: Float #%d: float data cannot be processed => add for an update of the create_decoding_buffers_cts4_for_bodc code\n', ...
               g_decArgo_floatNum);
         else
...
% compute max profNumCyNumPrev and profNumCyNumNext for each cyNumRaw
tabMaxProfNumCyNumPrev = zeros(1, length(tabCyNumRaw)); % if a cyNumRaw is not received assign 0 to its profNumRaw
tabMaxProfNumCyNumNext = zeros(1, length(tabCyNumRaw)); % if a cyNumRaw is not received assign 0 to its profNumRaw
uTabCyNumRaw = unique(tabCyNumRaw);
for idS = 1:length(uTabCyNumRaw)
   idF1 = find(tabCyNumRaw == uTabCyNumRaw(idS));
   idF2 = find(tabCyNumRaw == max(uTabCyNumRaw(idS)-1, 0));
   if (~isempty(idF2))
      tabMaxProfNumCyNumPrev(idF1) = max(tabProfNumRaw(idF2));
   end
   idF3 = find(tabCyNumRaw == uTabCyNumRaw(idS)+1);
   if (~isempty(idF3))
      tabMaxProfNumCyNumNext(idF1) = max(tabProfNumRaw(idF3));
   end
end
...
if (ismember(a_decoderId, g_decArgo_decoderIdListNkeCts4Ice))
   ...
   tabCyNum(idSurfSensorParam) = (tabCyNumRaw(idSurfSensorParam)+1)*100 + tabMaxProfNumCyNumNext(idSurfSensorParam);
end
...
tabCyNum(idSurf) = max(tabCyNumRaw(idSurf)-1, 0)*100 + tabMaxProfNumCyNumPrev(idSurf);
...
for sesNum = sessionList
   ...
   if ((length(idFVectorTechSatTrans) > 1) || ...
      ...
      for idB = 1:length(baseList)-1
         ...
         delayed = 1;
         if (any(ismember(tabPackType(idForCheck), [248, 254, 255])))
            fprintf('\nINFO: Float #%d : session number %d : packets (248, 254 or 255) have been received in delayed transmission - check packet profNum consistency\n', ...
         ...
         if (completed == 1)
            ...
            tabCyNum(idF) = (tabCyNumRaw(idF)+1)*100 + tabMaxProfNumCyNumNext(idF);
...
% assign not managed configuration packets (248, 249, 254, 255) to current
% transmission session => NOT RELIABLE IN CASE OF DELAYED TRANSMISSION
idToDo = find(tabDone == 0);
for idTD = 1:length(idToDo)
   if (ismember(tabPackType(idToDo(idTD)), [248 254 255]))
      rankList = unique(tabRank(tabSession == tabSession(idToDo(idTD))));
      rankList(rankList == -1) = [];
      tabRank(idToDo(idTD)) = rankList;
      cynNumList = unique(tabCyNum(tabSession == tabSession(idToDo(idTD))));
      cynNumList(cynNumList == tabCyNum(idToDo(idTD))) = [];
      tabCyNum(idToDo(idTD)) = cynNumList;
      profNumRawList = unique(tabProfNumRaw(tabSession == tabSession(idToDo(idTD))));
      profNumRawList(profNumRawList == tabProfNumRaw(idToDo(idTD))) = [];
      tabProfNumRaw(idToDo(idTD)) = profNumRawList;
      tabDone(idToDo(idTD)) = 1;
      tabDelayed(idToDo(idTD)) = 0;
      tabCompleted(idToDo(idTD)) = 1;
      tabGo(idToDo(idTD)) = 1;
   end
end
