...
%    decode_data_apx_8(a_argosDataData, a_argosDataUsed, a_argosDataDate, ...
...
profData = ones(15+(lastMsgNum-3)*29, 1)*hex2dec('FF');
profReceived = zeros(15+(lastMsgNum-3)*29, 1);
profRedundancy = ones(15+(lastMsgNum-3)*29, 1)*-1;
...
for idL = 1:size(a_sensorData, 1)
   ...
   elseif (msgNum == 3)
      ...
      firstBit = 17;
      % item bit lengths
      tabNbBits = [16 12 12 16 16 16 12 12];
      % get item bits
      decData = get_bits(firstBit, tabNbBits, msgData);
      ...
      % store profile data
      profData(1:15) = msgData(17:31);
      profReceived(1:15) = 255;
      profRedundancy(1:15) = msgRed;
      ...
      % store profile data
      profData(15+(msgNum-4)*29+1:15+(msgNum-4)*29+29) = msgData(3:31);
      profReceived(15+(msgNum-4)*29+1:15+(msgNum-4)*29+29) = 255;
      profRedundancy(15+(msgNum-4)*29+1:15+(msgNum-4)*29+29) = msgRed;
