...
%    compute_prv_dates_215(a_tabTech, a_refDay)
%
% INPUT PARAMETERS :
...
elseif (length(idF1) == 1)
   ...
   if (~isempty(o_cycleStartDate))
      ...
      if (mod(g_decArgo_cycleNum, inAirAcqPeriod) == 0)
         ...
         % note that PT32 is not correctly reported (always = 0) in firmware
         % 5608A11 => we use the value of PT32 at launch
         if (finalBuoyancyAcqSec == 0)
            idPos = find(strncmp('CONFIG_PT32', g_decArgo_floatConfig.DYNAMIC.NAMES, length('CONFIG_PT32')) == 1, 1);
            finalBuoyancyAcqSec = g_decArgo_floatConfig.DYNAMIC.VALUES(idPos, 1)/100;
         end
