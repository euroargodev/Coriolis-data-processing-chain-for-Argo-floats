...
% ------------------------------------------------------------------------------
function print_buffer_info_ir_rudics_cts4_105_to_110_112
      ...
      for idM = 1:size(cycleProf, 1)
         ...
         for idS = 1:length(sensorNum)
            ...
            if (~isempty(id0SensorCycleProfDesc))
               nbRecDesc = length(id0SensorCycleProfDesc);
            end
            ...
            if (~isempty(id0SensorCycleProfDrift))
               nbRecDrift = length(id0SensorCycleProfDrift);
            end
            ...
            if (~isempty(id0SensorCycleProfAsc))
               nbRecAsc = length(id0SensorCycleProfAsc);
            end
