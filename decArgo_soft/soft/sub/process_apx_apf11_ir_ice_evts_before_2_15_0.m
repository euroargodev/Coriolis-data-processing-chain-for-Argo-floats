...
%
% SYNTAX :
%  [o_iceDetection] = process_apx_apf11_ir_ice_evts_before_2_15_0(a_events)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_iceDetection] = process_apx_apf11_ir_ice_evts_before_2_15_0(a_events)
...
for idEv = 1:length(events)
   ...
   if (any(strfind(dataStr, PATTERN_THERMAL_DETECT)))
      ...
      elseif (any(strfind(dataStr, PATTERN_THERMAL_DETECT_MEDIAN_TEMP)))
         idStart = strfind(dataStr, MEDIAN_TEMP) + length(MEDIAN_TEMP) + 1;
         idEnd = strfind(dataStr(idStart:end), ' ');
         medianTemp = str2double(dataStr(idStart:idStart+idEnd(1)-2));
