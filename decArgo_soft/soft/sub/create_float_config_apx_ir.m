...
%
% SYNTAX :
%  create_float_config_apx_ir(a_configInfoLog, a_configInfoMsg, a_decoderId)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function create_float_config_apx_ir(a_configInfoLog, a_configInfoMsg, a_decoderId)
...
for idInput = 1:2
   ...
   for idC = 1:length(inputConfigInfo)
      ...
      [configName, configValue, metaName] = get_config(configInfo.label, configInfo.value, a_decoderId);
...
if (isnan(configValues(idF)))
   % try to find, in a previous configuration what may be the configuration
   % value
   cyNum = g_decArgo_cycleNum - 1;
   confNum = [];
   while ((cyNum >= 0) && isempty(confNum))
      idFCy = find(g_decArgo_floatConfig.USE.CYCLE == cyNum);
      if (~isempty(idFCy))
         confNum = g_decArgo_floatConfig.USE.CONFIG(idFCy);
      else
         cyNum = cyNum - 1;
      end
   end
   ...
   idFL = find(strcmp(g_decArgo_floatConfig.NAMES, 'CONFIG_N_ParkAndProfileCycleLength'));
   prevConfVal = g_decArgo_floatConfig.VALUES(idFL, confNum+1);
   if (~isnan(prevConfVal))
      fprintf('WARNNG: Float #%d Cycle #%d: Configuration parameter ''%s'' is mandatory => set to previous value: %d\n', ...
         g_decArgo_floatNum, ...
         g_decArgo_cycleNum, ...
         'CONFIG_N_ParkAndProfileCycleLength', prevConfVal);
      configValues(idF) = prevConfVal;
   else
...
%    get_config(a_configInfoLabel, a_configInfoValue, a_decoderId)
%
% INPUT PARAMETERS :
...
switch (a_configInfoLabel)
   ...
   case 'FloatId'
      if (~ismember(a_decoderId, [1314]))
         ...
         o_metaName = []; % for Apex Iridium SBD floats PTT is used to store IMEI number
      end
   case 'FloatRudicsId'
      if (~ismember(a_decoderId, [1314]))
         o_metaName = 'PTT';
