...
%
% SYNTAX :
%  [o_tabTrajNMeas, o_tabTrajNCycle] = process_trajectory_data_202( ...
...
% ------------------------------------------------------------------------------
function [o_tabTrajNMeas, o_tabTrajNCycle] = process_trajectory_data_202( ...
   a_cycleNum, a_deepCycle, ...
   a_gpsData, a_iridiumMailData, ...
   a_cycleStartDate, ...
   a_descentToParkStartDate, a_firstStabDate, a_firstStabPres, a_descentToParkEndDate, ...
...
if (a_deepCycle == 1)
   ...
   if (~isempty(a_tabTech))
      ...
      if ~((length(unique(tabTech2(13:15))) == 1) && (unique(tabTech2(13:15)) == 0))
         ...
         pres = sensor_2_value_for_pressure_202(tabTech2(10));
