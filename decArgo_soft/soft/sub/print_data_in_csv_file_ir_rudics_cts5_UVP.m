      ...
      switch (treat)
         case {'(RW)', '(DW)'}
            if (ismember(phase, [{'PARK'} {'SHORT_PARK'} {'SURFACE'}]))
               fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; IMAGE_NUMBER_PARTICLES (count); DEPTH_SIZE_SPECTRA_PARTICLES (dbar); TEMP_PARTICLES (°C); NB_SIZE_SPECTRA_PARTICLES (nb/analyzed images);;;;;;;;;;;;;;;;;; GREY_SIZE_SPECTRA_PARTICLES (bit)\n', ...
               ...
               fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; DEPTH_SIZE_SPECTRA_PARTICLES (dbar); TEMP_PARTICLES (°C); NB_SIZE_SPECTRA_PARTICLES (Nb/analyzed images);;;;;;;;;;;;;;;;;; GREY_SIZE_SPECTRA_PARTICLES (bit)\n', ...
                  g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
                  fileTypeStr, phase, sensorNum, sensorName);
...
if (~isempty(a_uvpBlackData))
   ...
   for idP = 1:length(a_uvpBlackData)
      ...
      switch (treat)
         case {'(RW)', '(DW)'}
            fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; DEPTH_BLACK_PARTICLE (dbar); BLACK_TEMP (°C); BLACK_NB_SIZE_SPECTRA_PARTICLES (count)\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
               fileTypeStr, phase, sensorNum, sensorName);
