      ...
      switch (treat)
         case {'(RW)', '(DW)'}
            if (ismember(phase, [{'PARK'} {'SHORT_PARK'} {'SURFACE'}]))
               fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; NB_IMAGE_PARTICLES (count); DEPTH_SIZE_SPECTRA_PARTICLES (dbar); TEMP_PARTICLES (degC); NB_SIZE_SPECTRA_PARTICLES_PER_IMAGE (nb/analyzed images);;;;;;;;;;;;;;;;;; GREY_SIZE_SPECTRA_PARTICLES (bit)\n', ...
               ...
               fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; DEPTH_SIZE_SPECTRA_PARTICLES (dbar); TEMP_PARTICLES (degC); NB_SIZE_SPECTRA_PARTICLES_PER_IMAGE (Nb/analyzed images);;;;;;;;;;;;;;;;;; GREY_SIZE_SPECTRA_PARTICLES (bit)\n', ...
                  g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
                  fileTypeStr, phase, sensorNum, sensorName);
...
if (~isempty(a_uvpLpmV2Data))
   ...
   for idP = 1:length(a_uvpLpmV2Data)
      ...
      switch (treat)
         case {'(RW)', '(DW)', '(AM)'}
            fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; DEPTH_SIZE_SPECTRA_PARTICLES (dbar); NB_IMAGE_PARTICLES (count); TEMP_PARTICLES (degC); NB_SIZE_SPECTRA_PARTICLES (nb/analyzed images);;;;;;;;;;;;;;;;;; GREY_SIZE_SPECTRA_PARTICLES (bit)\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
               fileTypeStr, phase, sensorNum, sensorName);
...
if (~isempty(a_uvpBlackV2Data))
   ...
   for idP = 1:length(a_uvpBlackV2Data)
      ...
      switch (treat)
         case {'(RW)'}
            fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; DEPTH_BLACK_PARTICLE (dbar); BLACK_NB_IMAGE_PARTICLES (count); BLACK_TEMP_PARTICLES (degC); BLACK_NB_SIZE_SPECTRA_PARTICLES (count)\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
               fileTypeStr, phase, sensorNum, sensorName);
   ...
   for idP = 1:length(a_uvpTaxoV2Data)
      ...
      switch (treat)
         case {'(RW)', '(DW)', '(AM)'}
            ...
            fprintf(g_decArgo_outputCsvFileId, ['%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; DEPTH_SIZE_SPECTRA_PARTICLES (dbar); NB_IMAGE_CATEGORY (count); NUMBER_OF_OBJECT (count)' headerData '\n'], ...
               g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
               fileTypeStr, phase, sensorNum, sensorName);
