...
for idP = 1:length(a_uvpBlackData)
   ...
   elseif (phaseId == g_decArgo_cts5PhaseSurface)
      ...
      fprintf('WARNING: Float #%d Cycle #%d: (Cy,Ptn)=(%d,%d): Nothing done yet for processing UVP-BLACK profiles with phase Id #%d\n', ...
         g_decArgo_floatNum, ...
         g_decArgo_cycleNum, ...
   ...
   if (~isempty(data))
      ...
      switch (treatId)
         ...
         otherwise
            fprintf('ERROR: Float #%d Cycle #%d: (Cy,Ptn)=(%d,%d): Treatment #%d not managed - UVP-BLACK data ignored\n', ...
               g_decArgo_floatNum, ...
               g_decArgo_cycleNum, ...
               g_decArgo_cycleNumFloat, ...
               g_decArgo_patternNumFloat, ...
