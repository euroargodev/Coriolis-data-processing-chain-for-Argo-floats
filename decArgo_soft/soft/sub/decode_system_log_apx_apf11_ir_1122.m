...
for idFile = 1:length(a_systemLogFileList)
   ...
   idEvts = find(strcmp({events.functionName}, 'mission_cfg') | strcmp({events.functionName}, 'sample_cfg'));
   if (~isempty(idEvts))
      [missionCfg, sampleCfg] = process_apx_apf11_ir_config_evts_1122(events(idEvts));
   ...
   if (~isempty(idEvts))
      [o_cycleTimeData] = process_apx_apf11_ir_time_evts_1122(events(idEvts), o_cycleTimeData);
   ...
   if (~isempty(idEvts))
      grounding = process_apx_apf11_ir_grounding_evts_1122(events(idEvts));
