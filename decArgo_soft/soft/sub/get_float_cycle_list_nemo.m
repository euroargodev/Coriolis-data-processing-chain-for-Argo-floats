% ------------------------------------------------------------------------------
% Retrieve the list of existing cycles of a NEMO float.
%
% SYNTAX :
%  [o_cycleList] = get_float_cycle_list_nemo(a_floatNum, a_floatRudicsId)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_cycleList] = get_float_cycle_list_nemo(a_floatNum, a_floatRudicsId)
...
existingCycles = [];
fileNames = dir([iriDirName sprintf('%04d', a_floatRudicsId) '*' num2str(a_floatNum) '*.profile']);
for idFile = 1:length(fileNames)
   fileName = fileNames(idFile).name;
   idF = strfind(fileName, '_');
   cyNum = fileName(idF(end)+1:idF(end)+4);
   [cyNum, status] = str2num(cyNum);
   if (status)
      if (ismember(cyNum, existingCycles))
         fprintf('ERROR: Float #%d: multiple .profile files for cycle #%d\n', a_floatNum, cyNum);
      end
