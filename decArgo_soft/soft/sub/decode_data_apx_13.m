...
%    decode_data_apx_13(a_argosDataData, a_argosDataUsed, a_argosDataDate, ...
...
profData = ones(14+(lastMsgNum-2)*29, 1)*hex2dec('FF');
profReceived = zeros(14+(lastMsgNum-2)*29, 1);
profRedundancy = ones(14+(lastMsgNum-2)*29, 1)*-1;
...
for idL = 1:size(a_sensorData, 1)
   ...
   elseif (msgNum == 2)
      ...
      o_parkData.paramList = [paramPres paramTemp paramSal paramFrequencyDoxy paramDoxy];
      
      % add parameter data to the data structure
      o_parkData.data = [parkPres parkTemp parkSal parkFrequencyDoxy parkDoxy];
      
      % add parameter data redundancy to the profile structure
      o_parkData.dataRed = repmat(msgRed, 1, 5);  
      ...
      % store profile data
      profData(14+(msgNum-3)*29+1:14+(msgNum-3)*29+29) = msgData(3:31);
      profReceived(14+(msgNum-3)*29+1:14+(msgNum-3)*29+29) = 255;
      profRedundancy(14+(msgNum-3)*29+1:14+(msgNum-3)*29+29) = msgRed;
...
if (nbLev > 0)
   ...
   o_profData.paramList = [paramPres paramTemp paramSal paramFrequencyDoxy paramDoxy];
   
   % add parameter data to the profile structure
   o_profData.data = [profPres profTemp profSal profFrequencyDoxy profDoxy];
   
   % add parameter data redundancy to the profile structure
   o_profData.dataRed = [profPresRed profTempRed profSalRed profFrequencyDoxyRed profDoxyRed];
end
