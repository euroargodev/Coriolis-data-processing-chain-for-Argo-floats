...
%    o_floatClockDrift] = compute_nva_dates_2001_2002(a_tabTech, a_deepCycle)
%
% INPUT PARAMETERS :
...
if (g_decArgo_finalEolMode == 0)
   ...
   if (size(a_tabTech, 1) == 1)
      ...
      if ~((a_tabTech(id, 40+ID_OFFSET) == 0) && ...
         ...
         o_gpsDate = gregorian_2_julian_dec_argo(sprintf('%04d/%02d/%02d %s', ...
            year, month, a_tabTech(id, 40+ID_OFFSET), format_time_dec_argo(a_tabTech(id, 41+ID_OFFSET))));
      else
      ...
      o_cycleStartDate = gregorian_2_julian_dec_argo(sprintf('%04d/%02d/%02d %s', ...
         a_tabTech(id, (48:-1:46)+ID_OFFSET), format_time_dec_argo(a_tabTech(id, 45+ID_OFFSET))));
      ...
      if (a_deepCycle == 1)
         ...
         cycleTimeStruct.timeToGpsFix = a_tabTech(id, 49+ID_OFFSET);
      end
   ...
   for idTech = 1:size(a_tabTech, 1)
      ...
      if ~((a_tabTech(idTech, 40+ID_OFFSET) == 0) && ...
         ...
         if (day < a_tabTech(idTech, 40+ID_OFFSET))
            if (month > 1)
               month = month - 1;
            else
               month = 12;
         ...
         o_gpsDate = gregorian_2_julian_dec_argo(sprintf('%04d/%02d/%02d %s', ...
            year, month, a_tabTech(idTech, 40+ID_OFFSET), format_time_dec_argo(a_tabTech(idTech, 41+ID_OFFSET))));
