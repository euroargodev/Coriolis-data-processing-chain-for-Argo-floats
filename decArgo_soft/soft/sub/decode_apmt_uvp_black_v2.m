...
%
% SYNTAX :
%  [o_uvpBlackV2Data] = decode_apmt_uvp_black_v2(a_data, a_lastByteNum, a_inputFilePathName)
%
% INPUT PARAMETERS :
%   a_data              : input UVP-BLK V2 data to decode
%   a_lastByteNum       : number of the last useful byte of the data
%   a_inputFilePathName : APMT UVP file to decode
%
% OUTPUT PARAMETERS :
%   o_uvpBlackV2Data : UVP-BLK decoded data
%
% EXAMPLES :
%
% SEE ALSO :
...
% ------------------------------------------------------------------------------
function [o_uvpBlackV2Data] = decode_apmt_uvp_black_v2(a_data, a_lastByteNum, a_inputFilePathName)
...
while (currentByte <= lastByteNum)
   ...
   if ((currentPhaseNum ~= -1) && (currentTreatNum ~= -1))
      ...
      if (ismember(currentTreatNum, [g_decArgo_cts5Treat_RW]))
         % raw data
         data = nan(1, 9);
