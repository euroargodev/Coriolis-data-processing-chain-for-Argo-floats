...
%
% SYNTAX :
%  update_mail_data_ir_rudics_sbd2(a_tabTrajNMeas)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function update_mail_data_ir_rudics_sbd2(a_tabTrajNMeas)
...
for id = 1:length(idFCy)
   ...
   elseif (~isempty(idF2))
      % first cycle
      cyNum = g_decArgo_iridiumMailData(idF2).cycleNumber;
      idForCy = find([g_decArgo_iridiumMailData.cycleNumber] == cyNum);
      if ((min([g_decArgo_iridiumMailData(idForCy).timeOfSessionJuld]) - g_decArgo_iridiumMailData(idFCy(id)).timeOfSessionJuld) < MIN_SUB_CYCLE_DURATION_IN_DAYS)
         tabCyNum(id) = cyNum;
      else
         tabCyNum(id) = max(0, cyNum - 1);
      end
   elseif (~isempty(idF1))
      % last cycle
