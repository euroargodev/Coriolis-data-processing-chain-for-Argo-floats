...
for sesNum = sessionList
   ...
   [completed, deep, ~] = check_buffer(idForCheck, tabPackType, ...
      tabExpNbDesc, tabExpNbDrift, tabExpNbAsc, tabExpNbNearSurface, tabExpNbInAir, cyNum, 0, 0);
   delayed = 0;
...
for cyNum = 1:length(cyNumList)
   ...
   % check current session contents
   [completed, deep, ~] = check_buffer(idForCheck, tabPackType, tabExpNbDesc, tabExpNbDrift, tabExpNbAsc, a_decoderId, cyNum, 0, 1);
   if (completed == 1)
      tabCompleted(idForCheck) = 1;
   end
end
      ...
      for rankNum = rankNumList
         ...
         if (tabCompleted(idRankCy) == 0)
            [~, ~, why] = check_buffer(idForRankCy, tabPackType, ...
               tabExpNbDesc, tabExpNbDrift, tabExpNbAsc, tabExpNbNearSurface, tabExpNbInAir, cyNum, 1, 0);
...
if (~isempty(g_decArgo_outputCsvFileId))
   if (1)
      ...
      if (fId ~= -1)
         ...
         header = '#;Rank;RnkByCycle;Session;SesDeep;Base;Date;DiffDate;Eol;CyNum;IrSession;Deep;Done;Delayed;Completed;Go;PackType;ExpNbDesc;tabExpNbDrift;tabExpNbAsc;tabExpNbNS;tabExpNbIA;CyNumRaw;ResetDate;ResetFlag;OffsetDate;PackTypeInfo';
         fprintf(fId, '%s\n', header);
         ...
         for idL = 1:length(tabPackType)
            ...
            fprintf(fId, '%d;%d;%d;%d;%d;%d;%s;%s;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%s;%d;%d;%s\n', ...
               idL, ...
               tabRank(idL), ...
               tabRankByCycle(idL), ...
               tabSession(idL), ...
...
%    a_tabExpNbNearSurface, a_tabExpNbInAir, a_cycleNum, a_whyFlag, a_msgFlag)
%
% INPUT PARAMETERS :
