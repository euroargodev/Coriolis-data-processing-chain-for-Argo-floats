...
%   01/19/2024 - RNU - V 6.5: TEST #8: revised version of "Pressure increasing
%                             test"
%                             New global rule: if PRES_QC=4 then <PARAM>_QC=4
%                             for c, b, ic and ib parameters (tests #6, #8, #13
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% List of parameters that have at least one RTQC test (see
...
if (testFlagList(15) == 1)
   ...
   % parameters
   if (~isempty(presQc4ProfIdList))
      [dataStruct, testFailedList, ~] = update_qc_for_bad_pres(dataStruct, unique(presQc4ProfIdList), testFailedList, [], 15);
   end
end
...
if (testFlagList(19) == 1)
   ...
   if (isempty(profilePres))
      ...
      % list of PRES parameters concerned by this test
      presParameterList = [ ...
         {'PRES'}; ...
         {'PRES2'}; ...
         ];
      ...
      % for PRES
      constPresDataModeParameterList = [ ...
         {'PRES'}; ...
         {'TEMP'}; ...
         {'PSAL'}; ...
         {'CNDC'}; ...
      ...
      for idPres = 1:length(presParameterList)
         presName = presParameterList{idPres};

         for idProf = 1:length(juld)

            for idParam = 1:length(ncParamNameList)
               paramName = ncParamNameList{idParam};
               if (ismember(paramName, constPresDataModeParameterList))
                  presDataModeFlag = 0;
               else
                  presDataModeFlag = 1;
               end
...
if (testFlagList(8) == 1)
               ...
               if (length(profPres) > 1)
                  if (strncmp(vssList{idProf}, 'Near-surface sampling:', length('Near-surface sampling:')))
                     ...
                     for id = idStart-1:-1:1
                        if (profPres(id) >= pMin + PRES_REVERSAL)
                     ...
                     for id = idStart+1:length(profPres)
                        if (profPres(id) <= pMax - PRES_REVERSAL)
                           idToFlag = [idToFlag id];
                        else
                           pMax = profPres(id);
                        end
   ...
   % parameters
   if (~isempty(presQc4ProfIdList))
      [dataStruct, testFailedList, testFailedListForTraj] = update_qc_for_bad_pres(dataStruct, unique(presQc4ProfIdList), testFailedList, testFailedListForTraj, 8);
   end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% OBSOLETE REPLACED BY TEST 25 (EXCEPT FOR DOXY)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (testFlagList(13) == 1)
   ...
   for idProf = 1:length(juld)
      if (~strncmp(vssList{idProf}, 'Near-surface sampling:', length('Near-surface sampling:'))) % test not performed on NS profile (where values can be stuck)
         ...
         for idP = 1:length(ncParamNameList)
   ...
   % parameters
   if (~isempty(presQc4ProfIdList))
      [dataStruct, testFailedList, testFailedListForTraj] = update_qc_for_bad_pres(dataStruct, unique(presQc4ProfIdList), testFailedList, testFailedListForTraj, 13);
   end
end
...
%
% SYNTAX :
% [o_dataStruct, o_testFailedList, o_testFailedListForTraj] = ...
%   update_qc_for_bad_pres(a_dataStruct, a_profIdList, a_testFailedList, a_testFailedListForTraj, a_testNum)
%
% INPUT PARAMETERS :
%   a_dataStruct            : input data structure
%   a_profIdList            : list of profile Id with any PRES_QC = 4
%   a_testFailedList        : input profile test report list
%   a_testFailedListForTraj : input trajectory test report list
%   a_testNum               : test number
% OUTPUT PARAMETERS :
%   o_dataStruct            : output data structure
%   o_testFailedList        : output profile test report list
%   o_testFailedListForTraj : output trajectory test report list
%
% EXAMPLES :
%
% SEE ALSO :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   01/19/2024 - RNU - creation
% ------------------------------------------------------------------------------
function [o_dataStruct, o_testFailedList, o_testFailedListForTraj] = ...
   update_qc_for_bad_pres(a_dataStruct, a_profIdList, a_testFailedList, a_testFailedListForTraj, a_testNum)

% output parameters initialization
o_dataStruct = a_dataStruct;
o_testFailedList = a_testFailedList;
o_testFailedListForTraj = a_testFailedListForTraj;

% QC flag values
global g_decArgo_qcStrBad;           % '4'


if (~isempty(a_profIdList))

   ncParamNameList = o_dataStruct.ncParamNameList;
   ...
   for idPres = 1:length(presNameList)
      presName = presNameList{idPres};

      for idProf = a_profIdList
         ...
         for idDM = 1:2
            ...
            % retrieve PRES data
            [presData, presDataQc, ~, ~, ~] = ...
               get_param_data(presName, o_dataStruct, idProf, dataMode);

            if (~isempty(presData))
               profPresQc = presDataQc(idProf, :);
               idToFlag = find(profPresQc == g_decArgo_qcStrBad);
               if (~isempty(idToFlag))
                  for idP = 1:length(ncParamNameList)
                     paramName = ncParamNameList{idP};
                     if (~strncmp(paramName, 'PRES', length('PRES')))
                        [paramData, paramDataQc, paramDataFillValue, ~, paramDataQcName] = ...
                           get_param_data(paramName, o_dataStruct, idProf, dataMode);
                        if (~isempty(paramData))
                           profParam = paramData(idProf, :);
                           idNoDef = find((profParam(idToFlag) ~= paramDataFillValue));

                           if (~isempty(idNoDef))
                              % set QC 4
                              paramDataQc(idProf, idToFlag(idNoDef)) = set_qc(paramDataQc(idProf, idToFlag(idNoDef)), g_decArgo_qcStrBad);
                              o_dataStruct.(paramDataQcName) = paramDataQc;
                              if (~isempty(o_testFailedList))
                                 o_testFailedList(a_testNum, idProf) = 1;
                              end
                              if (~isempty(o_testFailedListForTraj))
                                 o_testFailedListForTraj{a_testNum, idProf} = [o_testFailedListForTraj{13, idProf} idToFlag(idNoDef)];
                              end
                           end
                        end
                     end
