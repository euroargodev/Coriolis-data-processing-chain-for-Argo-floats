...
%
% SYNTAX :
%  [o_tabProfiles] = merge_profile_meas_ir_rudics_cts5_usea_crover(a_tabProfiles)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_tabProfiles] = merge_profile_meas_ir_rudics_cts5_usea_crover(a_tabProfiles)
   ...
   for idP = 1:length(idProfForDir)
      ...
      switch (a_tabProfiles(idProf).treatType)
         case {g_decArgo_treatRaw, g_decArgo_treatAverage, g_decArgo_treatDecimatedRaw}
            ...
            finalData = [finalData; ...
               data, ...
               ones(size(data, 1), 1)*paramTransParticleBeamAtt660StDev.fillValue, ...
               ones(size(data, 1), 1)*paramPresMed.fillValue, ...
               ones(size(data, 1), 1)*paramTransParticleBeamAtt660Med.fillValue, ...
               ];
