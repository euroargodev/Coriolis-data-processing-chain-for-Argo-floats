...
%
% SYNTAX :
%  [o_sbephData] = decode_apmt_sbeph(a_fileNameInfo)
%
% INPUT PARAMETERS :
%   a_fileNameInfo : information on APMT SBEPH file to decode
%
% OUTPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_sbephData] = decode_apmt_sbeph(a_fileNameInfo)
...
if ~(exist(inputFilePathName, 'file') == 2)
   fprintf('ERROR: decode_apmt_sbeph: File not found: %s\n', inputFilePathName);
   return
end
...
try
   ...
   switch (data(1))
      case {11}
         o_sbephData = decode_apmt_sbeph_data(data, lastByteNum, inputFilePathName);
      otherwise
