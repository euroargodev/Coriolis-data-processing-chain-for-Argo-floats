...
fprintf(g_decArgo_outputCsvFileId, ['%d; %d; %d; %s; SUNA APF; Date; ' ...
...
dataAPF(:, 13) = compute_profile_NITRATE_from_MOLAR_NITRATE_105_to_109_121(dataAPF(:, 11), ...
...
for idL = 1:size(dataAPF, 1)
   if (dataAPF(idL, 1) ~= g_decArgo_dateDef)
      ...
      fprintf(g_decArgo_outputCsvFileId, '%d; %d; %d; %s; SUNA APF; %s; %g; %g; %g; %g; %g; %g; %g; %g; %g; %g; %g', ...
         g_decArgo_floatNum, a_cycleNum, a_profNum, get_phase_name(a_phaseNum), ...
         date, dataAPF(idL, 3:13));
   else
      fprintf(g_decArgo_outputCsvFileId, '%d; %d; %d; %s; SUNA raw; ; %g; %g; %g; %g; %g; %g; %g; %g; %g; %g; %g', ...
         g_decArgo_floatNum, a_cycleNum, a_profNum, get_phase_name(a_phaseNum), ...
         dataAPF(idL, 3:13));
   end
