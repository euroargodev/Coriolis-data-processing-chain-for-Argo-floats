...
%    create_prv_drift_3_17_24_31(a_tabDrifCTD, a_nbDriftMeas, ...
...
for idMsg = 1:size(a_tabDrifCTD, 1)
   ...
   if (noDates == 0)
      ...
      hourDelay = a_tabDrifCTD(idMsg, 3);
      date = descentStartDay + day + ceil(descentStartHour+hourDelay)/24;

      msgDates(idMsg) = date;
      
%       if (idMsg == 1)
%          if (firstDriftMesDate ~= date)
%             fprintf('ERROR: **** DIFFERE THEORIE ***********************************************************\n');
%          end
%       end
   end
...
% if (~isempty(find(msgNbMeas > a_descentToProfStartDate, 1)))
%    fprintf('ERROR: ******* HORS INTERVALLE **********************************************\n');
% end
