...
%
% SYNTAX :
%  [o_miscInfo, o_auxInfo, o_profData, o_parkData, o_astData, o_surfData, ...
%    o_metaData, o_techData, o_trajData, o_timeInfo, o_timeData, o_presOffsetData] = ...
%    decode_data_apx_6(a_argosDataData, a_argosDataUsed, a_argosDataDate, ...
...
% ------------------------------------------------------------------------------
function [o_miscInfo, o_auxInfo, o_profData, o_parkData, o_astData, o_surfData, ...
   o_metaData, o_techData, o_trajData, o_timeInfo, o_timeData, o_presOffsetData] = ...
   decode_data_apx_6(a_argosDataData, a_argosDataUsed, a_argosDataDate, ...
   a_sensorData, a_sensorDate, a_cycleNum, a_timeData, a_presOffsetData, a_decoderId)
...
profData = ones(6+(lastMsgNum-3)*29, 1)*hex2dec('FF');
profReceived = zeros(6+(lastMsgNum-3)*29, 1);
profRedundancy = ones(6+(lastMsgNum-3)*29, 1)*-1;
...
for idL = 1:size(a_sensorData, 1)
   ...
   elseif (msgNum == 3)
      ...
      firstBit = 17;
      % item bit lengths
      tabNbBits = [16 12 12 16 16 16 12 12 16 16 16 12 12];
      % get item bits
      decData = get_bits(firstBit, tabNbBits, msgData);
      ...
      surfPres = sensor_2_value_for_apex_apf9_pressure(decData(1), g_decArgo_presDef);
      surfBPhaseDoxy = sensor_2_value_for_apex_apf9_bPhaseDoxy(decData(2), g_decArgo_bPhaseDoxyDef);
      surfTempDoxy = sensor_2_value_for_apex_apf9_tempDoxy(decData(3), g_decArgo_tempDoxyDef);
      
      % compute PPOX_DOXY
      surfDoxy = compute_PPOX_DOXY_1006(surfPres, ...
         surfBPhaseDoxy, surfTempDoxy, ...
         g_decArgo_presDef, ...
         g_decArgo_bPhaseDoxyDef, g_decArgo_tempDoxyDef, g_decArgo_doxyDef);
      ...
      o_surfData.dataRed = repmat(msgRed, 1, 4);

      % data sampled at end of the park phase
      parkTemp = sensor_2_value_for_apex_apf9_temperature(decData(4), g_decArgo_tempDef);
      parkSal = sensor_2_value_for_apex_apf9_salinity(decData(5), g_decArgo_salDef);
      parkPres = sensor_2_value_for_apex_apf9_pressure(decData(6), g_decArgo_presDef);
      parkBPhaseDoxy = sensor_2_value_for_apex_apf9_bPhaseDoxy(decData(7), g_decArgo_bPhaseDoxyDef);
      parkTempDoxy = sensor_2_value_for_apex_apf9_tempDoxy(decData(8), g_decArgo_tempDoxyDef);
      
      % compute DOXY
      parkDoxy = compute_DOXY_1006(parkPres, parkTemp, parkSal, ...
         parkBPhaseDoxy, parkTempDoxy, ...
      ...
      % add parameter variables to the data structure
      o_parkData.paramList = [paramPres paramTemp paramSal paramBPhaseDoxy paramTempDoxy paramDoxy];
      
      % add parameter data to the data structure
      o_parkData.data = [parkPres parkTemp parkSal parkBPhaseDoxy parkTempDoxy parkDoxy];
      
      % add parameter data redundancy to the profile structure
      o_parkData.dataRed = repmat(msgRed, 1, 6);
      
      % data sampled at start of the profile phase
      astTemp = sensor_2_value_for_apex_apf9_temperature(decData(9), g_decArgo_tempDef);
      astSal = sensor_2_value_for_apex_apf9_salinity(decData(10), g_decArgo_salDef);
      astPres = sensor_2_value_for_apex_apf9_pressure(decData(11), g_decArgo_presDef);
      astBPhaseDoxy = sensor_2_value_for_apex_apf9_bPhaseDoxy(decData(12), g_decArgo_bPhaseDoxyDef);
      astTempDoxy = sensor_2_value_for_apex_apf9_tempDoxy(decData(13), g_decArgo_tempDoxyDef);

      % compute DOXY
      astDoxy = compute_DOXY_1006(astPres, astTemp, astSal, ...
         astBPhaseDoxy, astTempDoxy, ...
         g_decArgo_presDef, g_decArgo_tempDef, g_decArgo_salDef, ...
         g_decArgo_bPhaseDoxyDef, g_decArgo_tempDoxyDef, g_decArgo_doxyDef);
      ...
      astBPhaseDoxy(find(astBPhaseDoxy == g_decArgo_bPhaseDoxyDef)) = paramBPhaseDoxy.fillValue;
      astTempDoxy(find(astTempDoxy == g_decArgo_tempDoxyDef)) = paramTempDoxy.fillValue;
      astDoxy(find(astDoxy == g_decArgo_doxyDef)) = paramDoxy.fillValue;

      % add parameter variables to the data structure
      o_astData.paramList = [paramPres paramTemp paramSal paramBPhaseDoxy paramTempDoxy paramDoxy];
      
      % add parameter data to the data structure
      o_astData.data = [astPres astTemp astSal astBPhaseDoxy astTempDoxy astDoxy];
      
      % add parameter data redundancy to the profile structure
      o_astData.dataRed = repmat(msgRed, 1, 6);      

      % store profile data
      profData(1:6) = msgData(26:31);
      profReceived(1:6) = ones(6, 1)*255;
      profRedundancy(1:6) = msgRed;
      ...
      % store profile data
      profData(6+(msgNum-4)*29+1:6+(msgNum-4)*29+29) = msgData(3:31);
      profReceived(6+(msgNum-4)*29+1:6+(msgNum-4)*29+29) = ones(29, 1)*255;
      profRedundancy(6+(msgNum-4)*29+1:6+(msgNum-4)*29+29) = msgRed;
...
if (nbLev > 0)
   ...
   firstBit = 1;
   % item bit lengths
   tabNbBits = repmat([16 16 16 12 12], 1, nbLev);
   % get item bits
   ...
   for idLev = 1:nbLev
      ...
      profPresRaw = [profPresRaw; profData((idLev-1)*9+5) profData((idLev-1)*9+6)];
   ...
   redData = ones(nbLev*5, 1)*-1;
   for id = 1:nbLev
      redData((id-1)*5+1) = min(profRedundancy((id-1)*9+1), profRedundancy((id-1)*9+2));
      redData((id-1)*5+2) = min(profRedundancy((id-1)*9+3), profRedundancy((id-1)*9+4));
      redData((id-1)*5+3) = min(profRedundancy((id-1)*9+5), profRedundancy((id-1)*9+6));
      redData((id-1)*5+4) = min(profRedundancy((id-1)*9+7), profRedundancy((id-1)*9+8));
      redData((id-1)*5+5) = min(profRedundancy((id-1)*9+8), profRedundancy((id-1)*9+9));
   end
   profTempRed = redData(1:5:end);
   profSalRed = redData(2:5:end);
   profPresRed = redData(3:5:end);
   profBPhaseDoxyRed = redData(4:5:end);
   profTempDoxyRed = redData(5:5:end);
   ...
   if (profileLength >= 0)
      ...
      if (profPres(end) <= 6)
         ...
         if (~isempty(idLastPres))
            idOk = find(rem(idLastPres-5, 9) == 0);
            if (~isempty(idOk))
               idLastPres = idLastPres(idOk);
               firstAuxByte = idLastPres(1) + 5;
            end
         end
      end
   end
   ...
   % compute DOXY
   profDoxy = compute_DOXY_1006(profPres, profTemp, profSal, ...
      profBPhaseDoxy, profTempDoxy, ...
      g_decArgo_presDef, g_decArgo_tempDef, g_decArgo_salDef, ...
      g_decArgo_bPhaseDoxyDef, g_decArgo_tempDoxyDef, g_decArgo_doxyDef);
   ...
   o_profData.paramList = [paramPres paramTemp paramSal paramBPhaseDoxy paramTempDoxy paramDoxy];
   
   % add parameter data to the profile structure
   o_profData.data = [profPres profTemp profSal profBPhaseDoxy profTempDoxy profDoxy];
...
if (((profileLength >= 0) && (length(profData) > profileLength*9)) || ...
      ((firstAuxByte ~= -1) && (length(profData) >= firstAuxByte)))
   
   if (profileLength >= 0)
      auxData = profData(nbLev*9+1:end);
      msgRed = max(profRedundancy(nbLev*9+1:end)); % because we have only one useful redundancy (with -1 if the first bytes have not been received, see 3901080 #145)
      auxReceived = profReceived(nbLev*9+1:end);
   else
