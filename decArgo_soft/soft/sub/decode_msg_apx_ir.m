% ------------------------------------------------------------------------------
% Decode msg file from one cycle of APEX Iridium data.
%
% SYNTAX :
...
for idFile = 1:length(a_msgFileList)
   ...
   if (idFile == length(a_msgFileList))
      if (~isempty(driftMeasDataStr))
         o_driftData = parse_apx_ir_drift_data(driftMeasDataStr, a_decoderId);
      end
   end
   ...
   if (idFile == length(a_msgFileList))
      if (~isempty(profLowResMeasDataStr))
         [o_parkData, o_profLrData, expectedProfLrNbSamples] = ...
            parse_apx_ir_LR_profile_data(profLowResMeasDataStr, a_decoderId);
   ...
   if (idFile == length(a_msgFileList))
      if (~isempty(profHighResMeasDataStr))
         [o_profHrData, profHrInfo] =  ...
            decode_apx_ir_HR_profile_data(profHighResMeasDataStr, a_decoderId);
   ...
   if (~isempty(gpsFixDataStr))
      [o_gpsData, o_gpsInfo, o_techData] = ...
         process_apx_ir_gps_data(gpsFixDataStr, o_techData, o_gpsData);
