...
%    decode_data_apx_4(a_argosDataData, a_argosDataUsed, a_argosDataDate, ...
...
firstAuxByte = -1;
% management of information transmitted twice in the data
techId1023Done = 0;
trajMc595Done = 0;
...
for idL = 1:size(a_sensorData, 1)
   ...
   elseif (msgNum == 2)
      ...
      firstBit = 17;
      % item bit lengths
      tabNbBits = [1 1 1 2 2 2 2 2 2 2 2 2 2 2 2 2]*8;
      % get item bits
      decData = get_bits(firstBit, tabNbBits, msgData);
      ...
      dataStruct.value = num2str(decData(1));
      o_techData{end+1} = dataStruct;

      dataStruct = get_apx_misc_data_init_struct('Data msg', msgNum, msgRed, msgDate);
      dataStruct.label = 'Ice evasion record';
      dataStruct.raw = sprintf('%#X', decData(2));
      dataStruct.rawFormat = '%s';
      dataStruct.value = dec2bin(decData(2), 8);
      dataStruct.format = '%s';
      o_miscInfo{end+1} = dataStruct;
      ...
      nbMixedlayerTempSamples = decData(3);
      ...
      if (dataStruct.value ~= g_decArgo_tempDef)
         ...
         dataStruct.value = num2str(sensor_2_value_for_apex_apf9_temperature(decData(5), g_decArgo_tempDef));
         o_techData{end+1} = dataStruct;
      end
      ...
      dataStruct.raw = decData(16);
      dataStruct.rawFormat = '%d';
      dataStruct.value = sensor_2_value_for_apex_apf9_pressure(decData(16), g_decArgo_presDef);
      dataStruct.format = '%.1f';
      dataStruct.unit = 'dbar';
      o_miscInfo{end+1} = dataStruct;
...
if (((profileLength >= 0) && (length(profData) > profileLength*6)) || ...
   ...
   if ~((length(unique(auxData)) == 1) && (unique(auxData) == 255))
      ...
      tabNbBits = [1 2 1 repmat(1, 1, nbPresMarkMax)]*8;
      % get item bits
      ...
      if ~((msgRed < 2) && (length(decData) >= 3) && ...
         ...
         nbMixedlayerTempSamples = -1;
         nbPresMark = -1;
         presMark = [];
         for idL = 1:length(decData)
            
            if (idL == 1)
               if ((receivedData(1) == 255) && (decData(1) ~= 255))
                  dataStruct = get_apx_misc_data_init_struct('Aux data', lastMsgNum, msgRed, a_sensorDate(end));
                  dataStruct.label = 'Number of temperature samples collected between 50dbars and the surface';
                  dataStruct.value = decData(1);
                  dataStruct.format = '%d';
                  o_auxInfo{end+1} = dataStruct;
                  ...
                  nbMixedlayerTempSamples = decData(1);
               end
            elseif (idL == 2)
