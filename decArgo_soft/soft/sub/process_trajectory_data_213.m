...
%
% SYNTAX :
%  [o_tabTrajNMeas, o_tabTrajNCycle, o_tabTechNMeas] = process_trajectory_data_213( ...
%    a_cycleNum, a_deepCycle, a_irSessionNum, ...
...
% ------------------------------------------------------------------------------
function [o_tabTrajNMeas, o_tabTrajNCycle, o_tabTechNMeas] = process_trajectory_data_213( ...
   a_cycleNum, a_deepCycle, a_irSessionNum, ...
   a_gpsData, a_iridiumMailData, ...
   a_cycleStartDate, ...
   a_descentToParkStartDate, a_firstStabDate, a_firstStabPres, a_descentToParkEndDate, ...
...
if (a_deepCycle == 1)
   ...
   if (~isempty(a_cycleStartDate))
      ...
      for idP = 1:size(a_evAct, 1)
         tabDate = [tabDate; a_evAct(idP, 2) + g_decArgo_julD2FloatDayOffset];
         tabPres = [tabPres; a_evAct(idP, 3)];
         tabDur = [tabDur; a_evAct(idP, 4)];
         tabType = [tabType; 6];
      end
         
      for idP = 1:size(a_pumpAct, 1)
         tabDate = [tabDate; a_pumpAct(idP, 2) + g_decArgo_julD2FloatDayOffset];
         tabPres = [tabPres; a_pumpAct(idP, 3)];
         tabDur = [tabDur; a_pumpAct(idP, 4)];
         tabType = [tabType; 7];
      end
   ...
   if (~isempty(tabTech2))
      ...
      temp = sensor_2_value_for_temperature_204_to_214_217(tabTech2(16+ID_OFFSET));
   ...
   if (a_irSessionNum == 1)
      ...
      % to keep N_CYCLE arrays for the prelude and the surface cycles
      trajNCycleStruct.surfOnly = 2;
      
      trajNCycleStruct.grounded = 'U';
      
      % transmission start date
      measStruct = create_one_meas_float_time(g_MC_TST, -1, g_JULD_STATUS_9, floatClockDrift);
      trajNMeasStruct.tabMeas = [trajNMeasStruct.tabMeas; measStruct];
