...
if (~isempty(profInfo))
   ...
   for idP = 1:length(idSensor6)
      ...
      % the received data (i.e. looking for 'TEMP' parameter in the profile
      % measured parameters) than from the SUNA configuration)
      % FOR PROVOR CTS5:
      ...
      if (a_decoderId <= 120)
         % PROVOR CTS4 float
         if (isempty(find(strcmp('TEMP', paramNameList) == 1, 1))) % to be sure we are in "APF frame" mode
            % look for the associated CTD profile
            idF = find((profInfo(:, 2) == 0) & ...
         ...
         idF = find((profInfo(:, 2) == 6) & ...
            ...
            (profInfo(:, 5) == profSuna.profileNumber) & ...
            (profInfo(:, 6) == profSuna.phaseNumber));
         idF = setdiff(idF, idSensor6(idP));
         if (length(idF) == 1)
            profCtd = a_tabProfiles(profInfo(idF, 1));
         else
            if (isempty(idF))
               fprintf('WARNING: Float #%d Cycle #%d Profile #%d: unable to find the SUNA associated CTD profile to compute NITRATE parameter for ''%c'' profile of SUNA sensor => NITRATE data set to fill value\n', ...
                  g_decArgo_floatNum, ...
                  profSuna.cycleNumber, ...
                  profSuna.profileNumber, ...
                  profSuna.direction);
            else
               fprintf('WARNING: Float #%d Cycle #%d Profile #%d: %d SUNA associated CTD profiles have been found to compute NITRATE parameter for ''%c'' profile of SUNA sensor => NITRATE data set to fill value\n', ...
                  g_decArgo_floatNum, ...
                  profSuna.cycleNumber, ...
                  profSuna.profileNumber, ...
                  length(idF), ...
...
for idP = 1:length(paramToDeriveList)
   ...
   if (~isempty(idF))
      ...
      downIrr380 = compute_DOWN_IRRADIANCE380_105_to_112_121_122_124( ...
         a_profOcr.data(:, idF), ...
...
for idP = 1:length(paramToDeriveList)
   ...
   if (~isempty(idF))
      ...
      downIrr412 = compute_DOWN_IRRADIANCE412_105_to_112_121_122_124( ...
         a_profOcr.data(:, idF), ...
         paramToDerive.fillValue, derivedParam.fillValue);
...
for idP = 1:length(paramToDeriveList)
   ...
   if (~isempty(idF))
      ...
      downIrr490 = compute_DOWN_IRRADIANCE490_105_to_112_121_122_124( ...
         a_profOcr.data(:, idF), ...
         paramToDerive.fillValue, derivedParam.fillValue);
...
for idP = 1:length(paramToDeriveList)
   ...
   if (~isempty(idF))
      ...
      downPar = compute_DOWNWELLING_PAR_105_to_112_121_122_124( ...
         a_profOcr.data(:, idF), ...
         paramToDerive.fillValue, derivedParam.fillValue);
...
for idP = 1:length(paramToDeriveList)
   ...
   if (~isempty(idF))
      ...
      chla = compute_CHLA_105_to_112_121_122_124( ...
         a_profEco2.data(:, idF), ...
         paramToDerive.fillValue, derivedParam.fillValue);
...
for idP = 1:length(paramToDeriveList)
   ...
   if (~isempty(idF))
      ...
      cdom = compute_CDOM_105_to_107_110_112_121_122_124( ...
         a_profEco3.data(:, idF), ...
         paramToDerive.fillValue, derivedParam.fillValue);
...
if (~isempty(ctdDataNoDef))
   ...
   % measurements
   ctdIntData = compute_interpolated_CTD_measurements(ctdDataNoDef, a_BBP_pres, 0);
   if (~isempty(ctdIntData))
      
      idNoNan = find(~(isnan(ctdIntData(:, 2)) | isnan(ctdIntData(:, 3))));
      
      if (a_lambda == 700)
         o_BBP(idNoNan) = compute_BBP700_105_to_112_121_122_124( ...
...
for idP = 1:length(paramToDeriveList)
   ...
   if (~isempty(idF))
      ...
      bbp700 = compute_BBP700_105_to_112_121_122_124_V1( ...
         a_profEco3.data(:, idF), ...
         paramToDerive.fillValue, derivedParam.fillValue);
...
if (~FITLM_MATLAB_FUNCTION_NOT_AVAILABLE)
   if (a_decoderId ~= 110)
      ...
      for idP = 1:size(paramToDeriveList, 1)
         ...
         if (~isempty(idF1) && ~isempty(idF2))
            ...
            nitrate = compute_profile_NITRATE_105_to_109_111_112_121_122_124( ...
   ...
   for idP = 1:length(paramToDeriveList)
      ...
      if (~isempty(idF))
         ...
         nitrate = compute_prof_NITRATE_from_MOLAR_105_to_109_111_112_121_122_124( ...
            a_profSuna.data(:, idF), ...
            paramToDerive.fillValue, derivedParam.fillValue, ...
            a_profSuna.data(:, 1), ctdMeasData, ...
            paramPres.fillValue, ...
...
if (~isempty(ctdDataNoDef))
   ...
   % measurements
   ctdIntData = compute_interpolated_CTD_measurements(ctdDataNoDef, a_DOXY_pres, 0);
   if (~isempty(ctdIntData))
      ...
      switch (a_decoderId)
         ...
         case {112, 123}
            ...
            o_DOXY(idNoNan) = compute_DOXY_112_123( ...
               a_C1PHASE_DOXY(idNoNan), ...
               a_C2PHASE_DOXY(idNoNan), ...
               a_TEMP_DOXY(idNoNan), ...
               a_C1PHASE_DOXY_fillValue, ...
...
if (~isempty(ctdDataNoDef))
   ...
   ctdIntData = compute_interpolated_CTD_measurements(ctdDataNoDef, a_VRS_PH_pres, 0);
   if (~isempty(ctdIntData))
      
      idNoNan = find(~(isnan(ctdIntData(:, 2)) | isnan(ctdIntData(:, 3))));
      
      % compute PH_IN_SITU_FREE and PH_IN_SITU_TOTAL values
      [o_PH_IN_SITU_FREE(idNoNan), o_PH_IN_SITU_TOTAL(idNoNan)] = compute_PH_123( ...
         a_VRS_PH(idNoNan), ...
         a_VRS_PH_fillValue, ...
         ctdIntData(idNoNan, 1), ...
         ctdIntData(idNoNan, 2), ...
