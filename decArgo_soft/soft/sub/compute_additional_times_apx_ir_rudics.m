...
%
% SYNTAX :
%  [o_timeDataLog] = compute_additional_times_apx_ir_rudics( ...
...
% ------------------------------------------------------------------------------
function [o_timeDataLog] = compute_additional_times_apx_ir_rudics( ...
   a_timeDataLog, a_driftData)      
...
if (~isempty(a_driftData))
   ...
   parkDescentTime = get_config_value_apx_ir_rudics('CONFIG_PDP_ParkDescentPeriod', g_decArgo_cycleNum);
   downTime = get_config_value_apx_ir_rudics('CONFIG_DOWN_DownTime', g_decArgo_cycleNum);
   deepProfileDescentTime = get_config_value_apx_ir_rudics('CONFIG_DPDP_DeepProfileDescentPeriod', g_decArgo_cycleNum);
   deepProfilePressure = get_config_value_apx_ir_rudics('CONFIG_TP_ProfilePressure', g_decArgo_cycleNum);
   parkPressure = get_config_value_apx_ir_rudics('CONFIG_PRKP_ParkPressure', g_decArgo_cycleNum);
   pnPCycleLen = get_config_value_apx_ir_rudics('CONFIG_N_ParkAndProfileCycleLength', g_decArgo_cycleNum);
   
   timeDataLog = o_timeDataLog;
   if (isempty(timeDataLog))
      timeDataLog = get_apx_ir_rudics_float_time_init_struct;
   end
