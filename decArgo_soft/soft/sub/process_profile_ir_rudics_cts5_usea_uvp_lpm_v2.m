% ------------------------------------------------------------------------------
% Create the UVP-LPM V2 profiles of CTS5-USEA decoded data.
%
% SYNTAX :
%  [o_tabProfiles, o_tabDrift, o_tabDesc2Prof, o_tabDeepDrift, o_tabSurf] = ...
%    process_profile_ir_rudics_cts5_usea_uvp_lpm_v2(a_uvpLpmDataV2, a_timeData, a_gpsData)
%
% INPUT PARAMETERS :
%   a_uvpLpmDataV2 : CTS5-USEA UVP-LPM V2 data
...
if (isempty(a_uvpLpmDataV2))
   return
end

% process the profiles
for idP = 1:length(a_uvpLpmDataV2)

   dataStruct = a_uvpLpmDataV2{idP};
   ...
   elseif (phaseId == g_decArgo_cts5PhaseSurface)
      ...
      fprintf('WARNING: Float #%d Cycle #%d: (Cy,Ptn)=(%d,%d): Nothing done yet for processing UVP-LPM V2 profiles with phase Id #%d\n', ...
         g_decArgo_floatNum, ...
         g_decArgo_cycleNum, ...
      ...
      switch (treatId)
         case {g_decArgo_cts5Treat_RW, g_decArgo_cts5Treat_AM, g_decArgo_cts5Treat_DW}
            ...
            profStruct.paramList = [ ...
               paramPres paramNbImPart paramTempPart paramNbSizeSpecPart paramGreySizeSpecPart ...
               ];
         ...
         otherwise
            fprintf('ERROR: Float #%d Cycle #%d: (Cy,Ptn)=(%d,%d): Treatment #%d not managed - UVP-LPM V2 data ignored\n', ...
               g_decArgo_floatNum, ...
               g_decArgo_cycleNum, ...
               g_decArgo_cycleNumFloat, ...
               g_decArgo_patternNumFloat, ...
