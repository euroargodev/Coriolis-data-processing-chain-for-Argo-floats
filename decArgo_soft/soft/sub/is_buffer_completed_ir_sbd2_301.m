...
%
% SYNTAX :
%  [o_completed, o_cycleProf] = is_buffer_completed_ir_sbd2_301
%
% INPUT PARAMETERS :
%
% OUTPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_completed, o_cycleProf] = is_buffer_completed_ir_sbd2_301
...
if (~isempty(g_decArgo_0TypeReceivedData) && ...
      ...
      ~isempty(g_decArgo_254TypeReceivedData) && ...
      ~isempty(g_decArgo_255TypeReceivedData))
   
   % nominal case (deep cycle)
...
elseif (isempty(g_decArgo_0TypeReceivedData) && ...
   ...
   % it is not a deep cycle (typically, it is a second iridium session)

   % the buffer is complete
   o_completed = 1;
   o_cycleProf = g_decArgo_phaseSurfWait;
   
elseif (isempty(g_decArgo_0TypeReceivedData) && ...
      ~isempty(g_decArgo_250TypeReceivedData) && ...
      ~isempty(g_decArgo_253TypeReceivedData))
   
   % it is a deep cycle but the float didn't sample any data
   ...
   for idM = 1:size(cycleProf, 1)
      ...
      % cycle
      id253Phase = find( ...
         (g_decArgo_253TypeReceivedData(:, 3) == g_decArgo_phaseSatTrans) | ...
         (g_decArgo_253TypeReceivedData(:, 3) == g_decArgo_phaseEndOfLife));
      if (isempty(id253Phase))
         return;
      else
         id253PhaseCyProf = find( ...
...
elseif (isempty(g_decArgo_0TypeReceivedData) && ...
   ...
   phaseList = unique(g_decArgo_253TypeReceivedData(:, 3));
   if ((length(phaseList) == 1) && (phaseList == g_decArgo_phaseEndOfLife))
   
      % the float is in EOL mode
      
      % the buffer is complete
      o_completed = 1;
      o_cycleProf = g_decArgo_phaseEndOfLife;
   end
end
