...
if ((~isempty(a_profCtdCpH)) && (~isempty(a_profCtdCp)))
   ...
   a_profCtdCpH.data = [a_profCtdCpH.data; ...
      [a_profCtdCp.data ...
      ones(size(a_profCtdCp.data, 1), 1)*double(paramVrsPh.fillValue) ...
      ones(size(a_profCtdCp.data, 1), 1)*double(paramNbSampleSfet.fillValue) ...
      ones(size(a_profCtdCp.data, 1), 1)*double(paramPhInSituFree.fillValue) ...
      ones(size(a_profCtdCp.data, 1), 1)*double(paramPhInSituTotal.fillValue)]];
   ...
   if (~isempty(a_profCtdCpH.dataAdj))
      a_profCtdCpH.dataAdj = [a_profCtdCpH.dataAdj; ...
         [a_profCtdCp.dataAdj ...
         ones(size(a_profCtdCp.dataAdj, 1), 1)*double(paramVrsPh.fillValue) ...
         ones(size(a_profCtdCp.dataAdj, 1), 1)*double(paramNbSampleSfet.fillValue) ...
         ones(size(a_profCtdCp.dataAdj, 1), 1)*double(paramPhInSituFree.fillValue) ...
         ones(size(a_profCtdCp.dataAdj, 1), 1)*double(paramPhInSituTotal.fillValue)]];
   ...
   elseif (~isempty(profCtdPtsStruct))
      ...
      if (~isempty(idShallow) || ~isempty(idDeep))
         ...
         if (~isempty(idShallow))
            profCtdCpHStruct.data = [profCtdCpHStruct.data; ...
               [profCtdPtsStruct.data(idShallow, :) ...
               ones(length(idShallow), 1)*double(paramNbSampleCtd.fillValue) ...
               ones(length(idShallow), 1)*double(paramVrsPh.fillValue) ...
               ones(length(idShallow), 1)*double(paramNbSampleSfet.fillValue) ...
               ones(length(idShallow), 1)*double(paramPhInSituFree.fillValue) ...
               ones(length(idShallow), 1)*double(paramPhInSituTotal.fillValue)]];
            [~, idSort] = sort(profCtdCpHStruct.data(:, idPresCtdCpH), 'descend');
            profCtdCpHStruct.data = profCtdCpHStruct.data(idSort, :);
            if (~isempty(profCtdCpHStruct.dataAdj))
               profCtdCpHStruct.dataAdj = [profCtdCpHStruct.dataAdj; ...
         ...
         if (~isempty(idDeep))
            profCtdCpHStruct.data = [profCtdCpHStruct.data; ...
               [profCtdPtsStruct.data(idDeep, :) ...
               ones(length(idDeep), 1)*double(paramNbSampleCtd.fillValue) ...
               ones(length(idDeep), 1)*double(paramVrsPh.fillValue) ...
               ones(length(idDeep), 1)*double(paramNbSampleSfet.fillValue) ...
               ones(length(idDeep), 1)*double(paramPhInSituFree.fillValue) ...
               ones(length(idDeep), 1)*double(paramPhInSituTotal.fillValue)]];
            [~, idSort] = sort(profCtdCpHStruct.data(:, idPresCtdCpH), 'descend');
            profCtdCpHStruct.data = profCtdCpHStruct.data(idSort, :);
            if (~isempty(profCtdCpHStruct.dataAdj))
               profCtdCpHStruct.dataAdj = [profCtdCpHStruct.dataAdj; ...
