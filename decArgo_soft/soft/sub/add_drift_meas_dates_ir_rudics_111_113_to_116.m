...
if (~isempty(a_dataCTDMeanDate))
   ...
   for idL = 1:length(idDrift)
      [a_dataCTDMeanDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(0, firstPres, ...
...
if (~isempty(a_dataCTDStdMedDate))
   idDrift = find(a_dataCTDStdMedDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataCTDStdMedDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(0, firstPres, ...
...
if (~isempty(a_dataCTDRawDate))
   idDrift = find(a_dataCTDRawDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataCTDRawDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(0, firstPres, ...
...
if (~isempty(a_dataOXYMeanDate))
   idDrift = find(a_dataOXYMeanDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataOXYMeanDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(1, firstPres, ...
...
if (~isempty(a_dataOXYStdMedDate))
   idDrift = find(a_dataOXYStdMedDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataOXYStdMedDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(1, firstPres, ...
...
if (~isempty(a_dataOXYRawDate))
   idDrift = find(a_dataOXYRawDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataOXYRawDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(1, firstPres, ...
...
if (~isempty(a_dataOCRMeanDate))
   idDrift = find(a_dataOCRMeanDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataOCRMeanDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(2, firstPres, ...
...
if (~isempty(a_dataOCRStdMedDate))
   idDrift = find(a_dataOCRStdMedDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataOCRStdMedDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(2, firstPres, ...
...
if (~isempty(a_dataOCRRawDate))
   idDrift = find(a_dataOCRRawDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataOCRRawDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(2, firstPres, ...
...
if (~isempty(a_dataECO2MeanDate))
   idDrift = find(a_dataECO2MeanDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataECO2MeanDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(3, firstPres, ...
...
if (~isempty(a_dataECO2StdMedDate))
   idDrift = find(a_dataECO2StdMedDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataECO2StdMedDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(3, firstPres, ...
...
if (~isempty(a_dataECO2RawDate))
   idDrift = find(a_dataECO2RawDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataECO2RawDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(3, firstPres, ...
...
if (~isempty(a_dataECO3MeanDate))
   idDrift = find(a_dataECO3MeanDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataECO3MeanDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(3, firstPres, ...
...
if (~isempty(a_dataECO3StdMedDate))
   idDrift = find(a_dataECO3StdMedDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataECO3StdMedDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(3, firstPres, ...
...
if (~isempty(a_dataECO3RawDate))
   idDrift = find(a_dataECO3RawDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataECO3RawDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(3, firstPres, ...
...
if (~isempty(a_dataFLNTUMeanDate))
   idDrift = find(a_dataFLNTUMeanDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataFLNTUMeanDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(4, firstPres, ...
...
if (~isempty(a_dataFLNTUStdMedDate))
   idDrift = find(a_dataFLNTUStdMedDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataFLNTUStdMedDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(4, firstPres, ...
...
if (~isempty(a_dataFLNTURawDate))
   idDrift = find(a_dataFLNTURawDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataFLNTURawDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(4, firstPres, ...
...
if (~isempty(a_dataCROVERMeanDate))
   idDrift = find(a_dataCROVERMeanDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataCROVERMeanDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(5, firstPres, ...
...
if (~isempty(a_dataCROVERStdMedDate))
   idDrift = find(a_dataCROVERStdMedDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataCROVERStdMedDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(5, firstPres, ...
...
if (~isempty(a_dataCROVERRawDate))
   idDrift = find(a_dataCROVERRawDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataCROVERRawDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(5, firstPres, ...
...
if (~isempty(a_dataSUNAMeanDate))
   idDrift = find(a_dataSUNAMeanDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataSUNAMeanDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(6, firstPres, ...
...
if (~isempty(a_dataSUNAStdMedDate))
   idDrift = find(a_dataSUNAStdMedDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataSUNAStdMedDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(6, firstPres, ...
...
if (~isempty(a_dataSUNARawDate))
   idDrift = find(a_dataSUNARawDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataSUNARawDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(6, firstPres, ...
...
if (~isempty(a_dataSUNAAPFDate))
   idDrift = find(a_dataSUNAAPFDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataSUNAAPFDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(6, firstPres, ...
...
if (~isempty(a_dataSUNAAPF2))
   if (~isempty(a_dataSUNAAPF2Date))
      idDrift = find(a_dataSUNAAPF2Date(:, 3) == g_decArgo_phaseParkDrift);
      for idL = 1:length(idDrift)
         [a_dataSUNAAPF2Date(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(6, firstPres, ...
...
if (~isempty(a_dataSEAFETMeanDate))
   idDrift = find(a_dataSEAFETMeanDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataSEAFETMeanDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(4, firstPres, ...
...
if (~isempty(a_dataSEAFETStdMedDate))
   idDrift = find(a_dataSEAFETStdMedDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataSEAFETStdMedDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(4, firstPres, ...
...
if (~isempty(a_dataSEAFETRawDate))
   idDrift = find(a_dataSEAFETRawDate(:, 3) == g_decArgo_phaseParkDrift);
   for idL = 1:length(idDrift)
      [a_dataSEAFETRawDate(idDrift(idL), 4:end)] = compute_drift_dates_ir_rudics_111_113_to_116(4, firstPres, ...
