% ------------------------------------------------------------------------------
% Create the IMU Tilt & Heading profiles of CTS5-USEA decoded data.
%
% SYNTAX :
%  [o_tabProfiles, o_tabDrift, o_tabSurf] = ...
%    process_profile_ir_rudics_cts5_usea_imu_tilt_heading(a_imuTiltHeading, a_timeData, a_gpsData)
%
% INPUT PARAMETERS :
%   a_imuTiltHeading : CTS5-USEA IMU Tilt & Heading data
...
if (isempty(a_imuTiltHeading))
   return
end

% process the profiles
for idP = 1:length(a_imuTiltHeading)
   
   dataStruct = a_imuTiltHeading{idP};
   ...
   elseif (phaseId == g_decArgo_cts5PhaseSurface)
      ...
      fprintf('WARNING: Float #%d Cycle #%d: (Cy,Ptn)=(%d,%d): Nothing done yet for processing IMU Tilt & Heading profiles with phase Id #%d\n', ...
         g_decArgo_floatNum, ...
         g_decArgo_cycleNum, ...
   ...
   if (~isempty(data))
      ...
      switch (treatId)
         case {g_decArgo_cts5Treat_RW, g_decArgo_cts5Treat_AM, g_decArgo_cts5Treat_DW}
            ...
            paramTilt = get_netcdf_param_attributes('TILT');
            paramHeading = get_netcdf_param_attributes('HEADING');

            profStruct.paramList = [ ...
               paramPres paramTilt paramHeading];
         ...
         otherwise
            fprintf('ERROR: Float #%d Cycle #%d: (Cy,Ptn)=(%d,%d): Treatment #%d not managed - IMU Tilt & Heading data ignored\n', ...
               g_decArgo_floatNum, ...
               g_decArgo_cycleNum, ...
               g_decArgo_cycleNumFloat, ...
               g_decArgo_patternNumFloat, ...
