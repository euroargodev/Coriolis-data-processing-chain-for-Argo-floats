...
%    decode_data_apx_9(a_argosDataData, a_argosDataUsed, a_argosDataDate, ...
...
profData = ones(9+(lastMsgNum-3)*29, 1)*hex2dec('FF');
profReceived = zeros(9+(lastMsgNum-3)*29, 1);
profRedundancy = ones(9+(lastMsgNum-3)*29, 1)*-1;
...
for idL = 1:size(a_sensorData, 1)
   ...
   if (msgNum == 1)
      ...
      dataStruct = get_apx_misc_data_init_struct('Data msg', msgNum, msgRed, msgDate);
      dataStruct.label = 'OPTODE status word';
      ...
      dataStruct.value = sensor_2_value_for_apex_apf9_voltage(decData(13));
      ...
      dataStruct.rawFormat = '%d';
      dataStruct.rawUnit = 'count';
      dataStruct.value = sensor_2_value_for_apex_apf9_current(decData(14));
      dataStruct.format = '%.3f';
      dataStruct.unit = 'mA';
      o_miscInfo{end+1} = dataStruct;
      ...
      dataStruct.rawFormat = '%d';
      dataStruct.rawUnit = 'count';
      dataStruct.value = sensor_2_value_for_apex_apf9_voltage(decData(15));
      dataStruct.format = '%.3f';
      dataStruct.unit = 'V';
      o_miscInfo{end+1} = dataStruct;
      ...
      dataStruct.rawFormat = '%d';
      dataStruct.rawUnit = 'count';
      dataStruct.value = sensor_2_value_for_apex_apf9_current(decData(16));
      dataStruct.format = '%.3f';
      dataStruct.unit = 'mA';
      o_miscInfo{end+1} = dataStruct;
      ...
      dataStruct.rawFormat = '%d';
      dataStruct.rawUnit = 'count';
      dataStruct.value = sensor_2_value_for_apex_apf9_voltage(decData(17));
      dataStruct.format = '%.3f';
      dataStruct.unit = 'V';
      o_miscInfo{end+1} = dataStruct;
      ...
      dataStruct.rawFormat = '%d';
      dataStruct.rawUnit = 'count';
      dataStruct.value = sensor_2_value_for_apex_apf9_current(decData(18));
      dataStruct.format = '%.3f';
      dataStruct.unit = 'mA';
      o_miscInfo{end+1} = dataStruct;
      ...
      dataStruct.rawFormat = '%d';
      dataStruct.rawUnit = 'count';
      dataStruct.value = sensor_2_value_for_apex_apf9_voltage(decData(19));
      dataStruct.format = '%.3f';
      dataStruct.unit = 'V';
      o_miscInfo{end+1} = dataStruct;
      ...
      for id = 2:length(idList)
         ...
         dataStruct.rawFormat = '%d';
         dataStruct.rawUnit = 'count';
         dataStruct.value = sensor_2_value_for_apex_apf9_voltage(decDataBis(id, 19));
         dataStruct.format = '%.3f';
         dataStruct.unit = 'V';
         o_miscInfo{end+1} = dataStruct;
      end
      ...
      tabBatVolt = [];
      for id = 1:length(idListFB)
         tabBatVolt(end+1) = sensor_2_value_for_apex_apf9_voltage(decDataBis(id, 19));
      end
      dataStruct = get_apx_tech_data_init_struct(msgRed);
      ...
      dataStruct.rawFormat = '%d';
      dataStruct.rawUnit = 'count';
      dataStruct.value = sensor_2_value_for_apex_apf9_current(decData(20));
      dataStruct.format = '%.3f';
      dataStruct.unit = 'mA';
      o_miscInfo{end+1} = dataStruct;
      ...
      for id = 2:length(idList)
         ...
         dataStruct.rawFormat = '%d';
         dataStruct.rawUnit = 'count';
         dataStruct.value = sensor_2_value_for_apex_apf9_current(decDataBis(id, 20));
         dataStruct.format = '%.3f';
         dataStruct.unit = 'mA';
         o_miscInfo{end+1} = dataStruct;
      end
      ...
      tabBatCur = [];
      for id = 1:length(idListFB)
         tabBatCur(end+1) = sensor_2_value_for_apex_apf9_current(decDataBis(id, 20));
      end
      dataStruct = get_apx_tech_data_init_struct(msgRed);
   ...
   elseif (msgNum == 3)
      ...
      firstBit = 17;
      % item bit lengths
      tabNbBits = [8 8 16 16 14 6 12 16 16 16 14 6 12];
      % get item bits
      decData = get_bits(firstBit, tabNbBits, msgData);
      ...
      for id = 2:length(idList)
         dataStruct = get_apx_misc_data_init_struct('Data msg bis', decDataNumBis(id), decDataRedBis(id), decDateBis(id));
         dataStruct.label = 'Air bladder pressure during transmission';
         dataStruct.value = decDataBis(id, 2);
         dataStruct.format = '%d';
         dataStruct.unit = 'count';
         o_miscInfo{end+1} = dataStruct;
      end
      ...
      tabBladPres = [];
      for id = 1:length(idListFB)
         tabBladPres(end+1) = decDataBis(id, 2);
      end
      dataStruct = get_apx_tech_data_init_struct(msgRed);
      ...
      surfPres = sensor_2_value_for_apex_apf9_pressure(decData(4), g_decArgo_presDef);
      surfTPhaseDoxy = sensor_2_value_for_apex_apf9_tPhaseDoxy(decData(5), g_decArgo_tPhaseDoxyDef);
      surfRPhaseDoxy = sensor_2_value_for_apex_apf9_rPhaseDoxy(decData(6), g_decArgo_rPhaseDoxyDef);
      surfTempDoxy = sensor_2_value_for_apex_apf9_tempDoxy(decData(7), g_decArgo_tempDoxyDef);
      
      % compute PPOX_DOXY
      surfDoxy = compute_PPOX_DOXY_1009( ...
      ...
      o_surfData.dataRed = repmat(msgRed, 1, 5);

      % data sampled at end of the park phase
      parkTemp = sensor_2_value_for_apex_apf9_temperature(decData(8), g_decArgo_tempDef);
      parkSal = sensor_2_value_for_apex_apf9_salinity(decData(9), g_decArgo_salDef);
      parkPres = sensor_2_value_for_apex_apf9_pressure(decData(10), g_decArgo_presDef);
      parkTPhaseDoxy = sensor_2_value_for_apex_apf9_tPhaseDoxy(decData(11), g_decArgo_tPhaseDoxyDef);
      parkRPhaseDoxy = sensor_2_value_for_apex_apf9_rPhaseDoxy(decData(12), g_decArgo_rPhaseDoxyDef);
      parkTempDoxy = sensor_2_value_for_apex_apf9_tempDoxy(decData(13), g_decArgo_tempDoxyDef);
      
      % compute DOXY
      parkDoxy = compute_DOXY_1009(parkTPhaseDoxy, parkTempDoxy, ...
         g_decArgo_tPhaseDoxyDef, g_decArgo_tempDoxyDef, ...
      ...
      % add parameter variables to the data structure
      o_parkData.paramList = [paramPres paramTemp paramSal paramTPhaseDoxy paramRPhaseDoxy paramTempDoxy paramDoxy];
      
      % add parameter data to the data structure
      o_parkData.data = [parkPres parkTemp parkSal parkTPhaseDoxy parkRPhaseDoxy parkTempDoxy parkDoxy];
      
      % add parameter data redundancy to the profile structure
      o_parkData.dataRed = repmat(msgRed, 1, 7);  

      % store profile data
      profData(1:9) = msgData(23:31);
      profReceived(1:9) = 255;
      profRedundancy(1:9) = msgRed;
      ...
      % store profile data
      profData(9+(msgNum-4)*29+1:9+(msgNum-4)*29+29) = msgData(3:31);
      profReceived(9+(msgNum-4)*29+1:9+(msgNum-4)*29+29) = 255;
      profRedundancy(9+(msgNum-4)*29+1:9+(msgNum-4)*29+29) = msgRed;
...
% decode profile data
nbLev = floor(length(profData)/10);
...
if (nbLev > 0)
   ...
   firstBit = 1;
   % item bit lengths
   tabNbBits = repmat([16 16 16 14 6 12], 1, nbLev);
   % get item bits
   ...
   for idLev = 1:nbLev
      ...
      if ((receivedData(id+4) == 16383) && (decData(id+4) ~= 16383))
         tPhaseDoxy = sensor_2_value_for_apex_apf9_tPhaseDoxy(decData(id+4), g_decArgo_tPhaseDoxyDef);
      else
         tPhaseDoxy = g_decArgo_tPhaseDoxyDef;
      end
      if ((receivedData(id+5) == 63) && (decData(id+5) ~= 63))
         rPhaseDoxy = sensor_2_value_for_apex_apf9_rPhaseDoxy(decData(id+5), g_decArgo_rPhaseDoxyDef);
      else
         rPhaseDoxy = g_decArgo_rPhaseDoxyDef;
      end
      if ((receivedData(id+6) == 4095) && (decData(id+6) ~= 4095))
         tempDoxy = sensor_2_value_for_apex_apf9_tempDoxy(decData(id+6), g_decArgo_tempDoxyDef);
      else
         tempDoxy = g_decArgo_tempDoxyDef;
      end
      
      profPres = [profPres; pres];
      profPresRaw = [profPresRaw; profData((idLev-1)*10+5) profData((idLev-1)*10+6)];
      profTemp = [profTemp; temp];
      profSal = [profSal; sal];
      profTPhaseDoxy = [profTPhaseDoxy; tPhaseDoxy];
      profRPhaseDoxy = [profRPhaseDoxy; rPhaseDoxy];
      profTempDoxy = [profTempDoxy; tempDoxy];

   end
   
   % manage data redundancy
   redData = ones(nbLev*6, 1)*-1;
   for id = 1:nbLev
      redData((id-1)*6+1) = min(profRedundancy((id-1)*10+1), profRedundancy((id-1)*10+2));
      redData((id-1)*6+2) = min(profRedundancy((id-1)*10+3), profRedundancy((id-1)*10+4));
      redData((id-1)*6+3) = min(profRedundancy((id-1)*10+5), profRedundancy((id-1)*10+6));
      redData((id-1)*6+4) = min(profRedundancy((id-1)*10+7), profRedundancy((id-1)*10+8));
      redData((id-1)*6+5) = min(profRedundancy((id-1)*10+8), profRedundancy((id-1)*10+9));
      redData((id-1)*6+6) = min(profRedundancy((id-1)*10+9), profRedundancy((id-1)*10+10));
   end
   profTempRed = redData(1:6:end);
   profSalRed = redData(2:6:end);
   profPresRed = redData(3:6:end);
   profTPhaseDoxyRed = redData(4:6:end);
   profRPhaseDoxyRed = redData(4:6:end);
   profTempDoxyRed = redData(5:6:end);
   ...
   if (profileLength >= 0)
      ...
      profTPhaseDoxy(profileLength+1:end) = [];
      profRPhaseDoxy(profileLength+1:end) = [];
      ...
      profTPhaseDoxyRed(profileLength+1:end) = [];
      profRPhaseDoxyRed(profileLength+1:end) = [];
      ...
      if (~isempty(idStop))
         ...
         profTPhaseDoxy(profileLength2+1:end) = [];
         profRPhaseDoxy(profileLength2+1:end) = [];
         ...
         profTPhaseDoxyRed(profileLength2+1:end) = [];
         profRPhaseDoxyRed(profileLength2+1:end) = [];
      ...
      if (profPres(end) <= 6)
         ...
         if (~isempty(idLastPres))
            idOk = find(rem(idLastPres-5, 10) == 0);
            if (~isempty(idOk))
               idLastPres = idLastPres(idOk);
               firstAuxByte = idLastPres(1) + 6;
            end
         end
      end
   end
   ...
   idDel = (find((profPres == g_decArgo_presDef) & ...
      ...
      (profTPhaseDoxy == g_decArgo_tPhaseDoxyDef) & ...
      (profRPhaseDoxy == g_decArgo_rPhaseDoxyDef) & ...
      (profTempDoxy == g_decArgo_tempDoxyDef)));
   profPres(idDel) = [];
   profTemp(idDel) = [];
   profSal(idDel) = [];
   profTPhaseDoxy(idDel) = [];
   profRPhaseDoxy(idDel) = [];
   profTempDoxy(idDel) = [];
   
   profPresRed(idDel) = [];
   profTempRed(idDel) = [];
   profSalRed(idDel) = [];
   profTPhaseDoxyRed(idDel) = [];
   profRPhaseDoxyRed(idDel) = [];
   ...
   % compute DOXY
   profDoxy = compute_DOXY_1009(profTPhaseDoxy, profTempDoxy, ...
      g_decArgo_tPhaseDoxyDef, g_decArgo_tempDoxyDef, ...
      profPres, profTemp, profSal, ...
      g_decArgo_presDef, g_decArgo_tempDef, g_decArgo_salDef, ...
      g_decArgo_doxyDef);
   
   profDoxyRed = min([profPresRed profTempRed profSalRed profTPhaseDoxyRed profRPhaseDoxyRed], [], 2);
   ...
   profTPhaseDoxy(find(profTPhaseDoxy == g_decArgo_tPhaseDoxyDef)) = paramTPhaseDoxy.fillValue;
   profRPhaseDoxy(find(profRPhaseDoxy == g_decArgo_rPhaseDoxyDef)) = paramRPhaseDoxy.fillValue;
   ...
   o_profData.paramList = [paramPres paramTemp paramSal paramTPhaseDoxy paramRPhaseDoxy paramTempDoxy paramDoxy];
   
   % add parameter data to the profile structure
   o_profData.data = [profPres profTemp profSal profTPhaseDoxy profRPhaseDoxy profTempDoxy profDoxy];
   
   % add parameter data redundancy to the profile structure
   o_profData.dataRed = [profPresRed profTempRed profSalRed profTPhaseDoxyRed profRPhaseDoxyRed profTempDoxyRed profDoxyRed];
end

% decode auxiliary engineering data
if (((profileLength >= 0) && (length(profData) > profileLength*10)) || ...
      ((firstAuxByte ~= -1) && (length(profData) >= firstAuxByte)))
   
   if (profileLength >= 0)
      auxData = profData(nbLev*10+1:end);
      msgRed = max(profRedundancy(nbLev*10+1:end)); % because we have only one useful redundancy (with -1 if the first bytes have not been received, see 3901080 #145)
      auxReceived = profReceived(nbLev*10+1:end);
   else
