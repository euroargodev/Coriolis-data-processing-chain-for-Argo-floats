...
%
% SYNTAX :
%  [o_croverData] = decode_apmt_crover(a_fileNameInfo)
%
% INPUT PARAMETERS :
%   a_fileNameInfo : information on APMT CROVER file to decode
%
% OUTPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_croverData] = decode_apmt_crover(a_fileNameInfo)
...
if ~(exist(inputFilePathName, 'file') == 2)
   fprintf('ERROR: decode_apmt_crover: File not found: %s\n', inputFilePathName);
   return
end
...
try
   ...
   switch (data(1))
      case {10}
         o_croverData = decode_apmt_crover_data(data, lastByteNum, inputFilePathName);
      otherwise
