...
%
% SYNTAX :
%  [o_bitsToFreeze] = get_apx_bits_to_freeze(a_decoderId)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_bitsToFreeze] = get_apx_bits_to_freeze(a_decoderId)
...
bitsToFreeze = [];
switch (a_decoderId)
   case {1, 3, 11, 12, 17, 24} % V4.2 & V4.5 & V4.21 & V4.22 & V4.51 & V4.23
   ...
   otherwise
      fprintf('WARNING: Float #%d: Nothing done yet in get_apx_bits_to_freeze for decoderId #%d\n', ...
         g_decArgo_floatNum, ...
         a_decoderId);
end
