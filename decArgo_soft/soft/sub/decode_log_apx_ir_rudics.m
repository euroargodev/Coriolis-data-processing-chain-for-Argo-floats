% ------------------------------------------------------------------------------
% Decode log files from one cycle of APEX Iridium Rudics data.
%
% SYNTAX :
...
%    o_presOffsetData] = ...
%    decode_log_apx_ir_rudics(a_logFileList, a_presOffsetData, a_decoderId)
%
% INPUT PARAMETERS :
...
for idFile = 1:length(a_logFileList)
   ...
   if (any(strcmp({events.cmd}, 'LogConfiguration()')))
      ...
      configDataEvts = parse_apx_ir_rudics_config_data_evts(events(idEvts));
   ...
   if (any(strcmp({events.cmd}, 'GetSurfaceObs()')))
      ...
      o_surfData = process_apx_ir_rudics_surf_data_evts(events(idEvts), a_decoderId); % i.e. only the one of the last log file is used
   end
   
   if (any(strcmp({events.cmd}, 'GpsServices()')))
      idEvts = find(strcmp({events.cmd}, 'GpsServices()'));
      [gpsData, o_gpsInfo] = process_apx_ir_rudics_gps_data_evts(events(idEvts));
      if (~isempty(gpsData))
   ...
   if (any(strcmp({events.cmd}, 'Descent()')))
      ...
      o_pMarkData = parse_apx_ir_rudics_pmark_data_evts(events(idEvts)); % i.e. only the one of the last log file is used
   end
   
   o_timeData = process_apx_ir_rudics_time_data_evts(events, a_decoderId); % i.e. only the one of the last log file is used
