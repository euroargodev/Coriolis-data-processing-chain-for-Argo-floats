...
netcdf.putAtt(fCdf, launchConfigParameterValueVarId, '_FillValue', double(999999));
...
netcdf.putAtt(fCdf, configParameterValueVarId, '_FillValue', double(999999));
...
for idMeta = 1:length(a_inputAuxMetaName)
   ...
   metaDescription = g_decArgo_outputNcConfParamDescription{find(strcmp(metaName, g_decArgo_outputNcConfParamLabel), 1)};
...
if (~isempty(a_launchAuxConfigValue))
   a_launchAuxConfigValue(isnan(a_launchAuxConfigValue)) =  netcdf.getAtt(fCdf, launchConfigParameterValueVarId, '_FillValue');
   netcdf.putVar(fCdf, launchConfigParameterValueVarId, 0, length(a_launchAuxConfigValue), a_launchAuxConfigValue);
end
...
if (~isempty(a_missionAuxConfigValue))
   a_missionAuxConfigValue(isnan(a_missionAuxConfigValue)) =  netcdf.getAtt(fCdf, configParameterValueVarId, '_FillValue');
   netcdf.putVar(fCdf, configParameterValueVarId, [0 0], size(a_missionAuxConfigValue), a_missionAuxConfigValue);
end
