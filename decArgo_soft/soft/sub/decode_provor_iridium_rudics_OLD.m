...
global g_decArgo_tmp2Directory;
...
g_decArgo_tmp2Directory = [floatIriDirName 'mat/'];
if ~(exist(g_decArgo_tmp2Directory, 'dir') == 7)
   mkdir(g_decArgo_tmp2Directory);
end
...
if (a_floatDmFlag == 0)
   ...
   if (g_decArgo_realtimeFlag == 1)
      ...
      [sbdFileNameList, sbdFileRank, sbdFileDate, sbdFileCyNum, sbdFileProfNum] = ...
         read_buffer_list(a_floatNum, g_decArgo_dirInputDmBufferList, g_decArgo_archiveDmDirectory);
...
if (isempty(g_decArgo_outputCsvFileId))
   ...
   if (g_decArgo_realtimeFlag == 1)
      ...
      % in RT save the list of already processed rsync lo files in the tmp
      ...
      % in RT save the list of used rsync lo files in the tmp
      % directory of the float
