...
%    create_prv_drift_25(a_tabDrifCTDO, a_nbDriftMeas, ...
%    a_descentStartDate, a_floatClockDrift, ...
%    a_descentEndDate, a_descentToProfStartDate, ...
%    a_driftSamplingPeriod)
%
% INPUT PARAMETERS :
%   a_tabDrifCTDO            : drift CTD0 data
%   a_nbDriftMeas            : number of CTD0 measurements in drift
...
driftSampPeriodDay = double(a_driftSamplingPeriod)/24;

firstDriftMesDate = floor((a_descentEndDate-zeroOr3Min)*24)/24 + driftSampPeriodDay;
...
for idMsg = 1:size(a_tabDrifCTDO, 1)
   ...
   for idMes = 1:a_tabDrifCTDO(idMsg, 4)
      ...
      parkTemp = a_tabDrifCTDO(idMsg, 17+idMes-1);
      parkSal = a_tabDrifCTDO(idMsg, 23+idMes-1);
      parkOxy = a_tabDrifCTDO(idMsg, 29+idMes-1);
...
o_parkRawDoxy = ones(nbMeasTot, 1)*g_decArgo_molarDoxyCountsDef;
...
for id = 1:size(a_tabDrifCTDO, 1)
   ...
   for idMes = 1:a_tabDrifCTDO(idMsg, 4)
      ...
      parkPres = a_tabDrifCTDO(idMsg, 5+idMes-1);
      parkPresOk = a_tabDrifCTDO(idMsg, 11+idMes-1);
      parkTemp = a_tabDrifCTDO(idMsg, 17+idMes-1);
      parkSal = a_tabDrifCTDO(idMsg, 23+idMes-1);
      parkOxy = a_tabDrifCTDO(idMsg, 29+idMes-1);
