...
%
% SYNTAX :
%  print_data_in_csv_file_ir_rudics_cts5_RAMSES(a_ramsesData, a_ramses2Data)
%
% INPUT PARAMETERS :
%   a_ramsesData  : CTS5-USEA RAMSES data
%   a_ramses2Data : CTS5-USEA RAMSES2 data
%
% OUTPUT PARAMETERS :
%
% EXAMPLES :
...
% ------------------------------------------------------------------------------
function print_data_in_csv_file_ir_rudics_cts5_RAMSES(a_ramsesData, a_ramses2Data)
...
if (isempty(a_ramsesData) && isempty(a_ramses2Data))
   return
end
      ...
      switch (treat)
         case {'(RW)', '(DW)'}
            fprintf(g_decArgo_outputCsvFileId, ['%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; ' ...
               ...
               g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
               fileTypeStr, phase, sensorNum, sensorName);
            fprintf(g_decArgo_outputCsvFileId, '; RAW_UPWELLING_RADIANCE_%d (count)', 1:size(data, 2)-9);
            fprintf(g_decArgo_outputCsvFileId, '\n');
            outputFmt = ['%s; %s;%.1f;%d;%.2f;%.2f;%.2f;%.2f;%d;%d' repmat(';%d', 1, size(data, 2)-9)];
            if (strcmp(treat, '(RW)'))
               measType = 'raw';
         ...
         otherwise
            fprintf('ERROR: Float #%d: RAMSES2 treatment type not managed: %s\n', ...
               g_decArgo_floatNum, ...
               treat);
      end
