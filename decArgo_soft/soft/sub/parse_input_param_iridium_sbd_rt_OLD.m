% ------------------------------------------------------------------------------
% Parse input parameters and, for each Iridium mail file to process, create the
% list of associated float WMO number, float Imei and Iridium mail file name.
%
% SYNTAX :
%  [o_floatList, o_inputError] = ...
%    parse_input_param_iridium_sbd(a_varargin)
%
% INPUT PARAMETERS :
   ...
   if (rem(length(a_varargin), 2) ~= 0)
      ...
      for id = 1:2:length(a_varargin)
         ...
         elseif (strcmpi(a_varargin{id}, 'floatwmolist'))
            if (isempty(floatWmo) && isempty(floatWmoList))
               floatWmoList = load(a_varargin{id+1});
...
if (ryncParamFlag == 1)
   if (~isempty(ryncLogList))
      for idFloat = 1:length(ryncLogList)
         [floatImei, floatMailFiles, rsyncLogName] = parse_rsync_log_ir_sbd(ryncLogList{idFloat});
...
% filter the tabFloatImei and tabFloatMailFiles according to float list
