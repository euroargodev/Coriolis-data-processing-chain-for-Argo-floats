...
if (a_floatProgSensor(a_dataIndex, 3) == 0)
   ...
   for id = 0:48
      ...
      fprintf(g_decArgo_outputCsvFileId, '%d; %d; %d; %s; sensor prog param; %s; %d;;%s\n', ...
         g_decArgo_floatNum, a_cycleNum, a_profNum, get_phase_name(-1), ...
         name, a_floatProgSensor(a_dataIndex, id+8), ...
         sprintf('<PC %d 0 %d %d>', a_floatProgSensor(a_dataIndex, 2), id, a_floatProgSensor(a_dataIndex, id+8)));
   end
else
   ...
   for id = 0:lastId
      ...
      fprintf(g_decArgo_outputCsvFileId, '%d; %d; %d; %s; sensor prog param; %s; %g;;%s\n', ...
         g_decArgo_floatNum, a_cycleNum, a_profNum, get_phase_name(-1), ...
         name, a_floatProgSensor(a_dataIndex, id+8), ...
         sprintf('<PC %d 1 %d %g>', a_floatProgSensor(a_dataIndex, 2), id, a_floatProgSensor(a_dataIndex, id+8)));
   end
end
