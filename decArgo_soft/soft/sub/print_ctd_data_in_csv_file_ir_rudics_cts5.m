...
%
% SYNTAX :
%  print_ctd_data_in_csv_file_ir_rudics_cts5(a_ctdData)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function print_ctd_data_in_csv_file_ir_rudics_cts5(a_ctdData)
...
for idP = 1:length(a_ctdData)
   ...
   switch (treat)
      case {'(RW)', '(AM)', '(SS)'}
         fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; PRES (dbar); TEMP (°C); PSAL (PSU)\n', ...
      ...
      case '(AM)(SD)'
         fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; PRES (dbar); TEMP (°C); PSAL (PSU); TEM_STDP (°C); PSAL_STD (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
            fileTypeStr, phase, sensorNum, sensorName);
         outputFmt = '%s; %s;%.2f;%.4f;%.3f;%.4f;%.3f';
         measType = 'mean + stdev';
      case '(AM)(MD)'
         fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; PRES (dbar); TEMP (°C); PSAL (PSU); PRES_MED (dbar); TEMP_MED (°C); PSAL_MED (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
            fileTypeStr, phase, sensorNum, sensorName);
         outputFmt = '%s; %s;%.2f;%.4f;%.3f;%.2f;%.4f;%.3f';
         measType = 'mean + median';
      case '(AM)(SD)(MD)'
         fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; PRES (dbar); TEMP (°C); PSAL (PSU); TEMP_STD  (°C); PSAL_STD  (PSU); PRES_MED (dbar); TEMP_MED (°C); PSAL_MED (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
            fileTypeStr, phase, sensorNum, sensorName);
