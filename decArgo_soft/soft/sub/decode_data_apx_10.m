...
%    decode_data_apx_10(a_argosDataData, a_argosDataUsed, a_argosDataDate, ...
...
profData = ones(19+(lastMsgNum-3)*29, 1)*hex2dec('FF');
profReceived = zeros(19+(lastMsgNum-3)*29, 1);
profRedundancy = ones(19+(lastMsgNum-3)*29, 1)*-1;
...
for idL = 1:size(a_sensorData, 1)
   ...
   if (msgNum == 1)
      ...
      firstBit = 17;
      % item bit lengths
      tabNbBits = [1 2 1 1 2 1 2 1 1 1 1 4 1 1 1 1 1 1 1 1 1 2]*8;
      % get item bits
   ...
   elseif (msgNum == 3)
      ...
      % store profile data
      profData(1:19) = msgData(13:31);
      profReceived(1:19) = 255;
      profRedundancy(1:19) = msgRed;
      ...
      % store profile data
      profData(19+(msgNum-4)*29+1:19+(msgNum-4)*29+29) = msgData(3:31);
      profReceived(19+(msgNum-4)*29+1:19+(msgNum-4)*29+29) = 255;
      profRedundancy(19+(msgNum-4)*29+1:19+(msgNum-4)*29+29) = msgRed;
