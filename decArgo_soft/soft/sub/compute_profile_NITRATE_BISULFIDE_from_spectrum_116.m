...
%    compute_profile_NITRATE_BISULFIDE_from_spectrum_116( ...
...
% compute pixel interval that covers the [217 nm, max nm] wavelength interval
...
% the start pixel to use is
startPixelId = pixelBegin-floatPixelBegin+1;

% select useful data
tabOpticalWavelengthUv = tabOpticalWavelengthUv(pixelBegin:pixelEnd-(startPixelId-1));
tabENitrate = tabENitrate(pixelBegin:pixelEnd-(startPixelId-1));
tabESwaNitrate = tabESwaNitrate(pixelBegin:pixelEnd-(startPixelId-1));
tabEBisulfide = tabEBisulfide(pixelBegin:pixelEnd-(startPixelId-1));
tabUvIntensityRefNitrate = tabUvIntensityRefNitrate(pixelBegin:pixelEnd-(startPixelId-1));

tabUvIntensityNitrate = a_UV_INTENSITY_NITRATE(:, startPixelId:end);
...
if (~isempty(idNoDef))
   ...
   tabMolarBisulfide = nan(size(tabUvIntensityDarkNitrate, 1), 1);
   ...
   if (nbComplex > 0)
      fprintf('WARNING: Float #%d Cycle #%d Profile #%d: %d NITRATE and BISULFIDE values are complex - these values are set to fill value in ''%c'' profile of SUNA sensor\n', ...
         g_decArgo_floatNum, ...
         a_profSuna.cycleNumber, ...
         a_profSuna.profileNumber, ...
         nbComplex, ...
   ...
   if (any(isinf(o_BISULFIDE)))
      idToDef = find(isinf(o_BISULFIDE));
